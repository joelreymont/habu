...
FFI: SM-CUMEMFREE ( cuda-devptr -- rc ) SM-SYM cuMemFree_v2 FFI;
...
: SM-RUN ( ptr u8 n -- ) {: pa pu :}               \ run softmax cubin, fill SM-OUT
   ...
   s" libcuda.so.1" SM-LIB >CSTR
   SM-LIB RTLD-NOW DLOPEN CUDA-HANDLE0 SM-H !
   0 SM-CUINIT CUDA-RC0
   SM-DEV 0 >IDX SM-CUDEVICEGET CUDA-RC0
   SM-CTX SM-DEV @ >CUDA-DEV SM-CUDEVICEPRIMARYCTXRETAIN CUDA-RC0
   SM-CTX @ >CUDA-CTX SM-CUCTXSETCURRENT CUDA-RC0
   pa pu SM-PATH >CSTR
   SM-MOD SM-PATH SM-CUMODULELOAD CUDA-RC0
   s" SOFTMAX_ROWS" SM-KN >CSTR
   SM-FUNC SM-MOD @ >CUDA-MOD SM-KN SM-CUMODULEGETFUNCTION CUDA-RC0
   SM-DI 16 >LEN SM-CUMEMALLOC CUDA-RC0
   SM-DO 16 >LEN SM-CUMEMALLOC CUDA-RC0
   SM-DI @ >CUDA-DEVPTR SM-IN 16 >LEN SM-CUMEMCPYHTOD CUDA-RC0
   4 SM-KV !
   SM-FUNC @ >CUDA-FN 256 1 1 SM-CUFUNCSETBLOCKSHAPE CUDA-RC0
   SM-FUNC @ >CUDA-FN 20 >LEN SM-CUPARAMSETSIZE CUDA-RC0
   SM-FUNC @ >CUDA-FN 0 >IDX  SM-DI 8 >LEN SM-CUPARAMSETV CUDA-RC0
   SM-FUNC @ >CUDA-FN 8 >IDX  SM-DO 8 >LEN SM-CUPARAMSETV CUDA-RC0
   SM-FUNC @ >CUDA-FN 16 >IDX SM-KV 4 >LEN SM-CUPARAMSETV CUDA-RC0
   SM-FUNC @ >CUDA-FN 1 1 SM-CULAUNCHGRID CUDA-RC0
   SM-CUCTXSYNCHRONIZE CUDA-RC0
   SM-OUT SM-DO @ >CUDA-DEVPTR 16 >LEN SM-CUMEMCPYDTOH CUDA-RC0
   SM-DI @ >CUDA-DEVPTR SM-CUMEMFREE CUDA-RC0
   SM-DO @ >CUDA-DEVPTR SM-CUMEMFREE CUDA-RC0
   SM-MOD @ >CUDA-MOD SM-CUMODULEUNLOAD CUDA-RC0
   SM-DEV @ >CUDA-DEV SM-CUDEVICEPRIMARYCTXRELEASE CUDA-RC0 ;
