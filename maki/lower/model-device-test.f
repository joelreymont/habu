...
\ LOWER-MODEL-GOLDEN -> per-element evidence, plus the cad.f promotion refusal and the
...
\ device buffer. Then the cad.f gate path on the FFN model: device GOLDEN passes, while PROMOTE
\ refuses and writes nothing because no independent certifier or profiler exists yet.
\
...
\ the same FFN model through the cad.f gate: device golden passes, promotion remains refused
LMDM-CAD-GATE
