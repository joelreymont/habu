...
\ than a positional one; the generated REV-ID--RESULT:OK / :WRONG-WIDTH / :UNKNOWN
\ constructors and every MATCH site are unchanged, because both spellings and payload
\ binding order derive from the package, the family tail, and the declaration order,
\ none of which the mode changes.
ENUM id-result 1
