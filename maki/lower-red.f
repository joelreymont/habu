   ...
   MIR-N@ 0 ?do
      ...
      node rid LRED-IN-REGION? if
         node MIR-MOVE? if node MIR-NODE-REF MVW-CHECK
            ...
            node MIR-OP@ dup LRED-RED-OP? swap LRED-EW-OP? or 0= if E-LRED-OP throw then
         then
      then
   loop
...
: LRED-NR@ ( CAD-KIND:node-id -- n ) {: nd:CAD-KIND:node-id :}
   nd NODE>RAW {: raw:n :}
   raw 0 < raw LRED-NCAP >= or if E-LRED-REG throw then  LRED-NODE-REG raw cells + @ ;
: LRED-NR! ( n CAD-KIND:node-id -- ) {: r:n nd:CAD-KIND:node-id :}
   nd NODE>RAW {: raw:n :}
   raw 0 < raw LRED-NCAP >= or if E-LRED-REG throw then  r LRED-NODE-REG raw cells + ! ;
...
: LRED-REF-REG ( MIR:operand-ref -- n ) {: ref:MIR:operand-ref :}
   ...
   ref MIR-REF-INPUT? if E-LRED-REG throw then     \ input slot that is not a region input
   ref MIR-REF-NODE LRED-NR@ ;                      \ interior producer: its result reg
