...
\ spawn (not bare fork) also makes the test safe after CUDA was initialized in
\ another process (CUDA is fork-unsafe). The
...
\ Run: bin/hb --load maki/eval/device-fault-test.f
...
\
\ This is a manual device test and fails when the CUDA prerequisite is absent.
...
: EDFT-RUN ( -- )
   T-RESET
   CUDA:OPEN? 0= if
      s" eval-device-fault: CUDA device is required" 74 die then
