...
ROW-ELEMS 4 * constant ROW-CAP
CAP ROW-ELEMS * constant CACHE-ELEMS
CACHE-ELEMS 4 * constant CACHE-CAP
CACHE-CAP 8 + constant CACHE-ALLOC-CAP
ROW-CAP 8 + constant ROW-ALLOC-CAP
...
create ROW-Q ROW-CAP allot
create ROW-K ROW-CAP allot
create ROW-V ROW-CAP allot
create ROW-OUT ROW-ALLOC-CAP allot
create K-READ CACHE-ALLOC-CAP allot
create V-READ CACHE-ALLOC-CAP allot
...
: SIZES! ( -- )
   0 HEADS HD CAP GPT2-ATTN:LAUNCH-CHECK
   SHARED-N ! CACHE-N ! ROW-N !
   ROW-N @ ROW-CAP <> if E-PTX-BLOCK throw then
   CACHE-N @ CACHE-CAP <> if E-PTX-BLOCK throw then
   SHARED-N @ CAP 1+ 4 * <> if E-PTX-BLOCK throw then ;
...
: DEVICE-SETUP ( -- )
   ...
   DQ ROW-N @ >LEN CUDA:CU-MEM-ALLOC CUDA:RC0
   DQ @ >CUDA-DEVPTR CUDA-SCOPE:OWN-DEVPTR
   DK ROW-N @ >LEN CUDA:CU-MEM-ALLOC CUDA:RC0
   DK @ >CUDA-DEVPTR CUDA-SCOPE:OWN-DEVPTR
   DV ROW-N @ >LEN CUDA:CU-MEM-ALLOC CUDA:RC0
   DV @ >CUDA-DEVPTR CUDA-SCOPE:OWN-DEVPTR
   DKC CACHE-N @ 8 + >LEN CUDA:CU-MEM-ALLOC CUDA:RC0
   DKC @ >CUDA-DEVPTR CUDA-SCOPE:OWN-DEVPTR
   DVC CACHE-N @ 8 + >LEN CUDA:CU-MEM-ALLOC CUDA:RC0
   DVC @ >CUDA-DEVPTR CUDA-SCOPE:OWN-DEVPTR
   DOUT ROW-N @ 8 + >LEN CUDA:CU-MEM-ALLOC CUDA:RC0
   DOUT @ >CUDA-DEVPTR CUDA-SCOPE:OWN-DEVPTR
   DKC @ 4 + PKC !
   DVC @ 4 + PVC !
   DOUT @ 4 + POUT !
   DKC @ >CUDA-DEVPTR PTXSENT:POISON CACHE-N @ 4 / 2 + >COUNT CUDA:CU-MEMSET-D32 CUDA:RC0
   DVC @ >CUDA-DEVPTR PTXSENT:POISON CACHE-N @ 4 / 2 + >COUNT CUDA:CU-MEMSET-D32 CUDA:RC0 ;
...
: DEVICE-LAUNCH ( n n n n -- ) {: pos:n heads:n width:n cap:n :}
   ...
   shared SHARED-N @ <> if E-PTX-BLOCK throw then
...
: ROW-RUN ( n -- ) {: pos:n :}
   pos ROW-PACK
   DQ @ >CUDA-DEVPTR ROW-Q ROW-N @ >LEN CUDA:HTOD
   DK @ >CUDA-DEVPTR ROW-K ROW-N @ >LEN CUDA:HTOD
   DV @ >CUDA-DEVPTR ROW-V ROW-N @ >LEN CUDA:HTOD
   DOUT @ >CUDA-DEVPTR PTXSENT:POISON ROW-N @ 4 / 2 + >COUNT CUDA:CU-MEMSET-D32 CUDA:RC0
   pos HEADS HD CAP DEVICE-LAUNCH
   CUDA:CU-CTX-SYNCHRONIZE CUDA:RC0
   ROW-OUT DOUT @ >CUDA-DEVPTR ROW-N @ 8 + >LEN CUDA:DTOH
   ROW-OUT F32-BUF:LOAD PTXSENT:POISON T=
   ROW-OUT ROW-N @ 4 + + F32-BUF:LOAD PTXSENT:POISON T=
   pos REF-CHECK ;

: CACHE-CHECK-ONE ( ptr u8 ptr a -- ) {: read:ptr host:ptr :}
   read F32-BUF:LOAD PTXSENT:POISON T=
   read CACHE-N @ 4 + + F32-BUF:LOAD PTXSENT:POISON T=
   CACHE-ELEMS 0 ?do
...
: CACHE-CHECK ( -- )
   K-READ DKC @ >CUDA-DEVPTR CACHE-N @ 8 + >LEN CUDA:DTOH
   V-READ DVC @ >CUDA-DEVPTR CACHE-N @ 8 + >LEN CUDA:DTOH
...
: MAIN ( -- )
   ...
   DEVICE-RUN
   PTXTC:CLEAN
   s" gpt2 attention: positions 0..16 match independent F32 golden; sizes, refusals, and canaries intact" type cr
   T-REPORT ;
