...
\ GPT2-CLI owns errors -5664..-5666.
...
: LOAD-BPE ( ptr u8 n -- ) {: root:ptr rootu:n :}
   ...
   PATH pathu DIG SHA256-FILE-HEX THROW?
   DIG SHA-LEN GPT2PIN:MERGES-SHA256$ STR= 0= if E-DIGEST throw then
   PATH pathu MAKI:BPF-LOAD drop ;

: ENCODE ( ptr u8 n -- n ) {: prompt:ptr promptu:n :}
   promptu 0= if E-PROMPT throw then
   prompt promptu IDS ID-CAP MAKI:BPR-ENCODE
   dup 0= if drop E-PROMPT throw then ;

: ROW-LEN ( -- CAD-NUM:byte-len )
   MAKI:BPR-VOCAB F32-BYTES *
...
: GENERATE
   ( GPT2:model CAD-NUM:byte-len n n n -- GPT2:model n )
   {: bytes:CAD-NUM:byte-len pos:n count:n staged:n :}
   staged CONT-N = if 0 exit then
   pos count staged NEXT-ID LOGITS bytes GPT2:GREEDY
   MATCH result
      err OF ENDOF
      ok OF
         {: id:n :}
         pos count < if
            ...
            next count = if
               id ARM
               bytes next count 1 recurse
            else
               bytes next count staged recurse
            then
         else
            id staged STAGE
            bytes pos count staged 1+ recurse
         then
      ENDOF
   ;MATCH ;
...
: DECODE-STAGED ( -- )
   REQUIRE-STAGED
   IDS CONT-N OUT OUT-CAP MAKI:BPR-DECODE OUT-U ! ;

: OPEN-GENERATE ( ptr u8 n CAD-NUM:byte-len n -- )
   {: root:ptr rootu:n bytes:CAD-NUM:byte-len count:n :}
   root rootu FS-PATH:MAKE GPT2:OPEN
   MATCH result
      err OF throw ENDOF
      ok OF
         bytes 0 count 0 GENERATE {: code:n :}
...
: RUN-ACT ( ptr u8 n ptr u8 n -- )
   ...
   root rootu ROW-LEN count OPEN-GENERATE ;
