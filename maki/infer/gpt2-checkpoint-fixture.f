...
: MATCHING-CONFIG ( -- MDLCFG:mcfg )
   ...
   MODEL-ELEMENT-TYPE CONTEXT-SIZE VOCAB-SIZE LAYER-COUNT EMBED-SIZE HEAD-COUNT true 4 4 BUILD ;
...
: OTHER-IDENTITY-CONFIG ( -- MDLCFG:mcfg )
   NORM-EPSILON true GPT2
   MODEL-ELEMENT-TYPE CONTEXT-SIZE VOCAB-SIZE LAYER-COUNT EMBED-SIZE HEAD-COUNT false 4 4 BUILD ;

\ one more layer: the parsed tensor index count no longer matches
: EXTRA-LAYER-CONFIG ( -- MDLCFG:mcfg )
   NORM-EPSILON true GPT2
   MODEL-ELEMENT-TYPE CONTEXT-SIZE VOCAB-SIZE LAYER-COUNT 1 + EMBED-SIZE HEAD-COUNT true 4 4 BUILD ;

\ same parsed tensor index count, every embedding-shaped tensor twice as wide
: WIDE-EMBEDDING-CONFIG ( -- MDLCFG:mcfg )
   NORM-EPSILON true GPT2
   MODEL-ELEMENT-TYPE CONTEXT-SIZE VOCAB-SIZE LAYER-COUNT EMBED-SIZE 2 * HEAD-COUNT true 4 4 BUILD ;

\ the real 124M geometry for the presence-gated tests
: REAL-MODEL-CONFIG ( -- MDLCFG:mcfg )
   NORM-EPSILON true GPT2
   MODEL-ELEMENT-TYPE 1024 50257 12 768 12 true 50256 50256 BUILD ;
