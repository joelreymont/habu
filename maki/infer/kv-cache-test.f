...
: KVT-LANE ( n -- ) {: ptok:n :}
   ...
   KVT-CACHE KVT-LANE-TOKENS ALLOC-SEQ 0 KVT-H! 0 KVT-H@ KVT-LANE-FILL
...
: KVT-EVERY-BOUNDARY ( -- )
   ...
   65 0 ?do
      ...
      KVT-CACHE 65 PAGES-FOR KVT-CACHE i 1+ PAGES-FOR - T=
      KVT-CACHE CHECK
   loop
   KVT-CACHE 0 KVT-H@ SEQ-PAGES 5 T=
   KVT-CACHE 0 KVT-H@ SEQ-LEN 65 T= ;
...
s" every reservation boundary" T-LABEL ' KVT-EVERY-BOUNDARY 0 TTHROWS
...
\ Admission has one public spelling and always requires the declared maximum.
s" KV:ALLOC-SEQ/MAX" 0 search-wl 0= TTRUE
