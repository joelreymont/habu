...
: MOVE-RESHAPE ( ptr r n n ptr r n n -- ) {: s:ptr sr:n sc:n d:ptr dr:n dc:n :}
   sr sc *  dr dc *  <> if E-MV-EXTENT throw then
   s d  sr sc *  MV-COPY ;

\ transpose: dst[k,i] = src[i,k]; dst must be sc x sr (dr=sc, dc=sr).
: MOVE-TRANSPOSE ( ptr r n n ptr r n n -- ) {: s:ptr sr:n sc:n d:ptr dr:n dc:n :}
...
: MOVE-SLICE ( ptr r n n n n ptr r -- ) {: s:ptr sr:n sc:n r0:n r1:n d:ptr :}
...
: MOVE-CONCAT ( ptr r n n ptr r n n ptr r -- ) {: a:ptr ar:n ac:n b:ptr br:n bc:n d:ptr :}
   ac bc <> if E-MV-EXTENT throw then
   ar 0 ?do  a i     d i       ac MV-ROW  loop
   br 0 ?do  b i     d ar i +  ac MV-ROW  loop ;

\ gather: dst row k = src row idx[k]; idx is `in` float cells; dst (in x sc).
: MOVE-GATHER ( ptr r n n ptr r n ptr r -- ) {: s:ptr sr:n sc:n ix:ptr in:n d:ptr :}
   in 0 ?do
      ix i T-GET 0.5 f+ f>s  dup 0 < over sr >= or if E-MV-INDEX throw then
