...
: CU-BEGIN ( -- )
   ...
   EVND-DEV 0 >IDX CUDA:CUDEVICEGET CUDA:RC0
   EVND-CTX EVND-DEV @ >CUDA-DEV CUDA:CUDEVICEPRIMARYCTXRETAIN CUDA:RC0
   EVND-CTX @ >CUDA-CTX CUDA:CUCTXSETCURRENT CUDA:RC0 ;

: CU-LOAD ( ptr u8 n -- ) {: na:ptr nu:n :}    \ load CUBIN$, resolve function <name> into EVND-FUNC
   MAKI-GRADE:CUBIN$ EVND-PATH >CSTR
   EVND-MOD EVND-PATH CUDA:CUMODULELOAD CUDA:RC0
   na nu EVND-KN >CSTR
   EVND-FUNC EVND-MOD @ >CUDA-MOD EVND-KN CUDA:CUMODULEGETFUNCTION CUDA:RC0 ;

: CU-END ( -- )
   EVND-MOD @ >CUDA-MOD CUDA:CUMODULEUNLOAD CUDA:RC0
   EVND-DEV @ >CUDA-DEV CUDA:CUDEVICEPRIMARYCTXRELEASE CUDA:RC0 ;

\ ---- sumnorm device run: SOFTMAX_ROWS, grid = R blocks, block = 256 ----------
variable SN-DIN variable SN-DOUT variable SN-KV

: RUN-SUMNORM ( -- )                            \ launch the built cubin; readback -> SN-OUT
   CU-BEGIN
   s" SOFTMAX_ROWS" CU-LOAD
   SN-DIN SN-BYTES >LEN CUDA:CUMEMALLOC CUDA:RC0
   SN-DOUT SN-BYTES >LEN CUDA:CUMEMALLOC CUDA:RC0
   SN-DIN @ >CUDA-DEVPTR SN-IN SN-BYTES >LEN CUDA:CUMEMCPYHTOD CUDA:RC0
   SN-DOUT @ >CUDA-DEVPTR 0 SN-ELEMS >COUNT CUDA:CUMEMSETD32 CUDA:RC0   \ sentinel 0: an unwritten output stays 0
   SN-COLS SN-KV !
   ...
   SN-OUT SN-DOUT @ >CUDA-DEVPTR SN-BYTES >LEN CUDA:CUMEMCPYDTOH CUDA:RC0
   SN-DIN @ >CUDA-DEVPTR CUDA:CUMEMFREE CUDA:RC0
   SN-DOUT @ >CUDA-DEVPTR CUDA:CUMEMFREE CUDA:RC0
   CU-END ;
...
: RUN-GEMM ( -- )                               \ launch the built cubin; readback -> GEMM-C
   CU-BEGIN
   s" MM" CU-LOAD
   GM-DA GM-A-BYTES >LEN CUDA:CUMEMALLOC CUDA:RC0
   GM-DB GM-B-BYTES >LEN CUDA:CUMEMALLOC CUDA:RC0
   GM-DC GM-C-BYTES >LEN CUDA:CUMEMALLOC CUDA:RC0
   GM-DA @ >CUDA-DEVPTR GEMM-A GM-A-BYTES >LEN CUDA:CUMEMCPYHTOD CUDA:RC0
   GM-DB @ >CUDA-DEVPTR GEMM-B GM-B-BYTES >LEN CUDA:CUMEMCPYHTOD CUDA:RC0
   GM-DC @ >CUDA-DEVPTR 0 GM-C-ELEMS >COUNT CUDA:CUMEMSETD32 CUDA:RC0
   ...
   CUDA:CUCTXSYNCHRONIZE CUDA:RC0
   GEMM-C GM-DC @ >CUDA-DEVPTR GM-C-BYTES >LEN CUDA:CUMEMCPYDTOH CUDA:RC0
   GM-DA @ >CUDA-DEVPTR CUDA:CUMEMFREE CUDA:RC0
   GM-DB @ >CUDA-DEVPTR CUDA:CUMEMFREE CUDA:RC0
   GM-DC @ >CUDA-DEVPTR CUDA:CUMEMFREE CUDA:RC0
   CU-END ;
...
: RUN-ATTN ( -- )                               \ launch the built cubin; readback -> ATTN-O
   CU-BEGIN
   s" ATTN" CU-LOAD
   AT-DQ AT-BYTES >LEN CUDA:CUMEMALLOC CUDA:RC0
   AT-DK AT-BYTES >LEN CUDA:CUMEMALLOC CUDA:RC0
   AT-DV AT-BYTES >LEN CUDA:CUMEMALLOC CUDA:RC0
   AT-DO AT-BYTES >LEN CUDA:CUMEMALLOC CUDA:RC0
   AT-DQ @ >CUDA-DEVPTR ATTN-Q AT-BYTES >LEN CUDA:CUMEMCPYHTOD CUDA:RC0
   AT-DK @ >CUDA-DEVPTR ATTN-K AT-BYTES >LEN CUDA:CUMEMCPYHTOD CUDA:RC0
   AT-DV @ >CUDA-DEVPTR ATTN-V AT-BYTES >LEN CUDA:CUMEMCPYHTOD CUDA:RC0
   AT-DO @ >CUDA-DEVPTR 0 AT-ELEMS >COUNT CUDA:CUMEMSETD32 CUDA:RC0
   ...
   CUDA:CUCTXSYNCHRONIZE CUDA:RC0
   ATTN-O AT-DO @ >CUDA-DEVPTR AT-BYTES >LEN CUDA:CUMEMCPYDTOH CUDA:RC0
   AT-DQ @ >CUDA-DEVPTR CUDA:CUMEMFREE CUDA:RC0
   AT-DK @ >CUDA-DEVPTR CUDA:CUMEMFREE CUDA:RC0
   AT-DV @ >CUDA-DEVPTR CUDA:CUMEMFREE CUDA:RC0
   AT-DO @ >CUDA-DEVPTR CUDA:CUMEMFREE CUDA:RC0
   CU-END ;
