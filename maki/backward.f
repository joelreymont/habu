...
: REF-DT ( MIR:operand-ref -- CAD-KIND:dtype ) {: r:MIR:operand-ref :}
   r MIR-REF-INPUT? if r MIR-REF-SLOT MIR-SLOT-DT@ else r MIR-REF-NODE MIR-DT@ then ;
: REF-LAY ( MIR:operand-ref -- CAD-KIND:layout ) {: r:MIR:operand-ref :}
...
: BW-OP2 ( MIR:operand-ref MIR:operand-ref n MIR:operand-ref -- MIR:operand-ref )
   {: a:MIR:operand-ref b:MIR:operand-ref op:n dref:MIR:operand-ref :}
...
: BW-OP3 ( MIR:operand-ref MIR:operand-ref MIR:operand-ref n MIR:operand-ref -- MIR:operand-ref )
   {: a:MIR:operand-ref b:MIR:operand-ref c:MIR:operand-ref op:n dref:MIR:operand-ref :}
...
: BW-MM ( MIR:operand-ref MIR:operand-ref -- MIR:operand-ref )
   ...
   a REF-ROWS b REF-COLS a REF-DT LAY-ROW 0 1 MIR-OP+ MIR-NODE-REF ;
...
: BW-RS ( MIR:operand-ref CAD-KIND:rows CAD-KIND:cols CAD-KIND:dtype CAD-KIND:layout -- MIR:operand-ref )
   {: ct:MIR:operand-ref tr:CAD-KIND:rows tc:CAD-KIND:cols
      dt:CAD-KIND:dtype lay:CAD-KIND:layout :}
   MV-RESHAPE lay MV-RESHAPE-VERDICT tr ROWS-RAW tc COLS-RAW MV-PACK {: attr:n :}
   OP-RESHAPE MIR-OP-BEGIN  ct MIR-IN+
   tr tc dt lay attr 1 MIR-OP+ MIR-NODE-REF ;
...
: BW-STEP-SOFTMAX ( CAD-KIND:node-id MIR:operand-ref -- )
   {: fn:CAD-KIND:node-id ct:MIR:operand-ref :}
   fn 0 MIR-INPUT-IDX MIR-IN@ {: x:MIR:operand-ref :}
   ct fn MIR-NODE-REF OP-SOFTMAX-ROW-BWD x BW-OP2 x BW-ACCUM ;
...
: BW-STEP-SCALE ( CAD-KIND:node-id MIR:operand-ref -- )
   ...
   s REF-ROWS x REF-ROWS ROWS-EQUAL?
   s REF-COLS x REF-COLS COLS-EQUAL? and if
...
: BW-FIRST-BAD ( -- n )
   ...
   n 0 ?do
      i MIR-NODE-ID MIR-OP@ dup BW-OK-OP? 0= if unloop exit then drop
   loop -1 ;
