...
: TT-ADD-F  ( ptr r ptr r ptr r n -- ) {: xb:ptr yb:ptr zb:ptr len:n :}
   len 0 ?do  xb i T-GET  yb i T-GET  ADD-F  zb i T-SET  loop ;
: TT-MUL-F  ( ptr r ptr r ptr r n -- ) {: xb:ptr yb:ptr zb:ptr len:n :}
   len 0 ?do  xb i T-GET  yb i T-GET  MUL-F  zb i T-SET  loop ;
: TT-SUB-F  ( ptr r ptr r ptr r n -- ) {: xb:ptr yb:ptr zb:ptr len:n :}
   len 0 ?do  xb i T-GET  yb i T-GET  SUB-F  zb i T-SET  loop ;
: TT-RELU-F ( ptr r ptr r n -- )       {: xb:ptr zb:ptr len:n :}
   len 0 ?do  xb i T-GET  RELU-F   zb i T-SET  loop ;
: TT-SQUARE-F ( ptr r ptr r n -- )     {: xb:ptr zb:ptr len:n :}
   len 0 ?do  xb i T-GET  SQUARE-F  zb i T-SET  loop ;
: TT-SUM-F  ( ptr r n -- r )  T-SUM ;       \ reduce the whole tensor to a scalar

\ --- backward (map the scalar VJP; write the input-cotangent tensor(s)) ---
\ ADD: dX=dZ, dY=dZ
: TT-ADD-BWD ( ptr r ptr r ptr r n -- ) {: dzb:ptr dxb:ptr dyb:ptr len:n :}
   len 0 ?do
      dzb i T-GET  ADD-BWD              \ ( dx dy )
      dyb i T-SET  dxb i T-SET
   loop ;
\ MUL: dX=dZ*Y, dY=dZ*X  (saved primals X, Y)
: TT-MUL-BWD ( ptr r ptr r ptr r ptr r ptr r n -- )
