...
\ NB: the variant is `layout-conflict`, not `layout`: dot habu-cad-adt-swap's
\ descriptor trio makes `layout` a real type family (tensor-value.f), and a variant
\ tail may not share a name with a type family (checker rejects the collision).
ENUM reason
  multi-use
  matmul
  layout-conflict
