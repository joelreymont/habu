...
: SRT-BUILD ( n n -- MIR:input-slot ) {: rows:n cols:n :}
   MIR-RESET
   rows cols SHAPE DT-F32 LAY-ROW MIR-INPUT+ {: s:MIR:input-slot :}
   OP-GELU MIR-OP-BEGIN
   s MIR-IN-REF MIR-IN+
   rows cols SHAPE DT-F32 LAY-ROW 0 1 MIR-OP+ {: gelu:CAD-KIND:node-id :}
   OP-RELU MIR-OP-BEGIN
   gelu MIR-NODE-REF MIR-IN+
...
2 100 SRT-BUILD  AL-16 MIR-SLOT-AL!  FP-BUILD
0 TARGET:SM87 SK-KEY$ 7 SK-PUT-DURABLE
0 TARGET:SM87 SK-KEY$ SK-GET drop 7 T=            \ hot table
SK-TAB-COUNT 1 T=
0 TARGET:SM87 SK-KEY$ SCHED-GET drop 7 T=         \ durable file

\ ---- reload durability into a cleared table ---------------------------------
SK-TAB-RESET
SK-TAB-COUNT 0 T=
0 TARGET:SM87 SK-KEY$ SK-GET nip TFALSE           \ table empty after reset
STORE-REPLAY-LOAD
0 TARGET:SM87 SK-KEY$ SK-GET drop 7 T=            \ file -> memory
SK-TAB-COUNT 1 T=

\ ---- latest row per key wins on reload --------------------------------------
0 TARGET:SM87 SK-KEY$ 9 SK-PUT-DURABLE            \ memory update-in-place + newer file row
SK-TAB-RESET
STORE-REPLAY-LOAD
