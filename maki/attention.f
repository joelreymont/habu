...
: MM-NT-EL ( ptr r ptr r n n n -- r ) {: ab:ptr bb:ptr ix:n jx:n inner:n :}
   0.0  inner 0 ?do
      ab ix inner * i + T-GET   bb jx inner * i + T-GET   f*  f+
   loop ;
: MM-NT ( ptr r ptr r ptr r n n n -- ) {: ab:ptr bb:ptr cb:ptr rows:n rows2:n inner:n :}
   rows 0 ?do
...
: MM-TN-EL ( ptr r ptr r n n n n n -- r ) {: ab:ptr bb:ptr p:n q:n rows:n ca:n cbc:n :}
   0.0  rows 0 ?do
      ab i ca * p + T-GET   bb i cbc * q + T-GET   f* f+
   loop ;
: MM-TN ( ptr r ptr r ptr r n n n -- ) {: ab:ptr bb:ptr cb:ptr rows:n ca:n cbc:n :}
   ca 0 ?do
...
: ATTN-SMBWD-ROWS ( ptr r ptr r ptr r n -- ) {: dab:ptr ab:ptr dsb:ptr lc:n :}
   lc 0 ?do
...
: ATTN-BWD ( ptr r ptr r ptr r ptr r ptr r ptr r ptr r ptr r ptr r ptr r n n -- )
