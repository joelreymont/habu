...
: IMP-BIND ( n MIR:operand-ref -- ) {: ni:n ref:MIR:operand-ref :} \ bind name to MIR ref
   IMP-SET ni cells + @ 0<> if E-ONNX-NAME throw then
   ref ni IMP-REF!
   1 IMP-SET ni cells + ! ;

: IMP-RESOLVE ( n -- MIR:operand-ref ) {: ni:n :} \ name -> MIR ref; unbound fails closed
...
: IMP-INPUT-1 ( n -- ) {: gi:n :}
   ...
   ni IMP-INIT-FIND if {: iz:n :}              \ input listed as initializer (legacy export):
      ...
      exit then
   drop
   gi OGIN-ROWS@ gi OGIN-COLS@ MAKI:SHAPE MAKI:DT-F32 MAKI:LAY-ROW MAKI:MIR-INPUT+
      {: s:MIR:input-slot :}
...
: IMP-INIT-1 ( n -- ) {: iz:n :}
   ...
   rows cols MAKI:SHAPE MAKI:DT-F32 MAKI:LAY-ROW MAKI:MIR-INPUT+ {: s:MIR:input-slot :}
...
: IMP-COMMIT ( n CAD-KIND:rows CAD-KIND:cols -- )
   {: j:n rows:CAD-KIND:rows cols:CAD-KIND:cols :}
   rows cols MAKI:DT-F32 MAKI:LAY-ROW 0 1 MAKI:MIR-OP+ IMP-NODE+
      {: k:CAD-KIND:node-id :}
...
: IMP-COMMIT-MOVE ( n CAD-KIND:rows CAD-KIND:cols n -- )
   {: j:n rows:CAD-KIND:rows cols:CAD-KIND:cols attr:n :}
   rows cols MAKI:DT-F32 MAKI:LAY-ROW attr attr IMP-MOVE-MAT MAKI:MIR-OP+ IMP-NODE+
      {: k:CAD-KIND:node-id :}
   j OND-OUT@ k MAKI:MIR-NODE-REF IMP-BIND ;
...
: MK-MOVE ( CAD-KIND:rows CAD-KIND:cols n -- MIR:operand-ref )
   {: rows:CAD-KIND:rows cols:CAD-KIND:cols attr:n :}
...
: SYN-CONST ( r -- MIR:operand-ref ) {: v:r :}
   ...
   1 1 MAKI:SHAPE MAKI:DT-F32 MAKI:LAY-ROW MAKI:MIR-INPUT+ {: s:MIR:input-slot :}
...
: IMP-EW-REFS ( n -- MIR:operand-ref MIR:operand-ref ) {: j:n :}
...
: IMP-EW-BUILD ( n n MIR:operand-ref MIR:operand-ref -- )
   {: j:n op:n r0:MIR:operand-ref r1:MIR:operand-ref :}
...
: GEMM-T ( MIR:operand-ref -- MIR:operand-ref ) {: r:MIR:operand-ref :}
...
: IMP-RESHAPE ( n -- ) {: j:n :}               \ shape is an INT64 initializer; a runtime shape rejects
   ...
   j 0 OND-IN@ IMP-RESOLVE {: r0:MIR:operand-ref :}
   r0 IMP-REF-ROWS r0 IMP-REF-COLS MAKI:SHAPE-ELEMS
   tr tc MAKI:SHAPE MAKI:SHAPE-ELEMS MAKI:DIM-EQUAL? 0= if E-ONNX-SHAPE throw then
...
: IMP-CONCAT ( n -- ) {: j:n :}                \ 2-input row concat (axis 0): RxC + SxC -> (R+S)xC
   ...
   j ra IMP-REF-ROWS rb IMP-REF-ROWS MAKI:ROWS+
   ra IMP-REF-COLS attr IMP-COMMIT-MOVE ;
