...
: ENC-I64 ( n -- ) {: v:n :}                   \ append 8 raw LE bytes of an int64 (constant payload)
   v $FF and ENC-B          v 8  rshift $FF and ENC-B
   v 16 rshift $FF and ENC-B  v 24 rshift $FF and ENC-B
   v 32 rshift $FF and ENC-B  v 40 rshift $FF and ENC-B
   v 48 rshift $FF and ENC-B  v 56 rshift $FF and ENC-B ;
...
: INIT-BR ( -- )                               \ bias row 1x2 = [10,20]
   5 ENC-SUB
      1 1 ENC-INT  2 1 ENC-INT  1 2 ENC-INT  s" br" 8 ENC-STR
      9 ENC-SUB  10.0 ENC-F32  20.0 ENC-F32  ;ENC-SUB
   ;ENC-SUB ;

: INIT-SC ( -- )                               \ scale scalar 1x1 = [3]
   5 ENC-SUB
      1 1 ENC-INT  1 1 ENC-INT  1 2 ENC-INT  s" sc" 8 ENC-STR
      9 ENC-SUB  3.0 ENC-F32  ;ENC-SUB
   ;ENC-SUB ;

: INIT-COL ( -- )                              \ column 2x1 = [[5],[6]] (Rx1, an illegal Add class)
   5 ENC-SUB
      2 1 ENC-INT  1 1 ENC-INT  1 2 ENC-INT  s" col" 8 ENC-STR
      9 ENC-SUB  5.0 ENC-F32  6.0 ENC-F32  ;ENC-SUB
   ;ENC-SUB ;

: INIT-R2 ( -- )                               \ 2x2 = ones (ragged vs a 3x2 operand)
   5 ENC-SUB
      2 1 ENC-INT  2 1 ENC-INT  1 2 ENC-INT  s" r2" 8 ENC-STR
      9 ENC-SUB
         1.0 ENC-F32  1.0 ENC-F32  1.0 ENC-F32  1.0 ENC-F32
      ;ENC-SUB
   ;ENC-SUB ;
...
: INIT-SH ( -- )                               \ int64 shape constant "sh" = [1,4] (rank-1 dims=[2])
   5 ENC-SUB
      2 1 ENC-INT  7 2 ENC-INT  s" sh" 8 ENC-STR   \ dims [2], data_type INT64
      9 ENC-SUB  1 ENC-I64  4 ENC-I64  ;ENC-SUB          \ raw_data [1,4]
   ;ENC-SUB ;

: RESHAPE-NODE ( -- )                          \ Reshape x sh -> y
   1 ENC-SUB
      s" x" 1 ENC-STR  s" sh" 1 ENC-STR
      s" y" 2 ENC-STR  s" Reshape" 4 ENC-STR
   ;ENC-SUB ;
...
: TRANSPOSE-NODE ( -- )                        \ Transpose x -> y, perm [1,0] (unpacked ints)
   1 ENC-SUB
      s" x" 1 ENC-STR  s" y" 2 ENC-STR  s" Transpose" 4 ENC-STR
      5 ENC-SUB  s" perm" 1 ENC-STR
         1 8 ENC-INT  0 8 ENC-INT
      ;ENC-SUB
   ;ENC-SUB ;
...
: INIT-WT ( -- )                               \ wt 2x3 = [[1,0,1],[0,1,0]] (a transB weight)
   5 ENC-SUB
      2 1 ENC-INT  3 1 ENC-INT  1 2 ENC-INT  s" wt" 8 ENC-STR
      9 ENC-SUB
         1.0 ENC-F32  0.0 ENC-F32  1.0 ENC-F32
         0.0 ENC-F32  1.0 ENC-F32  0.0 ENC-F32
      ;ENC-SUB
   ;ENC-SUB ;

: GEMM-TB-NODE ( -- )                          \ Gemm x wt -> y, transB=1
   1 ENC-SUB
      s" x" 1 ENC-STR  s" wt" 1 ENC-STR  s" y" 2 ENC-STR
      s" Gemm" 4 ENC-STR
...
: GEMM-AB-NODE ( -- )                          \ Gemm x w b -> y, alpha=2 beta=0
   1 ENC-SUB
      s" x" 1 ENC-STR  s" w" 1 ENC-STR  s" b" 1 ENC-STR
      s" y" 2 ENC-STR  s" Gemm" 4 ENC-STR
      5 ENC-SUB  s" alpha" 1 ENC-STR  2.0 2 ENC-F32A  ;ENC-SUB
      5 ENC-SUB  s" beta"  1 ENC-STR  0.0 2 ENC-F32A  ;ENC-SUB
   ;ENC-SUB ;
...
: N-OPEN ( -- )          1 ENC-SUB ;
: N-IN  ( ptr u8 n -- )  1 ENC-STR ;      \ one input name
: N-OUT ( ptr u8 n -- )  2 ENC-STR ;      \ the single output name
: N-OP  ( ptr u8 n -- )  4 ENC-STR ;      \ op_type text
: N-CLOSE ( -- )         ;ENC-SUB ;

\ ---- INT64 constant initializers (raw_data field 9; ENC-I64 8-byte LE) ------------
: INIT-I1 ( n ptr u8 n -- ) {: v:n a:ptr u:n :}  \ int64 rank-1 [v] named a
   5 ENC-SUB
      1 1 ENC-INT  7 2 ENC-INT  a u 8 ENC-STR   \ dims [1], data_type INT64
      9 ENC-SUB  v ENC-I64  ;ENC-SUB
   ;ENC-SUB ;

: INIT-I2 ( n n ptr u8 n -- ) {: v0:n v1:n a:ptr u:n :}   \ int64 rank-1 [v0,v1] named a
   5 ENC-SUB
      2 1 ENC-INT  7 2 ENC-INT  a u 8 ENC-STR   \ dims [2], data_type INT64
      9 ENC-SUB  v0 ENC-I64  v1 ENC-I64  ;ENC-SUB
   ;ENC-SUB ;

\ ---- residual A: int64_data (field 7) INT64 initializer drives a Reshape -----------
: INIT-SH7 ( -- )                              \ int64_data shape "sh7" = [1,4] (packed varints)
   5 ENC-SUB
      2 1 ENC-INT  7 2 ENC-INT  s" sh7" 8 ENC-STR  \ dims [2], data_type INT64
      7 ENC-SUB  1 ENC-VARINT  4 ENC-VARINT  ;ENC-SUB  \ int64_data [1,4]
   ;ENC-SUB ;
...
: INIT-SH7U ( -- )                             \ int64_data shape "sh7u" = [1,4] (UNPACKED varints)
   5 ENC-SUB
      2 1 ENC-INT  7 2 ENC-INT  s" sh7u" 8 ENC-STR  \ dims [2], data_type INT64
      1 7 ENC-INT  4 7 ENC-INT                           \ int64_data as repeated varints
   ;ENC-SUB ;
...
: INIT-SHN7 ( -- )                             \ int64_data shape "shn7" = [-1,2] (negative 10-byte varint)
   5 ENC-SUB
      2 1 ENC-INT  7 2 ENC-INT  s" shn7" 8 ENC-STR  \ dims [2], data_type INT64
      7 ENC-SUB  -1 ENC-VARINT  2 ENC-VARINT  ;ENC-SUB
   ;ENC-SUB ;
...
: INIT-IDXF ( -- )                             \ float 1x2 = [2,0] named idxf (wrong dtype for indices)
   5 ENC-SUB
      1 1 ENC-INT  2 1 ENC-INT  1 2 ENC-INT  s" idxf" 8 ENC-STR
      9 ENC-SUB  2.0 ENC-F32  0.0 ENC-F32  ;ENC-SUB
   ;ENC-SUB ;
...
: TRY-DYN ( -- )                               \ symbolic dim_param -> fail closed
   ...
   11 ENC-SUB
      ...
      2 ENC-SUB  1 ENC-SUB
         1 1 ENC-INT
         2 ENC-SUB
            1 ENC-SUB  s" N" 2 ENC-STR  ;ENC-SUB
            2 DIM+
         ;ENC-SUB
      ;ENC-SUB  ;ENC-SUB
   ;ENC-SUB
...
: TRY-GEMMATTR ( -- )                          \ Gemm with an axis attr: outside the Gemm attribute set
   MDL
   1 ENC-SUB
      s" x" 1 ENC-STR  s" w" 1 ENC-STR  s" y" 2 ENC-STR
      s" Gemm" 4 ENC-STR
      5 ENC-SUB  s" axis" 1 ENC-STR  1 3 ENC-INT  ;ENC-SUB
   ;ENC-SUB
...
: TRY-AXIS ( -- )                              \ Softmax axis=0 is not the last axis
   MDL
   1 ENC-SUB
      s" x" 1 ENC-STR  s" y" 2 ENC-STR  s" Softmax" 4 ENC-STR
      5 ENC-SUB  s" axis" 1 ENC-STR  0 3 ENC-INT  ;ENC-SUB
   ;ENC-SUB
   s" x" 11 2 2 VI+  s" y" 12 2 2 VI+  ;MDL IMP! ;
...
: TRY-ADDATTR ( -- )                           \ a recognized attr on an op that allows none
   MDL
   1 ENC-SUB
      s" x" 1 ENC-STR  s" x" 1 ENC-STR  s" y" 2 ENC-STR
      s" Add" 4 ENC-STR
      5 ENC-SUB  s" axis" 1 ENC-STR  1 3 ENC-INT  ;ENC-SUB
   ;ENC-SUB
   s" x" 11 2 2 VI+  s" y" 12 2 2 VI+  ;MDL IMP! ;
...
: TRY-BADATTR ( -- )                           \ an attr name the importer cannot honor
   MDL
   1 ENC-SUB
      s" x" 1 ENC-STR  s" y" 2 ENC-STR  s" Relu" 4 ENC-STR
      5 ENC-SUB  s" pads" 1 ENC-STR  1 3 ENC-INT  ;ENC-SUB
   ;ENC-SUB
   s" x" 11 2 2 VI+  s" y" 12 2 2 VI+  ;MDL IMP! ;
...
: TRY-RANK3 ( -- )                             \ rank-3 input: the IR is 2D
   ...
   11 ENC-SUB
      ...
      2 ENC-SUB  1 ENC-SUB
         1 1 ENC-INT
         2 ENC-SUB  2 DIM+  2 DIM+  2 DIM+  ;ENC-SUB
      ;ENC-SUB  ;ENC-SUB
   ;ENC-SUB
...
: TRY-BADPERM ( -- )                           \ Transpose perm [0,1] is identity, not the 2D transpose
   ...
   1 ENC-SUB
      s" x" 1 ENC-STR  s" y" 2 ENC-STR  s" Transpose" 4 ENC-STR
      5 ENC-SUB  s" perm" 1 ENC-STR  0 8 ENC-INT  1 8 ENC-INT  ;ENC-SUB
   ;ENC-SUB
   s" x" 11 2 2 VI+  s" y" 12 2 2 VI+  ;MDL IMP! ;
...
: TRY-PERM3 ( -- )                             \ a rank-3 perm on the 2D importer
   ...
   1 ENC-SUB
      s" x" 1 ENC-STR  s" y" 2 ENC-STR  s" Transpose" 4 ENC-STR
      5 ENC-SUB  s" perm" 1 ENC-STR
         2 8 ENC-INT  1 8 ENC-INT  0 8 ENC-INT
      ;ENC-SUB
   ;ENC-SUB
   s" x" 11 2 2 VI+  s" y" 12 2 2 VI+  ;MDL IMP! ;
...
: TRY-NODATA ( -- )                            \ initializer without a payload
   MDL
   s" Add" s" x" s" c" s" y" NODE2
   5 ENC-SUB  s" c" INIT-HDR  ;ENC-SUB
   s" x" 11 2 2 VI+  s" y" 12 2 2 VI+  ;MDL IMP! ;
...
: TRY-I64OVER ( -- )                           \ int64_data varint overflows 64 bits: decode error
   ...
   5 ENC-SUB
      1 1 ENC-INT  7 2 ENC-INT  s" bad" 8 ENC-STR   \ dims [1], data_type INT64
      7 ENC-SUB  9 0 ?do $FF ENC-B loop  $7F ENC-B  ;ENC-SUB
   ;ENC-SUB
...
: TRY-I64BOTH ( -- )                           \ INT64 initializer with BOTH int64_data and raw_data
   ...
   5 ENC-SUB
      2 1 ENC-INT  7 2 ENC-INT  s" shb" 8 ENC-STR
      7 ENC-SUB  1 ENC-VARINT  4 ENC-VARINT  ;ENC-SUB
      9 ENC-SUB  1 ENC-I64  4 ENC-I64  ;ENC-SUB
   ;ENC-SUB
   s" x" 11 2 2 VI+  s" y" 12 1 4 VI+  ;MDL IMP! ;
...
: TRY-I64NONE ( -- )                           \ INT64 initializer with NEITHER payload source
   ...
   5 ENC-SUB
      2 1 ENC-INT  7 2 ENC-INT  s" shz" 8 ENC-STR
   ;ENC-SUB
   s" x" 11 2 2 VI+  s" y" 12 1 4 VI+  ;MDL IMP! ;

: TRY-TRUNC ( -- )                             \ fixture A cut one byte short
   MODEL-A  ENC$ 1- ONNX:IMPORT ;

\ ---- IMPORT-FILE round trip -------------------------------------------------------
: FIX-PATH$ ( -- ptr u8 n )  s" tmp/onnx-import-test.onnx" ;
: WRITE-FIX ( -- )  s" tmp" MAKE-DIRS  MODEL-B  FIX-PATH$ ENC$ WRITE-ALL ;
