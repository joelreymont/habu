...
: SL-TARGET-LAYOUT-SYS ( -- )
   HB-TARGET-LINUX? if
      s" src/os/linux/layout.f src/os/linux/sys.f src/os/linux/proc-watch.f src/os/linux/proc-control.f " type
      exit
   then
   HB-TARGET-MACOS? if
      s" src/os/macos/layout.f src/os/macos/sys.f src/os/macos/proc-watch.f src/os/macos/proc-control.f " type
      exit
   then
   SL-TARGET-UNKNOWN ;
...
: SL-PREFIX ( -- )
   s" src/os/env-base.f src/core/roles.f src/core/bytes.f " type
   ...
   s" src/core/type-family-sha.f src/core/combinators.f src/core/engine-error.f " type
