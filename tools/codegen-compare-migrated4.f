...
: C-ADD1 ( -- )
   s" : C-ADD1-N ( n -- n ) 1 + ;" 1 1 NMIGRATE:DEFINE ;

: C-MUL2 ( -- )
   s" : C-MUL2-N ( n -- n ) 2 * ;" 1 1 NMIGRATE:DEFINE ;

: C-AND7 ( -- )
   s" : C-AND7-N ( n -- n ) 7 and ;" 1 1 NMIGRATE:DEFINE ;

: C-XOR5 ( -- )
   s" : C-XOR5-N ( n -- n ) 5 xor ;" 1 1 NMIGRATE:DEFINE ;
...
: C-MAD ( -- )
   s" : C-MAD-N ( n -- n ) 3 * 5 + ;" 1 1 NMIGRATE:DEFINE ;
