...
\
\ The package is the namespace, so the seven words of the interface drop the
\ prefix they used to carry. The private helpers keep theirs deliberately:
\ this tool bundles the stdlib and so requires none of it, and it wraps or
\ re-implements what it needs - BL-DIE calls `die`, BL-WRITE calls `write`,
\ BL-BYTE-COPY and BL-STR= stand in for lib/string.f. Naming a helper after
\ the word it wraps would leave its body calling itself, because the
\ dictionary search returns the newest definition.
...
\ Mark a bundled module's path as already provided, so a `require lib/<mod>.f`
...
: BL-EMIT-PROVIDED-ALL ( -- )
   0 begin dup BL-NMOD @ < while
      dup BL-MOD$ BL-MODULE-PATH BL-EMIT-PROVIDED
      1+
   repeat drop ;
