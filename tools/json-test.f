...
: TEST-JSONL-ROW-OUTCOMES  \ ( -- )
   ...
   JSONL-NEXT-ROW ASSERT
   0 ASSERT=
   JSONL-ROW-JSON ASSERT=
   dup s" a" JSON-GET JSON-NUMBER$ s" 1" ASSERT$
   drop
   JSONL-LINE# 2 ASSERT=
   JSONL-NEXT-ROW ASSERT
   E-JSON-SYNTAX ASSERT=
...
: TEST-JSONL-FINAL-PARTIAL  \ ( -- )
   ...
   -1 ASSERT=
   JSONL-LINE# 2 ASSERT=
   JSONL-LINE$ s" not-json" ASSERT$
   JSONL-NEXT-ROW 0= ASSERT
   0 ASSERT=
   JSONL-ROW-EOF ASSERT=
   -1 ASSERT=
   JSONL-LINE# 2 ASSERT= ;
