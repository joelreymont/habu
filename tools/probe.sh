...
if [ "$PROBE_INSTALLED" = 1 ]; then
  ...
  if [ ! -x "$BIN" ] || [ "$(cat $T/hb-probe-repl.sum 2>/dev/null)" != "$SUM" ]; then
    cat > $T/hb-probe.fs <<FS
require $(pwd)/test/nf.fs
s" $BIN" FORTH-REPL-EXE
FS
    rm -f "$BIN"
    $G $T/hb-probe.fs -e bye > $T/hb-probe.log 2>&1 || true
    [ -x "$BIN" ] || { echo "BUILD FAILED:"; grep -aE 'error' $T/hb-probe.log | head -3; exit 1; }
    echo "$SUM" > $T/hb-probe-repl.sum
  fi
fi
rc=0
out=$({ for f in $PROBE_FILES; do cat "$f"; printf '\n'; done
        printf '%s\n' "$1"; } | timeout 10 "$BIN" 2>$T/hb-probe.err) || rc=$?
if [ $# -ge 2 ]; then
  [ "$out" = "$2" ] && { echo "OK [$out]"; exit 0; }
  echo "MISMATCH rc=$rc got=[$out] want=[$2]"; head -2 $T/hb-probe.err; exit 1
fi
echo "rc=$rc out=[$out]"; head -2 $T/hb-probe.err
