...
: T-GET ( -- )
   ...
   s" : T-GET-N ( ptr r n -- r ) T-AT-N @ ;" 2 1 REGS NMIGRATE:DEFINE-CALLING ;

: T-SET ( -- )
   s" T-AT-N" s" CODEGEN-CORPUS3:T-AT-N" CODEGEN-COMPARE:CODE-ENTRY 2 1 NMIGRATE:CALLEE
   s" : T-SET-N ( r ptr r n -- ) T-AT-N ! ;" 3 0 REGS NMIGRATE:DEFINE-CALLING ;
...
: T-SUM ( -- )
   ...
   s" : T-SUM-N ( ptr r n -- r ) {: base len :} 0.0  len 0 ?do  base i T-GET-N f+  loop ;"
...
: T-SGD ( -- )
   ...
   s" : T-SGD!-N ( r ptr r ptr r n -- ) {: lr wbase gbase len :} len 0 ?do wbase i T-GET-N lr gbase i T-GET-N f* f- wbase i T-SET-N loop ;"
   4 0 LOOP-REGS NMIGRATE:DEFINE-CALLING ;

\ maki/array.f:32 through the corpus: the two-pointer accumulation.
: T-DIST2 ( -- )
   s" T-GET-N" s" CODEGEN-CORPUS3:T-GET-N" CODEGEN-COMPARE:CODE-ENTRY 2 1 NMIGRATE:CALLEE
   s" : T-DIST2-N ( ptr r ptr r n -- r ) {: abase:ptr bbase:ptr len:n :} 0.0 len 0 ?do abase i T-GET-N bbase i T-GET-N f- dup f* f+ loop ;"
   3 1 LOOP-REGS NMIGRATE:DEFINE-CALLING ;

\ maki/array.f:39 through the corpus: the one-pointer square accumulation.
: T-NORM2 ( -- )
   s" T-GET-N" s" CODEGEN-CORPUS3:T-GET-N" CODEGEN-COMPARE:CODE-ENTRY 2 1 NMIGRATE:CALLEE
   s" : T-NORM2-N ( ptr r n -- r ) {: bbase:ptr len:n :} 0.0 len 0 ?do bbase i T-GET-N dup f* f+ loop ;"
   2 1 LOOP-REGS NMIGRATE:DEFINE-CALLING ;
...
: T-REL-L2 ( -- )
   ...
   s" : T-REL-L2-N ( ptr r ptr r n -- r ) {: abase:ptr bbase:ptr len:n :} abase bbase len T-DIST2-N fsqrt bbase len T-NORM2-N fsqrt f/ ;"
   3 1 LOOP-REGS NMIGRATE:DEFINE-CALLING ;
