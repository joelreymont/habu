...
: SC-MANUAL-TABLE ( [ ptr u8 n -- ] -- ) {: q :}
   ...
   \ snapshot relocation parity gate: the fifth gate of the same shape and the
   \ same Rocq dependency - it compiles formal/Common/Reloc.v and then a
   \ generated obligations file, so it fails outright where `rocq` is not on
   \ PATH. Runs spawned in the standalone stdlib gate; the resident tier stays
