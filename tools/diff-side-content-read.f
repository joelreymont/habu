...
: REQUIRE-VALID ( -- )
   VIEW-VALID @ 0= if E-SIDE-SYNTAX throw then ;

: RANGE ( n n -- ptr u8 ) {: off:n need:n :}
   off 0 < need 0 < or if E-SIDE-CAPACITY throw then
   off need CHECK-ADD VIEW-U @ > if E-SIDE-CAPACITY throw then
   VIEW-A@ off + ;
...
: U64@ ( n -- n )
   8 RANGE FS-U64@ dup 0 < if E-SIDE-CAPACITY throw then ;
...
: SIDE-CANONICAL ( n -- ) {: off:n :}
   ...
   off 8 + U64@ {: size:n :}
   off 16 + U64@ {: pathu:n :}
   present BOOL-BYTE? 0= binary BOOL-BYTE? 0= or if E-SIDE-SYNTAX throw then
   present 0= if
      kind KIND-ABSENT? 0= pathu 0<> or size 0<> or binary 0<> or if E-SIDE-SYNTAX throw then
   else
      kind KIND-ABSENT? if E-SIDE-SYNTAX throw then
      off SIDE-PATH$ PATH-SAFE? 0= if E-SIDE-SYNTAX throw then
   then
   kind KIND-FILE? 0= if binary 0<> if E-SIDE-SYNTAX throw then then
   kind KIND-GITLINK? kind KIND-ABSENT? or if
      size 0<> binary 0<> or if E-SIDE-SYNTAX throw then
...
: SIDE-CHECK ( n n -- ) {: off:n sideu:n :}
   sideu SIDE-HEAD-U < if E-SIDE-SYNTAX throw then
   off sideu RANGE drop
   off s" S" MAGIC? 0= if E-SIDE-SYNTAX throw then
   off 4 + SIDE-RESERVED-U ZERO-RANGE? 0= if E-SIDE-SYNTAX throw then
   off 24 + U64@ 0<> if E-SIDE-SYNTAX throw then
   off SIDE-TOTAL sideu <> if E-SIDE-SYNTAX throw then
   off SIDE-CANONICAL ;

: ROW-CHECK ( n -- ) {: ordinal:n :}
   CURSOR @ {: off:n :}
   off ROW-HEAD-U RANGE drop
   off BYTE@ ROW-C <> if E-SIDE-SYNTAX throw then
   off 1+ HEAD-RESERVED-U ZERO-RANGE? 0= if E-SIDE-SYNTAX throw then
   off 8 + U64@ ordinal <> if E-SIDE-SYNTAX throw then
   off 16 + U64@ {: rowu:n :}
   off 24 + U64@ {: oldu:n :}
   off 32 + U64@ {: newu:n :}
   rowu ROW-HEAD-U < if E-SIDE-SYNTAX throw then
   ROW-HEAD-U oldu CHECK-ADD newu CHECK-ADD rowu <> if E-SIDE-SYNTAX throw then
   off rowu CHECK-ADD TRAILER-OFF @ > if E-SIDE-CAPACITY throw then
   off ROW-HEAD-U + oldu SIDE-CHECK
   off ROW-HEAD-U + oldu + newu SIDE-CHECK
   off rowu + CURSOR ! ;

: HEADER-CHECK ( -- )
   0 s" HABUSIDE" MAGIC? 0= if E-SIDE-SYNTAX throw then
   8 BYTE@ VERSION <> if E-SIDE-SYNTAX throw then
   9 HEAD-RESERVED-U ZERO-RANGE? 0= if E-SIDE-SYNTAX throw then
   16 U64@ VIEW-N !
   24 U64@ PAYLOAD-U !
   104 8 ZERO-RANGE? 0= if E-SIDE-SYNTAX throw then
   HEADER-U PAYLOAD-U @ CHECK-ADD TRAILER-OFF !
   TRAILER-OFF @ TRAILER-U CHECK-ADD VIEW-U @ <> if E-SIDE-CAPACITY throw then ;

: TRAILER-CHECK ( -- )
   TRAILER-OFF @ {: off:n :}
   off s" EDISUBAH" MAGIC? 0= if E-SIDE-SYNTAX throw then
   off 8 + BYTE@ VERSION <> if E-SIDE-SYNTAX throw then
   off 9 + HEAD-RESERVED-U ZERO-RANGE? 0= if E-SIDE-SYNTAX throw then
   off 16 + U64@ VIEW-N @ <> if E-SIDE-SYNTAX throw then
   off 24 + U64@ VIEW-U @ <> if E-SIDE-SYNTAX throw then
...
: ROWS-CHECK ( -- )
   ...
   0 begin dup VIEW-N @ < while
      ...
      1+
   repeat drop
   CURSOR @ TRAILER-OFF @ <> if E-SIDE-SYNTAX throw then ;
...
: ENSURE-VIEW ( n -- ) {: need:n :}
   ...
   repeat
   GROW-CAP @ MEM-ALLOC-BYTES drop VIEW-A !
   GROW-CAP @ VIEW-CAP ! ;
...
: SELECTED ( -- )
   REQUIRE-VALID
   SELECT-I @ 0 < if E-SIDE-SYNTAX throw then ;
...
: VALIDATE ( ptr u8 n -- n ) {: a:ptr u:n :}
   false VIEW-VALID !
   u HEADER-U TRAILER-U + < if E-SIDE-CAPACITY throw then
   u ENSURE-VIEW
