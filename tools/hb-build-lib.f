...
: HBB-SIZE-LINE ( -- )
   ...
   s" hb-build data: " type
   IMAGE-SIZE:DATA-SPAN-BYTES FMT:.U
   s"  bytes used / " type
   IMAGE-SIZE:DATA-CAPACITY-BYTES FMT:.U
   s"  DATA-SIZE (" type
   IMAGE-SIZE:DATA-CAPACITY-BYTES IMAGE-SIZE:DATA-SPAN-BYTES - FMT:.U
   s"  free)" type cr ;
...
: HBB-CAPTURE-REPORT ( -- )
   ...
   IMAGE-SIZE:DATA-SPAN-BYTES IMAGE-SIZE:DATA-CAPACITY-BYTES
   HB-BUILD:CAPTURE-DATA ;
