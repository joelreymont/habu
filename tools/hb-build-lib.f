...
: HBB-MAKER-CACHE$ ( -- ptr u8 n )
   HBB-MAKER-KEY!
   HBB-MAKER-NAME!
   HBB-MAKER-NAME$ HBB-MAKER-PATH BUILD-CACHE:CHILD-INTO HBB-MAKER-U !
...
: HBB-ARTIFACT-PATHS ( -- )
   HBB-ARTIFACT-KEY!
   HBB-ARTIFACT-NAME!
   HBB-ARTIFACT-NAME$ HBB-ARTIFACT-PATH BUILD-CACHE:CHILD-INTO HBB-ARTIFACT-U !
   HBB-ARTIFACT-NAME$ s" .tmp" HBB-ARTIFACT-TMP-PATH BUILD-CACHE:CHILD-SUFFIX-INTO HBB-ARTIFACT-TMP-U !
   HBB-ARTIFACT-NAME$ s" .lock" HBB-ARTIFACT-LOCK-PATH BUILD-CACHE:CHILD-SUFFIX-INTO HBB-ARTIFACT-LOCK-U ! ;
...
: PATH-ERROR ( -- )
   HBB-JSON @ if PATH-ERROR$ else PATH-ERROR-TEXT$ then
   HBB-WERR
   HBB-WERR-LF ;
...
: BUILD-REST ( -- )
   HBB-RESTORE-ARTIFACT? if FINISH exit then
   HBB-OBJECT-HIT? if HBB-INSTALL-ARTIFACT FINISH exit then
   HBB-PREPARE-PROGRAM-SOURCE
   HBB-BUILD-MAKER
   HBB-RUN-MAKER
   HBB-STORE-OBJECT
...
: PREPARE-CACHE-CLI ( -- )
   [: HBB-PREPARE-ARTIFACT-CACHE ;] catch {: rc:n :}
   rc 0= if exit then
   rc E-BUILD-PATH = if
      HBB-CLEANUP
      PATH-ERROR
