...
-7500 constant E-SIDE-SYNTAX
-7501 constant E-SIDE-CAPACITY
-7502 constant E-SIDE-DIGEST
...
create SHA-CTX SHA256-CTX-BYTES allot   \ the writer's digest context
...
: REQUIRE-STATE ( n -- )
   STATE @ <> if E-SIDE-SYNTAX throw then ;

: CHECK-ADD ( n n -- n ) {: total:n add:n :}
   total 0 < add 0 < or if E-SIDE-CAPACITY throw then
   total add + total < if E-SIDE-CAPACITY throw then
   total add + ;

: NEED ( n -- ) {: add:n :}
   OUT-U @ add CHECK-ADD OUT-CAP @ > if E-SIDE-CAPACITY throw then ;

: U8 ( n -- ) {: value:n :}
   value 0 < value $FF > or if E-SIDE-SYNTAX throw then
   1 NEED
   value OUT-A@ OUT-U @ + c!
   OUT-U @ 1+ OUT-U ! ;

: U64! ( n ptr u8 -- ) {: value:n dst:ptr :}
   value 0 < if E-SIDE-SYNTAX throw then
   0 begin dup 8 < while
...
: KIND-CHECK ( bool content-kind ptr u8 n n -- )
   ...
   present if
      ...
      path pathu PATH-SAFE? 0= if E-SIDE-SYNTAX throw then
   else
      kind KIND-ABSENT? 0= if E-SIDE-SYNTAX throw then
      pathu 0<> bodyu 0<> or if E-SIDE-SYNTAX throw then
   then
   kind KIND-GITLINK? if
      bodyu 0<> if E-SIDE-SYNTAX throw then
   then ;
...
: HEADER ( n ptr u8 n -- ) {: count:n meta:ptr metau:n :}
   ...
   DIGEST-U ZEROES
   metau U64
   SHA-CTX meta metau SIDE-DIGEST SHA256-IN
...
: ARTIFACT-DIGEST ( -- )
   ...
   SHA-CTX OUT-A@ 32 SHA256-FEED
   SHA-CTX ZERO-DIGEST DIGEST-U SHA256-FEED
   SHA-CTX OUT-A@ 64 + OUT-U @ 64 - SHA256-FEED
   SHA-CTX PAYLOAD-DIGEST SHA256-END ;
...
: START ( ptr u8 n n ptr u8 n -- )
   ...
   count 0 < if E-SIDE-SYNTAX throw then
   cap HEADER-U TRAILER-U + < if E-SIDE-CAPACITY throw then
...
: ROW-BEGIN ( n n n -- ) {: ordinal:n oldu:n newu:n :}
   ...
   ordinal ROW-N @ <> if E-SIDE-SYNTAX throw then
   oldu SIDE-HEAD-U < newu SIDE-HEAD-U < or if E-SIDE-SYNTAX throw then
   OUT-U @ ROW-START !
   oldu ROW-OLD-U !
   newu ROW-NEW-U !
   0 SIDE-N !
...
: ROW-END ( -- )
   ...
   SIDE-N @ 2 <> if E-SIDE-SYNTAX throw then
   OUT-U @ ROW-START @ - {: got:n :}
   OUT-A@ ROW-START @ + 16 + FS-U64@ got <> if E-SIDE-SYNTAX throw then
   OUT-A@ ROW-START @ + 24 + FS-U64@ ROW-OLD-U @ <> if E-SIDE-SYNTAX throw then
   ROW-N @ $7FFFFFFFFFFFFFFF = if E-SIDE-CAPACITY throw then
   ROW-N @ 1+ ROW-N !
   STATE-READY STATE ! ;
...
: SIDE-CHUNK ( ptr u8 n -- ) {: a:ptr u:n :}
   ...
   BODY-SEEN @ u CHECK-ADD BODY-EXPECT @ > if E-SIDE-CAPACITY throw then
...
: SIDE-END ( -- )
   STATE-SIDE REQUIRE-STATE
   BODY-SEEN @ BODY-EXPECT @ <> if E-SIDE-SYNTAX throw then
...
: SIDE ( bool content-kind bool ptr u8 n ptr u8 n -- )
   ...
   present kind path pathu bodyu SIDE-BEGIN
   body bodyu SIDE-CHUNK
   SIDE-END
   OUT-A@ SIDE-START @ + 3 + c@ 0<> if
      binary 0= if E-SIDE-SYNTAX throw then
   else
      binary if E-SIDE-SYNTAX throw then
   then ;

: FINISH ( -- ptr u8 n )
   STATE-READY REQUIRE-STATE
   ROW-N @ EXPECT-N @ <> if E-SIDE-SYNTAX throw then
   ...
   total TRAILER
   OUT-U @ total <> if E-SIDE-CAPACITY throw then
   OUT-A@ OUT-U @ ;
