\ rmsnorm-device-test.f - GB10 device proof of the checked RMSNORM_ROWS forward and
\ its RMSNORM_BWD_ROWS VJP: a forward golden vs the maki CPU reference and a
\ finite-difference gradcheck of the backward, both on the actual GPU.
\
\ Self-contained, mirrors tools/ptx/softmax-gradcheck.f: spawns bin/hb to emit ONE
\ PTX module holding BOTH RMSNORM_ROWS and RMSNORM_BWD_ROWS (tools/ptx/rmsnorm-cg.f),
...
\ cubin, and pulls both handles. FORWARD GOLDEN: run RMSNORM_ROWS on a fixed row and
...
create HDXN 4 cells allot create HYREF 4 cells allot create HYDEV 4 cells allot
...
\ spawn bin/hb to emit the combined fwd+bwd module (rmsnorm-cg.f) to the private PTX
...
: RN-EMIT ( -- n )
   ...
   s" tools/ptx/rmsnorm-cg.f"  >LEN PROC-ARGV+
...
\ backward RMSNORM_BWD_ROWS: (HX, HDY) -> HDXA  (x=%rd1 dy=%rd2 out=%rd3 k=%r1)
: RN-BWD-RUN ( -- )
   RN-OUT 16 PTXSENT:FILL
   1 RNK 256 PTX-ROW-LAUNCH-CHECK
...
: RMSNORM-DEVICE-MAIN ( -- )
   ...
   RN-RUN
   PTXTC:CLEAN
   s" rmsnorm device: RMSNORM_ROWS forward golden vs maki RMS-FWD AND RMSNORM_BWD_ROWS finite-difference gradcheck verified on " type ATGT:LABEL$ type cr
   T-REPORT ;
