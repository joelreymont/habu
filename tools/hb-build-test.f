...
942 constant HBT-DEEP-ROOT-LEN
$2F constant HBT-SLASH
$7E constant HBT-DEEP-C
...
create HBT-DEEP-ROOT-BUF HBT-DEEP-ROOT-LEN allot
...
: HBT-DEEP-ROOT ( -- ptr u8 n )
   HBT-DEEP-ROOT-BUF HBT-DEEP-ROOT-LEN ;
...
: HBT-PREPARE-DEEP-ROOT ( -- )
   HBT-ROOT {: a:ptr u:n :}
   u HBT-DEEP-ROOT-LEN >= if E-FS-CAPACITY throw then
   a HBT-DEEP-ROOT-BUF u BYTE-COPY
   u begin dup HBT-DEEP-ROOT-LEN < while
      dup u - 101 mod 0= if HBT-SLASH else HBT-DEEP-C then
      over HBT-DEEP-ROOT-BUF + c!
      1+
   repeat drop ;
...
: HBT-CHECK-REPORT ( ptr u8 n n n n n n -- )
...
: HBT-BUILD-REPL ( -- )
   ...
   HB-BUILD:REPORT$ JT-FALSE JT-FALSE JT-FALSE JT-TRUE JT-TRUE HBT-CHECK-REPORT ;
...
: HBT-REBUILD-REPL-CACHE ( -- )
   ...
   HBT-REPL-OUT FILE? TTRUE
   HB-BUILD:REPORT$ JT-TRUE JT-FALSE JT-FALSE JT-FALSE JT-FALSE HBT-CHECK-REPORT ;
...
: HBT-CLI-REPORT ( -- )
   ...
   rc 0 T=
   erru 0 T=
   HBT-OUT outu JT-TRUE JT-FALSE JT-FALSE JT-FALSE JT-FALSE HBT-CHECK-REPORT ;
...
: HBT-CHECK-PATH-JSON ( -- )
   ...
   rc HBB-BUILD-RC T=
   outu 0 T=
   HBT-ERR erru JR-INIT
   JR-NEXT JT-OBJ T=
   s" schema" JR-FIND-KEY TTRUE
   s" hb-build-error" HBT-REPORT-STRING=
   s" version" JR-FIND-KEY TTRUE
   JR-TOKEN JT-INT T=
   JR-INT 1 T=
   s" code" JR-FIND-KEY TTRUE
   s" E-BUILD-PATH" HBT-REPORT-STRING=
   s" cache_selected" JR-FIND-KEY TTRUE
   JR-TOKEN JT-TRUE T=
   s" cache_root" JR-FIND-KEY TTRUE
   HBT-BAD-OUT HBT-REPORT-STRING=
   s" cache_source" JR-FIND-KEY TTRUE
   s" explicit" HBT-REPORT-STRING=
   s" cause" JR-FIND-KEY TTRUE
   s" E-FS-DIR" HBT-REPORT-STRING=
...
: HBT-DEEP-CACHE-PATH-REJECT ( -- )
   HBT-DEEP-ROOT MAKE-DIRS
   HBB-RESET-OPTIONS
   HBT-AOT-SRC HBT-AOT-OUT HBT-HBB-PREPARE-AOT
   HBT-DEEP-ROOT BUILD-CACHE:ROOT!
   s" deep cache child rejects" T-LABEL
   [: HBB-PREPARE-ARTIFACT-CACHE ;] catch E-BUILD-PATH T=
   s" deep cache child selected" T-LABEL
   BUILD-CACHE:SELECTED? TTRUE
   s" deep cache child root" T-LABEL
   BUILD-CACHE:SELECTED-ROOT$ HBT-DEEP-ROOT T$=
   s" deep cache child source" T-LABEL
   BUILD-CACHE:SELECTED-SOURCE BUILD-CACHE:SOURCE$ s" explicit" T$=
   s" deep cache child cause" T-LABEL
   BUILD-CACHE:CAUSE E-FS-CAPACITY T=
   s" deep cache child report" T-LABEL
   HB-BUILD:PATH-ERROR$ s\" \qcause\q:\qE-FS-CAPACITY\q" CONTAINS? TTRUE
   BF-TMP-RESET
   HBT-TMP BUILD-CACHE:ROOT! ;
...
: HBT-REPORT-INVALIDATION ( -- )
   ...
   HBT-REPL-SRC HBT-REPL-OUT HBT-HBB-PREPARE-REPL
   [: HB-BUILD:BUILD ;] catch E-BUILD-PATH T=
   BF-TMP-RESET
...
: HBT-BUILD-AOT-OBJECT-PRODUCER ( -- )
   ...
   HB-BUILD:REPORT$ JT-FALSE JT-TRUE JT-FALSE JT-FALSE JT-FALSE HBT-CHECK-REPORT
   HBT-RUN-AOT
   HBT-REMOVE-ARTIFACT
   HBT-REMOVE-AOT-OUT
   BF-TMP-RESET ;
...
: HBT-BUILD-AOT-MAKER-HIT ( -- )
   ...
   HBB-MAKER-RUN @ 0 <> TTRUE
   HB-BUILD:REPORT$ JT-FALSE JT-FALSE JT-TRUE JT-FALSE JT-TRUE HBT-CHECK-REPORT
   HBT-REMOVE-ARTIFACT
   HBT-REMOVE-AOT-OUT
   HBT-AOT-SRC HBT-AOT-SRC$ WRITE-ALL
   BF-TMP-RESET ;

: HBT-BUILD-AOT-MAKER-LATE-HIT ( -- )
   HBB-RESET-OPTIONS
   HBT-TMP BUILD-CACHE:ROOT!
   HBT-AOT-SRC HBT-AOT-OUT HBT-HBB-PREPARE-AOT
   HBB-PREPARE-MAKER-CACHE
   s" late maker is ready" T-LABEL
   HBB-MAKER-READY? TTRUE
   0 HBB-MAKER-HIT !
   0 HBB-MAKER-BUILD !
   HBB-BUILD-MAKER-FRESH
   s" late maker records hit" T-LABEL
   HBB-MAKER-HIT @ 0 <> TTRUE
   s" late maker does not record build" T-LABEL
   HBB-MAKER-BUILD @ 0= TTRUE
   BF-TMP-RESET ;
...
: HBT-BUILD-AOT-WRONG-OBJECT-FAILS ( -- )
   ...
   HBT-STORE-WRONG-AOT-OBJ
   HBT-AOT-SRC HBT-AOT-OUT HBT-HBB-PREPARE-AOT
   [: HB-BUILD:BUILD ;] E-OBJ-SCHEMA TTHROWSQ
   HBB-MAKER-RUN @ 0= TTRUE
...
: HBT-REPORT-SURFACE ( -- )
   s" HB-BUILD:CAPTURE" XREF-FIND 0= TTRUE
   s" HB-BUILD:RESET" XREF-FIND 0= TTRUE
   s" HB-BUILD:COMMIT" XREF-FIND 0= TTRUE
   s" HB-BUILD:BUILD" XREF-FIND 0= 0= TTRUE
   s" HB-BUILD:MAIN" XREF-FIND 0= 0= TTRUE ;
