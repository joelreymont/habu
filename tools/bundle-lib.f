...
: BL-NEEDS-COMBINATORS? ( ptr u8 n -- bool )
   s" array" BL-STR= ;

: BL-EMIT-COMBINATORS ( -- )
   BL-COMBINATORS-OUT @ if exit then
   s" src/core/combinators.f" BL-EMIT-FILE
   -1 BL-COMBINATORS-OUT ! ;

: BL-EMIT-DEPS ( ptr u8 n -- )
   BL-NEEDS-COMBINATORS? if BL-EMIT-COMBINATORS then ;
...
: BL-VERIFY-COMBINATORS ( ptr u8 n -- )
   BL-NEEDS-COMBINATORS? if
      s" src/core/combinators.f" BL-FILE? 0= if
         s" bundle-lib: missing dependency" BL-NOINPUT-RC BL-DIE
      then
   then ;
...
: BL-VERIFY ( -- )
   ...
   0 begin dup BL-NMOD @ < while
      ...
      dup BL-MOD$ BL-VERIFY-COMBINATORS
      1+
   repeat drop ;
