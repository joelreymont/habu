...
- **A whole-file DSL consumer imports once and speaks the public DSL.** The JSON
  performance probe became a misleading executable example when it reverted to
  `TEST-RUN:PREPARE`/`TEST-RUN:COMPLETE` after the file-scoped form was chosen.
  Use `using TEST-RUN` once, call its public lifecycle words bare, and let
  end-of-file close the import; write `;using` only to end the scope early.
  Prefer a semantic block pair such as `RUN … ;RUN` when the owner exposes one.
  Temporary probes must use the accepted source shape.
