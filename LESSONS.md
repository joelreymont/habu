...
- `function-lambda-expression` must snapshot lambda source before the compiler starts lowering. `src/compiler/compile.zig:3053-3104,4568-4596,5346-5357` was storing `(lambda . args)` by aliasing the live source conses, while later lowering paths such as `setf` place compilation mutate source lists in place (`src/compiler/compile.zig:7952-7955`). The generic proof is `src/tests/integration.zig`, where a compiled lambda containing `(setf (aref v 0) x)` now round-trips through `function-lambda-expression` and `eval` without corruption. The workload proof is `src/tests/rtest6_stream.zig:618-822`, where the isolated problem-20 `tellsimp` probe can now re-evaluate and re-coerce the extracted rule lambda cleanly.
...
- Complex numbers must store exact Lisp values for real/imag parts, not baked `f64`s. `src/runtime/objects.zig:530-539`, `src/runtime/heap.zig:2286-2289`, `src/runtime/gc.zig:2268-2272`, `src/runtime/primitives/arith.zig:1416-1469`, `src/runtime/primitives/complex.zig:9-112`, `src/runtime/primitives/hash.zig:475-476,566-570,804-812,933-941`, `src/runtime/primitives/io.zig:561-567,789-795`, `src/interp/vm.zig:7221-7234,8774-8778,8820,11727-11731`, and `src/reader/parser.zig:418-443` now keep exact fixnum/rational parts alive through reader, GC, arithmetic, hashing, and printing. The direct proof is `src/tests/integration.zig:7910-7940`, including `(write-to-string (complex 1/2 1/3)) => "#C(1/2 1/3)"`. The workload proof is canonical `./zig-out/bin/habu tools/maxima-rtest.lisp rtest6`, where problem `6` disappears and residue drops from `(2 6 39 47)` to `(2 39 47)`.
- Lambda-local `declare (special ...)` must be seeded into the lexical environment before lambda-list/default/body compilation, not only during later body `declare` filtering. `src/compiler/compile.zig:1482-1761,4560-5315,6298-6397,15472-15584` now carries lexical special declarations in `Env`, uses them when deciding whether symbol refs/sets are lexical or dynamic, and lets lambda parameter special-binding emission see locally declared specials before compiling defaults. The generic proof is `src/tests/integration.zig:12043-12114`, including `(funcall (eval '(lambda (x y z) (declare (special x y z)) x)) '(a) 2 3) => (A)`. The workload proof is canonical `./zig-out/bin/habu tools/maxima-rtest.lisp rtest6`, where problem `20` disappears and residue drops to `(2 6 39 47)`.
...
### Did Not Work
- Deleting the same-chunk `return-from` optimization outright was wrong. It masked the `progv` leak, but it immediately broke plain lexical `loop`/`return` code in external runs: `pregexp:pregexp` started failing in `PREGEXP-READ-BRANCH` with `TypeMismatch`, which then poisoned `commac` and loader cleanliness. The correct fix was not “always runtime”; it was “runtime only when the path crosses a real cleanup scope”.
- Probing `./zig-out/bin/habu` after only `zig build test` is still invalid. The stale binary hid the character-vector fix and falsely suggested `stringproc` was still failing on the old `(setf (char *chr64* 0) ...)` path; only a fresh `zig build` proved that problem `47` was actually gone.
- Keeping `#C(...)` reader canonicalization in float space was wrong. `src/reader/parser.zig` originally converted both parts through `toReal` and collapsed `#C(x 0)` via `imag == 0.0`, which would have reintroduced exactness loss even after the runtime cutover. The reader has to keep parts as `Value`s and use numeric-zero comparison (`numEq`) instead.
...
- `defmethod` class specializers must be stored and compared by canonical qualified symbol identity, not by raw local pname. `src/compiler/compile.zig:2507-2539,13982-14090,14761-14773` now canonicalizes built-in type symbols before storing specializers, preserves the qualified name for later `typep`/CPL checks, and resolves that qualified name back to the real symbol instead of re-interning in the current package. The direct proof is `src/tests/integration.zig:9113-9137`, where a package that shadows `NUMBER` still dispatches a `(defmethod ... ((x number)) ...)` on fixnums while `method-specializers` reports the canonical class. This fixed a real CLOS correctness bug even though it did not move the separate Maxima `BIGFLOAT` loader gate.
- Built-in type dispatch must canonicalize same-pname package symbols back to the canonical `COMMON-LISP` type symbol before matching. Maxima intentionally shadows `FLOAT` in `../maxima/src/maxima-package.lisp`, but `../maxima/src/hypergeometric.lisp:219-221` still uses `'float` as a type designator. `src/runtime/primitives/type.zig` now resolves non-CL symbols like `MAXIMA::FLOAT` to `COMMON-LISP:FLOAT` through read-only package lookup before built-in `typep`/`subtypep` dispatch, which fixes the real loader stop without another Maxima patch layer.
- Recursive compiler walkers over Lisp lists must root the live list head/tail across any nested `compile*` call that can allocate. `src/compiler/compile.zig:7030-7165` was keeping `and`/`or` argument tails in raw locals while compiling the first clause, so moving GC could invalidate the recursive tail and crash on the next cons dereference. Reusing the compiler's existing root-stack path for the whole live arg list, then recomputing `rest`/`second` from the rooted value after each nested compile, fixes the root cause generically for short-circuit walkers.
- Composite semantic outputs should be asserted through snapshot helpers, not by cons-walking in every test. `src/testing/snapshot.zig` now centralizes `write-to-string` + OhSnap for `src/tests/integration.zig` and `src/tests/oracle.zig`. Two Zig-specific gotchas matter: `OhSnap.snap` needs both `SourceLocation` and the expected text as `comptime` parameters, and when a multiline string is an argument, the separating comma must live outside the `\\...` literal line or Zig treats it as part of the string and silently drops an argument.
- Lambda parameters that are both captured and mutated need the same boxing cutover as `let` bindings, but at lambda entry instead of in a synthetic `let`. `src/compiler/compile.zig:4492-5216` now records symbol-backed required/optional/key/rest/supplied-p slots, computes the captured-and-mutated intersection for the lambda body, and carries the boxed slot list into `src/compiler/ir.zig:93-124,1346-1389,3068-3126`. `src/bytecode/emit.zig:1737-1744` then materializes those slots as boxes after argument/default setup and before body execution. The direct proof is `src/tests/integration.zig:4188-4215`, where `(defun param-labels (x) (labels ((f () (setq x '(a)) 0)) (list (f) x)))` now returns `(0 (A))` instead of `(0 NIL)`.
...
### Did Not Work
- Assuming the surviving `BIGFLOAT::/` failure was another type-canonicalization issue was wrong. The decisive probe was comparing `(method-function ...)` against the generic call on the same `(NUMBER NUMBER)` method after the Maxima prefix load: the method body returned `1/2`, but the generic call failed. That isolated the bug to dispatcher helper-name collision instead of method semantics.
...
- Compiler walkers must snapshot list structure before recursive sub-compilation when later iterations still need it. `src/compiler/compile.zig:7589-7618` was iterating raw `setf` place/value cons cells and recursing into `compileSetf` before advancing to `pair2.cdr`, so GC could move the list and leave the next dereference stale. Materializing the `(place,value)` pairs first and resolving them live per iteration fixed the fresh-executable `compileSetf` segfault and the new small-heap multipair `setf` regression.
