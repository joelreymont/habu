...
### Did Not Work
- Treating the surviving `hypergeometric`/`nfloat` loader overflow as just the remaining `defmethod` specializer bug was too narrow. The new specializer regression now passes, but canonical `tools/maxima-rtest.lisp rtest6` still dies at the same dirty-loader gate, so the `BIGFLOAT` `#.` path has another upstream state bug beyond CLOS specializer identity.
