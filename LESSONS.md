...
- **Consumer-only orchestration should speak the imported DSL.** An entry,
  runner, or orchestration file that calls several public words from one package
  uses `using PKG ... ;using` and bare words; repeated `PKG:WORD` hides the
  intended consumer DSL. Keep qualification for a one-off call or to escape a
