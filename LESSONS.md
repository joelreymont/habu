...
- **`IBUFSZ` measures baked prefix plus appended driver:** a stage fixpoint can
  pass while the later stdin-maker crosses the input mmap watermark. Keep the
  native/bootstrap constants synchronized, retain the labeled rc-74 failure,
  and prove both the retired edge now succeeds and the new edge fails closed.
- **Scale dictionary capacity as one layout unit:** the gate-runner-support
  closure crossed 8192 records, so `DICT-CAP`, `CFSTK-OFF`, `DICT-SIZE`,
  `HIDX-SLOTS`, and `HIDX-BYTES` moved together; `LFIND` derives probe masks
  from `HIDX-SLOTS`. Capacity failures must name the exhausted resource.
