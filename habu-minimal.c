...
habu_value_t VECTOR_P(habu_value_t);
habu_value_t STRING_P(habu_value_t);
habu_value_t NIL_P(habu_value_t);
habu_value_t STRING_COMPARE_LOOP(habu_value_t, habu_value_t, habu_value_t, habu_value_t);
...
habu_value_t IS_SPECIAL_CHAR_P(habu_value_t);
habu_value_t IS_SYMBOL_CHAR_P(habu_value_t);
habu_value_t DIGIT_TO_INT(habu_value_t);
habu_value_t READER_AT_END_P(habu_value_t);
...
habu_value_t SYM_EQ_P(habu_value_t, habu_value_t);
...
habu_value_t VECTOR_P(habu_value_t X) {
    return (value_to_fixnum(habu_get_tag(X)) == value_to_fixnum(fixnum_to_value(3)) ? fixnum_to_value(1) : NIL);
}
...
habu_value_t STRING_COMPARE_LOOP(habu_value_t S1, habu_value_t S2, habu_value_t IDX, habu_value_t LEN) {
    return (is_nil((value_to_fixnum(IDX) >= value_to_fixnum(LEN) ? fixnum_to_value(1) : NIL)) ? (is_nil((value_to_fixnum(habu_string_ref(S1, value_to_fixnum(IDX))) == value_to_fixnum(habu_string_ref(S2, value_to_fixnum(IDX))) ? fixnum_to_value(1) : NIL)) ? NIL : STRING_COMPARE_LOOP(S1, S2, fixnum_to_value(value_to_fixnum(IDX) + value_to_fixnum(fixnum_to_value(1))), LEN)) : fixnum_to_value(1));
}

habu_value_t STRING_EQ_P(habu_value_t S1, habu_value_t S2) {
    return (is_nil(STRING_P(S1)) ? NIL : (is_nil(STRING_P(S2)) ? NIL : ({
    habu_value_t LEN1 = fixnum_to_value(habu_string_length_raw(S1));
    ({
      habu_value_t LEN2 = fixnum_to_value(habu_string_length_raw(S2));
      (is_nil((value_to_fixnum(LEN1) == value_to_fixnum(LEN2) ? fixnum_to_value(1) : NIL)) ? NIL : STRING_COMPARE_LOOP(S1, S2, fixnum_to_value(0), LEN1));
      });
    })));
}

habu_value_t SYMBOL_EQ_P(habu_value_t SYM1, habu_value_t SYM2) {
    return (is_nil(SYMBOL_P(SYM1)) ? NIL : (is_nil(SYMBOL_P(SYM2)) ? NIL : STRING_EQ_P(habu_symbol_name(SYM1), habu_symbol_name(SYM2))));
}
...
habu_value_t IS_SPECIAL_CHAR_P(habu_value_t CH) {
...
habu_value_t IS_SYMBOL_CHAR_P(habu_value_t CH) {
    return (is_nil(IS_ALPHA_P(CH)) ? (is_nil(IS_DIGIT_P(CH)) ? IS_SPECIAL_CHAR_P(CH) : fixnum_to_value(1)) : fixnum_to_value(1));
}
...
habu_value_t READER_AT_END_P(habu_value_t STATE) {
...
habu_value_t READER_PEEK(habu_value_t STATE) {
    return (is_nil(READER_AT_END_P(STATE)) ? habu_string_ref(habu_car(STATE), value_to_fixnum(habu_cdr(STATE))) : NIL);
}
...
habu_value_t SKIP_WS(habu_value_t STATE) {
    return (is_nil(READER_AT_END_P(STATE)) ? ({
    habu_value_t CH = READER_PEEK(STATE);
    (is_nil(IS_WHITESPACE_P(CH)) ? STATE : SKIP_WS(READER_ADVANCE(STATE)));
    }) : STATE);
}
...
habu_value_t PARSE_NUM(habu_value_t STATE, habu_value_t ACC) {
    return (is_nil(READER_AT_END_P(STATE)) ? ({
    habu_value_t CH = READER_PEEK(STATE);
    (is_nil(IS_DIGIT_P(CH)) ? habu_cons(ACC, STATE) : PARSE_NUM(READER_ADVANCE(STATE), fixnum_to_value(value_to_fixnum(fixnum_to_value(value_to_fixnum(ACC) * value_to_fixnum(fixnum_to_value(10)))) + value_to_fixnum(DIGIT_TO_INT(CH)))));
    }) : habu_cons(ACC, STATE));
}
...
habu_value_t READ_ONE(habu_value_t STATE) {
    ...
    (is_nil(READER_AT_END_P(S2)) ? ({
      habu_value_t CH = READER_PEEK(S2);
      (is_nil((value_to_fixnum(CH) == value_to_fixnum(fixnum_to_value(40)) ? fixnum_to_value(1) : NIL)) ? (is_nil(IS_DIGIT_P(CH)) ? habu_cons(NIL, S2) : PARSE_NUM(S2, fixnum_to_value(0))) : PARSE_LIST(S2));
      }) : habu_cons(NIL, S2));
    });
}
...
habu_value_t ENV_LOOKUP(habu_value_t SYM, habu_value_t ENV) {
    ...
    (is_nil(SYMBOL_EQ_P(habu_car(BINDING), SYM)) ? ENV_LOOKUP(SYM, habu_cdr(ENV)) : habu_cdr(BINDING));
    }) : NIL);
}

habu_value_t SYM_EQ_P(habu_value_t SYM, habu_value_t NAME) {
    return (is_nil(SYMBOL_P(SYM)) ? NIL : STRING_EQ_P(habu_symbol_name(SYM), NAME));
}

habu_value_t EVAL_LIST(habu_value_t EXPRS, habu_value_t ENV) {
    return (is_nil(NIL_P(EXPRS)) ? habu_cons(EVAL(habu_car(EXPRS), ENV), EVAL_LIST(habu_cdr(EXPRS), ENV)) : NIL);
}

habu_value_t APPLY_PLUS(habu_value_t ARGS) {
    return (is_nil(NIL_P(ARGS)) ? fixnum_to_value(value_to_fixnum(habu_car(ARGS)) + value_to_fixnum(APPLY_PLUS(habu_cdr(ARGS)))) : fixnum_to_value(0));
}

habu_value_t APPLY_MULT(habu_value_t ARGS) {
    return (is_nil(NIL_P(ARGS)) ? fixnum_to_value(value_to_fixnum(habu_car(ARGS)) * value_to_fixnum(APPLY_MULT(habu_cdr(ARGS)))) : fixnum_to_value(1));
}

habu_value_t APPLY_BUILTIN(habu_value_t FN, habu_value_t ARGS) {
    return (is_nil(SYM_EQ_P(FN, habu_make_string("+", 1))) ? (is_nil(SYM_EQ_P(FN, habu_make_string("*", 1))) ? (is_nil(SYM_EQ_P(FN, habu_make_string("-", 1))) ? (is_nil(SYM_EQ_P(FN, habu_make_string("cons", 4))) ? (is_nil(SYM_EQ_P(FN, habu_make_string("car", 3))) ? (is_nil(SYM_EQ_P(FN, habu_make_string("cdr", 3))) ? (is_nil(SYM_EQ_P(FN, habu_make_string("list", 4))) ? NIL : ARGS) : habu_cdr(habu_car(ARGS))) : habu_car(habu_car(ARGS))) : habu_cons(habu_car(ARGS), habu_car(habu_cdr(ARGS)))) : (is_nil(NIL_P(habu_cdr(ARGS))) ? fixnum_to_value(value_to_fixnum(habu_car(ARGS)) - value_to_fixnum(APPLY_PLUS(habu_cdr(ARGS)))) : fixnum_to_value(value_to_fixnum(fixnum_to_value(0)) - value_to_fixnum(habu_car(ARGS))))) : APPLY_MULT(ARGS)) : APPLY_PLUS(ARGS));
}
...
habu_value_t EVAL(habu_value_t EXPR, habu_value_t ENV) {
    ...
    ({
      ...
      (is_nil(SYM_EQ_P(FIRST, habu_make_string("quote", 5))) ? (is_nil(SYM_EQ_P(FIRST, habu_make_string("if", 2))) ? (is_nil(SYM_EQ_P(FIRST, habu_make_string("progn", 5))) ? APPLY_BUILTIN(FIRST, EVAL_LIST(ARGS, ENV)) : EVAL_PROGN(ARGS, ENV)) : (is_nil(NIL_P(EVAL(habu_car(ARGS), ENV))) ? EVAL(habu_car(habu_cdr(ARGS)), ENV) : EVAL(habu_car(habu_cdr(habu_cdr(ARGS))), ENV))) : habu_car(ARGS));
      });
    })) : NIL) : EXPR);
}

habu_value_t EVAL_PROGN(habu_value_t EXPRS, habu_value_t ENV) {
    return (is_nil(NIL_P(habu_cdr(EXPRS))) ? ({
