;;;; Extended Recursive REPL - v1.2 Features
;;;; Adds: and, or, not, <=, >=, !=, cond
...
;;;; Evaluator with quote, if, let, lambda, and, or, cond
              ...
              (if (symbol=? first (make-symbol (quote "if")))
                ...
                (if (symbol=? first (make-symbol (quote "let")))
                  ...
                  (if (symbol=? first (make-symbol (quote "lambda")))
                    ...
                    (if (symbol=? first (make-symbol (quote "and")))
                      (eval-and (cdr expr) env)
                      (if (symbol=? first (make-symbol (quote "or")))
...
;;; NEW: and - short-circuit evaluation
(defun eval-and (args env)
  (if (nil? args) (quote 1)  ; (and) => true
    (let ((val (eval-expr (car args) env)))
      (if (nil? val) (quote nil)
        (if (nil? (cdr args)) val  ; Last arg, return its value
          (eval-and (cdr args) env))))))

;;; NEW: or - short-circuit evaluation
(defun eval-or (args env)
  (if (nil? args) (quote nil)  ; (or) => false
    (let ((val (eval-expr (car args) env)))
      (if (nil? val)
        (eval-or (cdr args) env)
        val))))  ; Return first truthy value

;;; NEW: cond - multi-way conditional
(defun eval-cond (clauses env)
  (if (nil? clauses) (quote nil)
    (let ((clause (car clauses)))
      (let ((test (car clause)))
        (let ((body (car (cdr clause))))
          (if (nil? (eval-expr test env))
                ...
                (if (symbol=? op (make-symbol (quote "<=")))
                  ...
                  (if (symbol=? op (make-symbol (quote ">=")))
                    (if (>= (eval-expr (car args) env)
                      ...
                      (quote nil))
                    ;;; NEW: not operator
                      (if (symbol=? op (make-symbol (quote "not")))
                        (if (nil? (eval-expr (car args) env))
                          (quote 1)
                          (quote nil))
...
(defun repl-start ()
  (progn
    (print (quote "Habu REPL - Extended (v1.2)"))
    (println)
    (print (quote "Features: let, lambda, defun, and, or, not, cond, <=, >="))
    (println)
    (repl-loop (quote nil))))
