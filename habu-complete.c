...
habu_value_t EVAL_SIMPLE(habu_value_t);
...
habu_value_t EVAL_SIMPLE(habu_value_t EXPR) {
    return (is_nil(FIXNUM_P(EXPR)) ? (is_nil(NIL_P(EXPR)) ? (is_nil(CONS_P(EXPR)) ? EXPR : ({
    habu_value_t OP = habu_car(EXPR);
    (is_nil(SYMBOL_EQ_P(OP, habu_make_symbol_from_string(habu_make_string("+", 1)))) ? (is_nil(SYMBOL_EQ_P(OP, habu_make_symbol_from_string(habu_make_string("-", 1)))) ? (is_nil(SYMBOL_EQ_P(OP, habu_make_symbol_from_string(habu_make_string("*", 1)))) ? EXPR : fixnum_to_value(value_to_fixnum(EVAL_SIMPLE(habu_car(habu_cdr(EXPR)))) * value_to_fixnum(EVAL_SIMPLE(habu_car(habu_cdr(habu_cdr(EXPR))))))) : fixnum_to_value(value_to_fixnum(EVAL_SIMPLE(habu_car(habu_cdr(EXPR)))) - value_to_fixnum(EVAL_SIMPLE(habu_car(habu_cdr(habu_cdr(EXPR))))))) : fixnum_to_value(value_to_fixnum(EVAL_SIMPLE(habu_car(habu_cdr(EXPR)))) + value_to_fixnum(EVAL_SIMPLE(habu_car(habu_cdr(habu_cdr(EXPR)))))));
    })) : NIL) : EXPR);
}

habu_value_t REPL_START() {
    return ({
  habu_print_value(habu_make_string("Habu REPL", 9));
  ({printf("\n"); NIL;});
  REPL_LOOP();
});
}
...
habu_value_t REPL_LOOP() {
  ...
  ({
        habu_value_t RESULT = EVAL_SIMPLE(fixnum_to_value(42));
        ({
