...
- The current external `rtest6` crash floor is a multi-cycle forwarded-root bug, not a random corrupt package object. `COMMON-LISP:*PACKAGE*` in `src/interp/vm.zig` / `src/runtime/gc.zig` can survive one GC as a forwarded root, then re-enter a later GC with a forwarding target that is itself forwarded. The old one-hop repair path in `copyValue` accepts that intermediate stale target as final and re-roots it, which eventually leaves `globals[102]` pointing at a boxed object whose first word is still a forwarding cell (`bad boxed root`). The fix has to make GC/VM forwarding repair follow the whole chain to a non-forwarding live object before writing roots back.
...
- The `testsuite.lisp` poison is no longer a reader-conditional mystery. `src/tests/rtest6_stream.zig` now proves that replaying the already-read printed `defparameter $testsuite_files ...` form still poisons the later `rtest6` `20..39` slice after resetting back to plain `"rtest6"`, so the live floor is in plain form evaluation / top-level state effects, not in `#+`/`#-` handling while reading `../maxima/src/testsuite.lisp`.
- Nested runtime `eval` was falsely reporting `ControlTransfer` when a callback only changed the multiple-values buffer. `src/interp/vm.zig` was treating `secondary_values_count` / `zero_values_returned` deltas as control movement in `hostCallbackMovedControl`, so `(let ((*package* ...)) (eval (read-from-string \"(progn 1)\")))` underflowed on the caller `ret` after `read-from-string` left one secondary value and the nested `eval` returned one primary value. The control-transfer check must stay about control state only; multiple-values state is part of the callback result surface.
...
- The nested `loadFile` compiler crash floor is closed. `src/compiler/compile.zig` no longer keeps raw lambda parameter/type `Value`s in `TypedParam` across body compilation and nested loads; it now precomputes stable lexical keys and retained type expressions before the post-body type-assertion pass. The focused proof is `src/tests/rtest6_stream.zig`, where the `rtest6` problem-20 eval/loadFile split now completes instead of segfaulting in `lookupTypeDeclKey`. Canonical external residue remains semantic at `20/22/39/54`.
...
- `function-lambda-expression` was handing back aliased compiler source, not a stable snapshot. `src/compiler/compile.zig:3053-3104,4585-4596,5373` now snapshots the lambda source tree before destructive lowering, which closes the generic mutable-source regression and lets the problem-20 isolated `tellsimp` probe re-evaluate the extracted rule lambda cleanly. This fixes introspection/source-stability, but the standalone external `tools/maxima-rtest.lisp rtest6` floor still reports real `20/22` semantic failures, so the next cut remains in the script/runner path rather than in `function-lambda-expression`.
- The generic `setSymbolPlist` GC hole is closed in `src/runtime/primitives/symbol.zig:248-382,515-540`. Rooting the target symbol through flat-plist-to-alist conversion removed one real stale-plist writeback bug, but it did not move canonical `rtest6`. The live `tellsimp` floor is now inside the generated rule function path itself: after the problem-20 setup from `../maxima/tests/rtest6.mac:73-76`, direct `(funcall '$frule1 '(($f simp) 1) 1 t)` still type-mismatches even though runtime-built `definitely-so`/special-binding probes pass. Focus the next cut on generated rule-function calling convention and compiled body semantics, not another plist-mutation theory.
...
- That boundary theory is now falsified. A temp wrapper script loaded through normal script mode reproduces the `tellsimp` loss for `rtest6` problems `20/22`: direct `maxima::meval*` of the exact strings from inside the loaded wrapper leaves `mget '$f 'operators` and `mget '$subvarrule1 'ruleof` nil. The next cut belongs in `LOAD`-active execution semantics, not in the external process boundary.
- Standalone repro `/tmp/progv_nlx_repro.lisp` now proves a generic VM/compiler bug: `error` and `throw` restore special `let` bindings, but `return-from` leaves the dynamic binding leaked (`RETURN (:OK NIL)` instead of `(:OK 10)`). That points at `src/bytecode/emit.zig:2119-2131` `emitProgv`, which currently emits a lexical `push_progv ... pop_progv` pair without unwind cleanup. The next cutover is to lower `progv` with unwind cleanup so non-local exits across its body restore bindings before block transfer continues.
...
- Live blocker narrowing (2026-04-12): semispace GC now gets past the duplicate-symbol stale-forward bug (`fa7cb77e`), but the next external `rtest6` crash is a vector layout invariant violation. `src/runtime/primitives/vector.zig:216,443` mutates live vectors to point at another vector's payload, while `src/runtime/objects.zig:904-1006` and `src/runtime/gc.zig:1843-1948` still assume `Vector.data` is always inline and physically owned by the current header. Fix this by giving vectors explicit backing ownership and physical-capacity tracking, then teach GC/object sizing/stale-root validation to follow that ownership instead of assuming inline payload forever.
