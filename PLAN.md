...
- The live `rtest6` problem-22 `tellsimp` floor is now narrowed to generic plist mutation under moving GC, not Maxima rule semantics. `src/runtime/primitives/list.zig` currently runs the `put`/`remprop` read-modify-write path with an unrooted target symbol across plist flattening and new cons allocation, so a GC in the middle can write the updated plist back through a stale symbol pointer and leave rule/operator metadata missing. Fix the public plist mutators by rooting the symbol for the whole mutation, prove it with a GC-forcing runtime regression, then rerun canonical `tools/maxima-rtest.lisp rtest6`.
