...
- Current loader floor after `ae2e71d5` is still in `BIGFLOAT` reader-time evaluation, not ordinary `typep`/`subtypep`: loading `../maxima/src/hypergeometric.lisp` and `../maxima/src/nfloat.lisp` overflows in `COMPLEX-NUMBER-P -> BIGFLOAT -> BIGFLOAT-OR-NUMBER-P -> $RATNUMP -> $RATP` during `#.` evaluation. `defmethod` specializer canonicalization was a real independent bug and is now fixed in `src/compiler/compile.zig`, but the loader gate did not move, so the remaining cause is further upstream in numeric/read-time state.
