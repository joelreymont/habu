\ bundle-lib-core.f - concatenate stdlib modules and one driver source.
...
: BL-USAGE ( -- )
   s" usage: bin/hb --load ... tools/bundle-lib.f -- [-o out.f] module... -- script.f" BL-USAGE-RC BL-DIE ;

: BL-BYTE-COPY ( ptr u8 ptr u8 n -- ) {: src:ptr dst:ptr u:n :}
   0 begin dup u < while
      dup src + c@ over dst + c!
      1+
   repeat drop ;

: BL-STR= ( ptr u8 n ptr u8 n -- bool ) {: a:ptr u:n b:ptr v:n :}
...
: BL-ZCOPY ( ptr u8 n ptr u8 -- ptr u8 ) {: a:ptr u:n dst:ptr :}
...
: BL-TRY-STAT-MODE ( ptr u8 n -- n ) {: a:ptr u:n :}
...
: BL-MODULE? ( ptr u8 n -- bool ) {: a:ptr u:n :}
...
: BL-DASH? ( ptr u8 n -- bool ) {: a:ptr u:n :}
...
: BL-MOD$ ( n -- ptr u8 n ) {: idx:n :}
   idx BL-MOD-A@ idx BL-MOD-U@ ;

: BL-MOD+ ( ptr u8 n -- ) {: a:ptr u:n :}
...
: BL-OUT! ( ptr u8 n -- ) {: a:ptr u:n :}
   a BL-OUT-A !
   u BL-OUT-U !
   -1 BL-OUT-SET ! ;

: BL-SCRIPT! ( ptr u8 n -- ) {: a:ptr u:n :}
   a BL-SCRIPT-A !
   u BL-SCRIPT-U ! ;
...
: BL-MODULE-PATH ( ptr u8 n -- ptr u8 n ) {: a:ptr u:n :}
   BL-PATH-RESET
   s" lib/" BL-PATH+
   a u BL-PATH+
   s" .f" BL-PATH+
...
: BL-COPY-FILE ( ptr u8 n -- ) {: a:ptr u:n :}
...
: BL-EMIT-BEGIN ( ptr u8 n -- ) {: a:ptr u:n :}
   s" \ BEGIN " BL-WRITE
   a u BL-WRITE
   BL-WRITE-LF ;

: BL-EMIT-END ( ptr u8 n -- ) {: a:ptr u:n :}
   BL-WRITE-LF
   s" \ END " BL-WRITE
   a u BL-WRITE
   BL-WRITE-LF ;

: BL-EMIT-FILE ( ptr u8 n -- ) {: a:ptr u:n :}
   a u BL-EMIT-BEGIN
   a u BL-COPY-FILE
   a u BL-EMIT-END ;

\ Mark a CARRIED module's path as already provided, so a `require lib/<mod>.f`
\ line inside another bundled module short-circuits instead of re-reading the
\ file from disk (which would redefine its words - duplicate definition). This
\ mirrors how the native engine marks its baked prefix files provided.
: BL-EMIT-PROVIDED ( ptr u8 n -- ) {: a:ptr u:n :}
   s\" s\q " BL-WRITE                          \ emit: s"<space>
   a u BL-WRITE
   s\" \q provided" BL-WRITE                    \ emit: " provided
   BL-WRITE-LF ;

: BL-VERIFY-MODULE ( n -- ) {: idx:n :}
...
: BL-TEMP-PATH ( ptr u8 n -- ptr u8 n ) {: a:ptr u:n :}
   BL-TMP-RESET
   a u BL-TMP+
   s" .bundle-lib." BL-TMP+
   mono-ns BL-APPEND-U
...
: BL-RENAME ( ptr u8 n ptr u8 n -- ) {: src:ptr srcu:n dst:ptr dstu:n :}
...
\ State a module the ENGINE already carries. The bundle does not copy such a
\ file - a second copy would redefine every word in it the moment the bundle ran
\ - so it records the assumption instead, and src/core/include.f
\ ?ENGINE-PROVIDES turns that record into a load-time refusal naming the file if
\ the engine running the bundle turns out not to have it.
: BL-EMIT-ASSUMED ( ptr u8 n -- ) {: a:ptr u:n :}
   s\" s\q " BL-WRITE
   a u BL-WRITE
   s\" \q ?ENGINE-PROVIDES" BL-WRITE
   BL-WRITE-LF ;

\ A module the engine provides is assumed; every other module is carried.
\ "Self-contained" therefore means self-contained relative to a STATED engine
\ surface, checked when the bundle loads - not relative to nothing, which is
\ what copying a file the engine already has silently pretended.
: BL-ASSUMED? ( ptr u8 n -- bool )
   ENGINE-PROVIDES? ;

: BL-EMIT-HEAD-ROW ( n -- ) {: ix:n :}
   ix BL-MOD$ BL-MODULE-PATH 2dup BL-ASSUMED? if BL-EMIT-ASSUMED exit then
   BL-EMIT-PROVIDED ;

: BL-EMIT-HEAD-ALL ( -- )
   0 begin dup BL-NMOD @ < while
      dup BL-EMIT-HEAD-ROW
      1+
   repeat drop ;

: BL-EMIT-BODY-ROW ( n -- ) {: ix:n :}
   ix BL-MOD$ BL-MODULE-PATH 2dup BL-ASSUMED? if 2drop exit then
   BL-EMIT-FILE ;

: BL-EMIT-BODY-ALL ( -- )
   0 begin dup BL-NMOD @ < while
      dup BL-EMIT-BODY-ROW
      1+
   repeat drop ;
...
: BL-EMIT-BUNDLE ( -- )
   ...
   BL-WRITE-LF
   s" \ `provided` rows are carried below; `?ENGINE-PROVIDES` rows are assumed" BL-WRITE
   BL-WRITE-LF
   s" \ of the engine and refused by name at load if it lacks them." BL-WRITE
   BL-WRITE-LF
   BL-EMIT-HEAD-ALL
   BL-EMIT-BODY-ALL
