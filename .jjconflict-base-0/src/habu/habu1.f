...
\ loop, native compiler dispatch and ENGINE-EMIT:FORTH follow in part 2 (habu2.f).
...
;package
\ runtime instruction-word constants emitted into the code region
...
\ The region's write bands track dictionary-record and code spans independently.
\ The bodies are EMIT-PROT-WINDOW below; definition, publication and patch
\ brackets in this file and habu2.f are their callers.
package PROT
public
variable LOPEN   variable LCLOSE   variable LGROW   variable LSPAN
;package
...
variable LENTRY
;package
variable LKWCMP  variable LBCAP  variable LBCS
variable LCREATE  variable LDOESPATCH
...
variable LKWLBRACE variable LKWCONST
...
variable LKWTRUSTED variable LKWKERNEL variable LKWPRIVATE variable LKWSEMIPACKAGE variable LKWDUPDEF variable LCHKPACKAGE variable LCHKPUB variable LCHKPRI variable LCHKENDPKG
...
\ Shared span-guard call. addr and len name the caller's runtime registers.
\ Move that pair into the x10/x11 ABI the engine-resident LPROTSPAN body reads,
\ handling every source-register aliasing case without losing a value, then
\ branch-with-link to the single guard body. This replaces the fully inlined
\ GUARD-SPAN at every emitter sink, so the multi-band interval test lives once
\ in the engine text (registered as the (PROT-SPAN) helper) instead of being
\ copied into each guarded primitive. The branch clobbers x10-x13 and x30; a
\ sink that needs any of those live across the guard must save it first (see
\ BATCAS/BPATCH32). The LPROTSPAN body itself reads x10/x11 and touches only
\ x12/x13, so a sink's addr/len registers survive the call.
: CALL ( n n -- ) {: addr:n len:n :}
   len 10 = if
      addr 11 = if
         12 11 0 ADDI,  11 10 0 ADDI,  10 12 0 ADDI,  \ crossed: swap x10/x11 via x12
      else
         11 10 0 ADDI,  10 addr 0 ADDI,               \ len in x10: move it up before x10 is overwritten
      then
   else
      addr 10 <> if 10 addr 0 ADDI, then              \ addr -> x10 (skip a no-op move)
      len 11 <> if 11 len 0 ADDI, then                \ len  -> x11 (skip a no-op move)
   then
   LPROTSPAN LABEL@ BL, ;
...
\ Reserve the region bytes a DIRECT write is about to place at CP ( x1 = the byte
\ count, clobbered ).
\
\ Every write into the code region goes through LCEMIT, which extends the open
\ bracket's write window itself — except the handful of sites that spill BYTES at
\ CP rather than instructions: the string and counted-string literals, the inline
\ bad-tag message, the defer metadata, and a long definition name. Each of those
\ knows its length before it starts, so it declares it here once instead of
\ paying a window check per byte. With the span already writable this costs a
\ call and no syscall.
\
...
\ reader that takes operands by position - tools/lint/clobber-lint.f is one -
\ reads the wrong register.
\
\ x1 is the argument and does not survive; x30 is framed here, so a site can
\ place this anywhere. It also clobbers the condition flags.
: RESERVE ( -- )
...
\ cp!/ndict! are the FORGET code-emit sinks: cp! redirects code emission to the
...
\ The index lives in x5 and each (arg,extent) pair is loaded straight into the
\ x10/x11 LPROTSPAN ABI, so the shared span guard is reached by a direct
\ branch-with-link instead of an inlined GUARD-SPAN per argument. x5/x6 are the
\ only scratch; x14/x15/x17 stay live for the loop and any second sweep.
: BFFI-GUARD-BOUNDS ( -- )
...
: BPATCH32 ( -- )                \ ( w addr -- ): RW-flip, store, RX, cache-sync —
   A G-POP  B G-POP              \ all inside ENGINE text (a region-resident caller
...
\ A region-resident caller that flipped the region to RW would remove execute
...
\ starts at `addr` as an address literal. The native publisher records the same
\ sites (src/compiler/native/publish.f).
\
...
\ must be a live code entry: DBASE <= xt < CP (x26/x28). Every real hook —
\ ' HOOK, SNAP-CHECK-HOOK, USER-HOOK, ES-VERDICT-HOOK — is a source-loaded
\ colon/TRUSTED: word compiled into [DBASE, CP), so this never rejects a
...
\ code entry, and an identical reinstall is inert. While checking stays armed,
...
\ dispatch byte for byte); a non-zero argument must be a live code entry,
...
\ OWNER-API-PRI-WID carries registered engine-helper records such as (PROT-SPAN)
\ and is never raw-searchable - test/engine-suite.f pins that no checked program
\ can name one. The refusal belongs to the PRIMITIVE and not to
...
\ It extends an open code range when CP leaves it and remains register-transparent
\ to its callers.
...
\ Compile brackets track dictionary-record and code ranges independently. LSPAN
\ widens whichever range intersects its argument, LOPEN declares code beginning
\ at CP, LCLOSE restores both ranges to RX, and LGROW extends an open code range.
\ Exact recorded endpoints ensure the close always covers every opened page.
...
: EMIT-PROT-SPAN-DECL ( -- )
   ...
   1 2 CMP,  C-LS rhi BCOND,
      1 2 0 ADDI,
   rhi LBL,
   1 0 CMP,  C-LS rskip BCOND,
      PAGE-OUT,
      PROT:RLO PROT:RHI rskip BAND-WIDEN,
   rskip LBL,
   \ code band [DBASE+DICT-SIZE, DBASE+REGION)
   0 3 0 ADDI,  1 4 0 ADDI,
   2 DICT-SIZE LIT64,  2 DBASE 2 ADD,
...
: EMIT-PROT ( -- )
   ...
   \ compile brackets record their bands in PROT:WLO/WINDOW and RLO/RHI
