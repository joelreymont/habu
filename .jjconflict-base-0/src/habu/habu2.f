\ habu2.f — engine-builder part 2: the outer interpreter, native-compiler
\ dispatch, definers, persistence, and ENGINE-EMIT:FORTH. Needs habu1.f (part 1).
\ EMIT-MAIN is split into
...
\ fixed x16 MOVZ/MOVK sequence below and materialize into
...
\ dedicated DATA-address emitters, which name the relocation kind at the site.

\ Scalar push for C-CONSTANT. The fixed x16 chain is deliberately not
\ registered as an address literal; relocation reads the structural address map.
: C-LIT ( -- )
   6 11 0 ADDI,  5 $FFFF MOVZ,
   7 6 5 AND,    7 7 5 LSLI,  8 $D2800010 LIT64,  9 8 7 ORR,  LCEMIT LABEL@ BL,
   7 6 16 LSRI,  7 7 5 AND,   7 7 5 LSLI,  8 $F2A00010 LIT64,  9 8 7 ORR,  LCEMIT LABEL@ BL,
   7 6 32 LSRI,  7 7 5 AND,   7 7 5 LSLI,  8 $F2C00010 LIT64,  9 8 7 ORR,  LCEMIT LABEL@ BL,
   7 6 48 LSRI,  7 7 5 AND,   7 7 5 LSLI,  8 $F2E00010 LIT64,  9 8 7 ORR,  LCEMIT LABEL@ BL,
...
\ DATA-address emitters are defined beside the relocation labels below and record
\ every emitted chain through SNAP-RELOC:MARK-SITE.
...
\ DATA address chains are registered structurally at emission. Capture can then
\ relocate them without inferring pointer identity from instruction bytes.
...
: C-DATA-ADDR-RAW ( -- )
   SNAP-RELOC:MARK-SITE
   C-ADDR-RAW ;
$D63F0200 constant DEFER-BLR-X16
...
\ ---- token comparison -------------------------------------------------------
\ LKWCMP compares the current token with one baked keyword, case-insensitively.
: EMIT-KW-COMPARE ( -- )
   LBL LBL LBL LBL {: no:label yes:label scan:label raw:label :}
   LKWCMP LABEL@ LBL,
      2 DATA TKL-CELL LDR,  2 1 CMP,  C-NE no BCOND,
      2 0 MOVZ,  3 $20 MOVZ,
      scan LBL,
         2 1 CMP,  C-GE yes BCOND,
         4 DATA TKA-CELL LDR,  4 4 2 ADD,  4 4 0 LDRB,
         4 $41 CMPI,  C-LT raw BCOND,  4 $5A CMPI,  C-GT raw BCOND,  4 4 3 ORR,
         raw LBL,
         5 0 2 ADD,  5 5 0 LDRB,
         4 5 CMP,  C-NE no BCOND,
         2 2 1 ADDI,  scan B,
      yes LBL,  0 1 MOVZ,  RET,
      no LBL,  0 0 MOVZ,  RET, ;
...
variable LDEFHINT
;package
variable LCHKDEFER  variable LSIGPTRA  variable LSIGA  variable LRECWPUB  variable LRECMIQ
...
;package
\ ADT lowering keywords handled by the native compiler.
variable LKWCONSTRUCT  variable LKWMATCH
...
create RESTAB-BUF
  ...
  10 c, $63 c, $68 c, $65 c, $63 c, $6B c, $2D c, \ "check-hook"
         $68 c, $6F c, $6F c, $6B c,
...
: EMIT ( -- )
   ...
   LKWLBRACE LABEL@ LBL, LBRACE-KW 2 BYTES,
   ...
   LKWCONSTRUCT LABEL@ LBL, s" construct" BYTES,  LKWMATCH LABEL@ LBL, s" match" BYTES,
...
\ ---- top-level keyword handlers emitted at build time -------------------------
...
\ defs are unchanged (first token == whole name).
...
\ (three captured chain words reproduced the bad branch as a SIGSEGV).
...
\ address in the code region, and it lives in the DP heap, which a snapshot
...
: DEF-KW-GUARD-ROW ( ptr a n -- ) {: kwvar:ptr kwlen:n :}
   0 kwvar LABEL@ ADR,  1 kwlen MOVZ,  LKWCMP LABEL@ BL,
   0 LDEFKWFAIL LABEL@ CBNZ, ;
s" def-kw-guard-row" s" ptr a n --" TRUST

TRUSTED: EM-HXT-EXECUTE ( n -- )
   execute ;

: KW-ENTRY ( label ptr a n n -- ) {: lmainlbl:label kwvar:ptr kwlen:n hxt:n :}
   LBL KWSKIP !
   0 kwvar LABEL@ ADR,  1 kwlen MOVZ,  LKWCMP LABEL@ BL,
   0 KWSKIP LABEL@ CBZ,
...
variable LMAIN  variable LEXIT  variable LUNDEF
...
variable LEVLL  variable LEVLP  variable LEVLD  variable LEVLN  variable LEVLR
variable LEVCORRUPT  variable LEVCORRUPTMSG
...
: EM-AOT-RELOC-CODE ( -- )
   ...
   cloop LBL,  22 23 CMP,  C-GE crdone BCOND,
      ...
      \ sites in the publisher's address-site map. It knows the literal is an
...
\ so it records each site in the publisher's address-site map; and like
...
\ Declare one persisted DATA cell as holding a code-region address, so the snapshot
...
\ the store word is the only code that knows a code-region address is going into a
...
\ decoding region bytes. Called by the address emitter before the chain is emitted, so
...
\ that code sits either inside the code region or in the engine's loaded __text.
...
\ (ADDRMAP-OFF, written by the native publisher and by the AOT seed's
...
\ why. OWNER-API-PRI-WID carries registered engine helpers such as (PROT-SPAN),
\ and habu1.f's BSWL states it: the refusal of that wordlist belongs
...
\ decide are each falsified by mutation on the real chain, and test/aot-wid-suite.f
...
\ canonical base (0). Pass 2 folds the live code region [DBASE, DBASE+span) -- the only
...
: EM-SNAPSHOT-RX-FLUSH ( -- )
   ...
   9 DATA PROT:RLO STR,  9 DATA PROT:RHI STR,
...
: CAPTURE-STRING ( -- )
   LMAIN LABEL@ LKWSQ     2 ['] CAPTURE-PLAIN-STRING   KW-ENTRY
   LMAIN LABEL@ LKWCQ     2 ['] CAPTURE-PLAIN-STRING   KW-ENTRY
   LMAIN LABEL@ LKWDOTQ   2 ['] CAPTURE-PLAIN-STRING   KW-ENTRY
   LMAIN LABEL@ LKWESQ    3 ['] CAPTURE-ESCAPED-STRING KW-ENTRY
   LMAIN LABEL@ LKWECQ    3 ['] CAPTURE-ESCAPED-STRING KW-ENTRY
   LMAIN LABEL@ LKWEDOTQ  3 ['] CAPTURE-ESCAPED-STRING KW-ENTRY ;
...
: EM-INTERPRET-DEFINE-KEYWORDS ( -- )
   s" package" KEEP? IF LMAIN LABEL@ LKWPACKAGE 7 ['] C-PACKAGE KW-ENTRY THEN
   s" public" KEEP? IF LMAIN LABEL@ LKWPUBLIC 6 ['] C-PUBLIC KW-ENTRY THEN
   s" private" KEEP? IF LMAIN LABEL@ LKWPRIVATE 7 ['] C-PRIVATE KW-ENTRY THEN
   s" ;package" KEEP? IF LMAIN LABEL@ LKWSEMIPACKAGE 8 ['] C-END-PACKAGE KW-ENTRY THEN
   s" using" KEEP? IF LMAIN LABEL@ LKWUSING 5 ['] C-USING KW-ENTRY THEN
   s" ;using" KEEP? IF LMAIN LABEL@ LKWSEMIUSING 6 ['] C-END-USING KW-ENTRY THEN
   s" export" KEEP? IF LMAIN LABEL@ LKWEXPORT 6 ['] C-EXPORT KW-ENTRY THEN
   s" trusted:" KEEP? IF LMAIN LABEL@ LKWTRUSTED 8 ['] C-TRUSTED KW-ENTRY THEN
   s" cast:" KEEP? IF LMAIN LABEL@ KWDATA:LKWCAST 5 ['] C-CAST KW-ENTRY THEN
   s" defer" KEEP? IF LMAIN LABEL@ LKWDEFER 5 ['] C-DEFER KW-ENTRY THEN
   s" create" KEEP? IF LMAIN LABEL@ LKWCREATE 6 ['] C-CREATE   KW-ENTRY THEN
   s" variable" KEEP? IF LMAIN LABEL@ LKWVAR    8 ['] C-VARIABLE KW-ENTRY THEN
   s" constant" KEEP? IF LMAIN LABEL@ LKWCONST  8 ['] C-CONSTANT KW-ENTRY THEN
   s" '" KEEP? IF LMAIN LABEL@ LKWTICK   1 ['] C-TICK     KW-ENTRY THEN
   s" char" KEEP? IF LMAIN LABEL@ LKWCHAR   4 ['] C-CHAR     KW-ENTRY THEN
   s" immediate" KEEP? IF LMAIN LABEL@ LKWIMM    9 ['] C-IMMEDIATE KW-ENTRY THEN ;
s" em-interpret-define-keywords" s" --" TRUST

: EM-INTERPRET-STRING-KEYWORDS ( -- )
   LMAIN LABEL@ LKWSQ     2 ['] C-ISDQ     KW-ENTRY
   LMAIN LABEL@ LKWCQ     2 ['] C-ICQ      KW-ENTRY
   LMAIN LABEL@ LKWDOTQ   2 ['] C-IDOTQ    KW-ENTRY
   LMAIN LABEL@ LKWESQ    3 ['] C-EISDQ    KW-ENTRY
   LMAIN LABEL@ LKWECQ    3 ['] C-EICQ     KW-ENTRY
   LMAIN LABEL@ LKWEDOTQ  3 ['] C-EIDOTQ   KW-ENTRY ;
...
: EMIT-DEF-KW-GUARD ( -- )
   ...
   SP SP 16 SUBI,  30 SP 0 STR,
   LKWIF 2 DEF-KW-GUARD-ROW
   LKWTHEN 4 DEF-KW-GUARD-ROW
   LKWELSE 4 DEF-KW-GUARD-ROW
   LKWBEGIN 5 DEF-KW-GUARD-ROW
   LKWUNTIL 5 DEF-KW-GUARD-ROW
   LKWAGAIN 5 DEF-KW-GUARD-ROW
   LKWWHILE 5 DEF-KW-GUARD-ROW
   LKWREPEAT 6 DEF-KW-GUARD-ROW
   LKWCASE 4 DEF-KW-GUARD-ROW
   LKWOF 2 DEF-KW-GUARD-ROW
   LKWENDOF 5 DEF-KW-GUARD-ROW
   LKWENDCASE 7 DEF-KW-GUARD-ROW
   LKWCONSTRUCT 9 DEF-KW-GUARD-ROW
   LKWMATCH 5 DEF-KW-GUARD-ROW
   LKWSQ 2 DEF-KW-GUARD-ROW
   LKWCQ 2 DEF-KW-GUARD-ROW
   LKWDOTQ 2 DEF-KW-GUARD-ROW
   LKWESQ 3 DEF-KW-GUARD-ROW
   LKWECQ 3 DEF-KW-GUARD-ROW
   LKWEDOTQ 3 DEF-KW-GUARD-ROW
   LKWBTICK 3 DEF-KW-GUARD-ROW
   LKWBCHAR 6 DEF-KW-GUARD-ROW
   LKWDOES 5 DEF-KW-GUARD-ROW
   LKWQUOT 2 DEF-KW-GUARD-ROW
   LKWIS 2 DEF-KW-GUARD-ROW
   LKWSEMIQ 2 DEF-KW-GUARD-ROW
   LKWDO 2 DEF-KW-GUARD-ROW
   LKWLOOP 4 DEF-KW-GUARD-ROW
   LKWI 1 DEF-KW-GUARD-ROW
   LKWTOR 2 DEF-KW-GUARD-ROW
   LKWRFROM 2 DEF-KW-GUARD-ROW
   LKWRFET 2 DEF-KW-GUARD-ROW
   LKWEXIT 4 DEF-KW-GUARD-ROW
   LKWREC 7 DEF-KW-GUARD-ROW
   LKWQDO 3 DEF-KW-GUARD-ROW
   LKWPLOOP 5 DEF-KW-GUARD-ROW
   LKWJ 1 DEF-KW-GUARD-ROW
   LKWLEAVE 5 DEF-KW-GUARD-ROW
   LKWUNLOOP 6 DEF-KW-GUARD-ROW
   LKWLBRACE 2 DEF-KW-GUARD-ROW
   30 SP 0 LDR,  SP SP 16 ADDI,  RET,
...
: EM-RESET-COMPILE-STATE ( -- )
   ...
   9 DATA BODYLEN-CELL STR,  9 DATA PEND-CELL STR,
   9 DATA TSIG-A-CELL STR,   9 DATA TSIG-U-CELL STR,
   9 DATA TCSIG-A-CELL STR,  9 DATA TCSIG-U-CELL STR,
   9 DATA CRSIG-A-CELL STR,  9 DATA CRSIG-U-CELL STR,
   9 DATA DOESB-CELL STR,
...
: EM-COMPILE-UNDEF ( -- )
   ...
   \ an undefined word that aborts a `:`-body at the tty REPL leaves the code
   \ region RW; EM-REPL-RECOVER -> LREAD -> RD-LINE then executes
...
: EMIT-MAIN ( -- )
   LBL LMAIN !  LBL LEXIT !  LBL LUNDEF !  LBL LUNDERFLOW !  LBL LARITY !
   EM-STARTUP
...
: CORE ( -- )
   ...
   LBL PROT:LOPEN !  LBL PROT:LCLOSE !  LBL PROT:LGROW !  LBL PROT:LSPAN !
...
: CONTROL ( -- )
   ...
   LBL LKWTRUSTED !  LBL KWDATA:LKWTRUSTDECL !  LBL KWDATA:LKWTRUSTRAW !  LBL LKWKERNEL !
   ...
   LBL LCHKDEFER !  LBL LRESTAB !  LBL LRECWPUB !  LBL LRECMIQ !  LBL NCOMP-EMIT:LWORD !  LBL NCOMP-EMIT:LUNSET !  LBL NCOMP-EMIT:LNEUTRAL !  LBL NCOMP-EMIT:LENTRY !
   ...
   LBL LKWCONSTRUCT !  LBL LKWMATCH ! ;

: RUNTIME ( -- )
   LBL LCREATE !  LBL LDOESPATCH !
...
: EMIT-DICTIONARY-SECTIONS ( -- )
   EMIT-CREATE
   DOESPATCH:EMIT
   EMIT-KW-COMPARE  EMIT-ESC-DECODE  EMIT-ESC-SCAN  EMIT-ESC-COPY
