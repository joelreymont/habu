...
| linux-spawn-fail | `n --` | Linux child-side spawn failure reporter: emits raw `write` to the exec-error pipe and exits the child without returning to Forth. | `lib/process-test.f`, `test/run.f` | src/habu/habu1.f:135 | 2026-06-24 |
| linux-dup2-fd | `n n n --` | Linux child-side raw syscall emitter for conditional `dup2` with exec-error-pipe reporting; label/register code and direct process exit path are not inferable as a Forth data transform. | `lib/process-test.f`, `test/run.f` | src/habu/habu1.f:147 | 2026-06-24 |
| linux-chdir-fd | `n n --` | Linux child-side raw syscall emitter for optional `chdir` with exec-error-pipe reporting; label/register code and direct process exit path are not inferable as a Forth data transform. | `lib/process-cwd-test.f`, `test/run.f` | src/habu/habu1.f:158 | 2026-06-24 |
...
| spawn-dup2-action | `n n --` | Build-side helper that emits one raw XNU `PSFA_DUP2` file-action record append; label/register code is not inferable as a Forth data transform. | `tools/spawn-emitter-test.f`, `test/proc-pty.f`, `test/engine-suite.f` | src/habu/habu1.f:488 | 2026-06-25 |
| spawn-chdir-action | `n n --` | Build-side helper that emits one raw XNU `PSFA_CHDIR` file-action record append, bounded NUL-path copy, and branch to a caller failure label; label/register code is not inferable as a Forth data transform. | `tools/spawn-emitter-test.f`, `lib/process-cwd-test.f`, `test/run.f` | src/habu/habu1.f:519 | 2026-06-25 |
...
| spawn-darwin-actions-reset | `n --` | Build-side helper that initializes the XNU file-action blob header at x13 for the requested action capacity and zero used-count. | `tools/spawn-emitter-test.f`, `lib/process-test.f`, `lib/process-cwd-test.f`, `test/run.f` | src/habu/habu1.f:546 | 2026-06-25 |
...
| spawn-darwin-nullable-adesc | `n --` | Build-side helper that emits the XNU empty-action rule: pass null descriptor when no dup2 actions were appended. | `tools/spawn-emitter-test.f`, `lib/process-test.f`, `test/run.f` | src/habu/habu1.f:579 | 2026-06-25 |
...
| spawn-darwin-pid-path | `n --` | Build-side helper that emits common `posix_spawn` pid-out and path register setup. | `tools/spawn-emitter-test.f`, `lib/process-test.f`, `test/run.f` | src/habu/habu1.f:589 | 2026-06-25 |
| spawn-darwin-argv-envp | `n n --` | Build-side helper that emits common argv/envp register setup when both vectors are runtime input registers. | `tools/spawn-emitter-test.f`, `lib/process-env-test.f`, `test/run.f` | src/habu/habu1.f:595 | 2026-06-25 |
| spawn-darwin-default-argv-envp | `n --` | Build-side helper that emits the default argv/envp runtime stack vectors for path-only spawn. | `tools/spawn-emitter-test.f`, `lib/process-test.f`, `test/run.f` | src/habu/habu1.f:603 | 2026-06-25 |
...
| spawn-darwin-argv-default-envp | `n --` | Build-side helper that emits `posix_spawn` argv input plus default empty envp argument registers. | `tools/spawn-emitter-test.f`, `lib/process-argv-test.f`, `test/run.f` | src/habu/habu1.f:619 | 2026-06-25 |
| spawn-darwin-finish | `n n --` | Build-side helper that emits shared Darwin `posix_spawn` syscall, carry/error handling, pid load, failure join, and data-stack push. | `tools/spawn-emitter-test.f`, `lib/process-test.f`, `lib/process-cwd-test.f`, `test/run.f` | src/habu/habu1.f:630 | 2026-06-25 |
