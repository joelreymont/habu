...
| MBUF | `-- ptr u8` | Target image-builder output buffer; checked drivers write the finalized executable bytes through this audited byte span. | `test/run.f`, `tools/build-fixpoint-test.f` | src/os/image-bytes.f:20 | 2026-06-26 |
...
| MP@ | `-- ptr u8` | Reads the shared image output cursor stored in a raw variable. | `test/run.f` | src/os/image-bytes.f:23 | 2026-06-26 |
...
| SNAP-CHECK-HOOK | `ptr u8 n -- n` | Snapshot image installs the fail-closed checker hook into the emitted image; recursive `CHECK!` hook bodies are trusted boundaries. | `test/run.f`, `test/gate-debug.f` | src/habu/snap.f:67 | 2026-06-26 |
| SNAP-INSTALL-HOOK | `--` | Snapshot image mutates the checker hook cell only through a named trusted installer. | `test/run.f`, `test/gate-debug.f` | src/habu/snap.f:69 | 2026-06-26 |
