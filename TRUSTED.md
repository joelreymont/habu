...
| MINT-GB-PROOF | `-- gb-proof` | Private GPT2BIND mint: the proof token sealing an authenticated `GPT2BIND:layerid`, minted only inside the sole constructor `LAYER` after the index range check and the cfgkey capture, so a raw n cannot forge a layer identity and MDLCFG's proof cannot substitute (rev-4 identity design; the `MINT-CFG-PROOF` shape). | `maki/infer/gpt2-roles-test.f` | maki/infer/gpt2-roles.f | 2026-07-26 | prim-axiom | habu-epic-gb10-uma-391d12e8 |
