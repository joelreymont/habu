...
| N>HANDLE | `n -- process-pty-handle` | Private PROCESS-PTY linear-token mint: COMMIT converts one validated consumed reservation into one live handle; successful read-only handle operations remint the same consumed authority. Confined by refine-lint; no public raw mint exists. Retire when TVK-RAW (`habu-nominal-storage-raw-a3430ef2`) lands. | `lib/process-pty-io.f`, `lib/process-pty-handle-test.f`, `lib/process-pty-io-test.f` | lib/process-pty-handle.f | 2026-07-16 |
| HANDLE>N | `process-pty-handle -- n` | Private PROCESS-PTY proof erasure: consumes the linear handle before slot, generation, live-state, and creating-pid validation; no public raw projection exists. Retire when TVK-RAW (`habu-nominal-storage-raw-a3430ef2`) lands. | `lib/process-pty-handle-test.f`, `lib/process-pty-io-test.f` | lib/process-pty-handle.f | 2026-07-16 |
| N>RESERVATION | `n -- process-pty-reservation` | Private PROCESS-PTY linear-token mint: RESERVE creates exactly one reservation from a newly activated generation; tests remint only copied raw identities to prove invalid-state rejection. Confined by refine-lint; no public raw mint exists. Retire when TVK-RAW (`habu-nominal-storage-raw-a3430ef2`) lands. | `lib/process-pty-handle-test.f` | lib/process-pty-handle.f | 2026-07-16 |
| RESERVATION>N | `process-pty-reservation -- n` | Private PROCESS-PTY proof erasure: consumes the linear reservation before COMMIT or CANCEL validates owner, generation, activity, and not-live state. Retire when TVK-RAW (`habu-nominal-storage-raw-a3430ef2`) lands. | `lib/process-pty-handle-test.f` | lib/process-pty-handle.f | 2026-07-16 |
| N>TEARDOWN | `n -- process-pty-teardown` | Private PROCESS-PTY linear-token mint: TAKE converts one validated consumed live handle into the sole draining authority; tests remint copied raw identities only to prove exact-once rejection. Confined by refine-lint; no public raw mint exists. Retire when TVK-RAW (`habu-nominal-storage-raw-a3430ef2`) lands. | `lib/process-pty-handle-test.f` | lib/process-pty-handle.f | 2026-07-16 |
| TEARDOWN>N | `process-pty-teardown -- n` | Private PROCESS-PTY proof erasure: consumes teardown authority before drain-state, generation, and owner validation; only TEARDOWN-DONE releases the slot. Retire when TVK-RAW (`habu-nominal-storage-raw-a3430ef2`) lands. | `lib/process-pty-handle-test.f` | lib/process-pty-handle.f | 2026-07-16 |
...
lib/process-pty-handle.f:N>HANDLE prim-axiom habu-epic-type-system-b88c9ecc
lib/process-pty-handle.f:HANDLE>N prim-axiom habu-epic-type-system-b88c9ecc
lib/process-pty-handle.f:N>RESERVATION prim-axiom habu-epic-type-system-b88c9ecc
lib/process-pty-handle.f:RESERVATION>N prim-axiom habu-epic-type-system-b88c9ecc
lib/process-pty-handle.f:N>TEARDOWN prim-axiom habu-epic-type-system-b88c9ecc
lib/process-pty-handle.f:TEARDOWN>N prim-axiom habu-epic-type-system-b88c9ecc
