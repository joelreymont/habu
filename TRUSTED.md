...
| BUMP-ADDR | `-- n` | Code-generator comparison boundary: the address of the corpus's own `create`d cell as the number a code generator has to materialise. The checker gives a created word the type `ptr a` so that a pointer cannot be done arithmetic on by accident, and there is no checked way to name the number that pointer is - which is exactly what a compiler needs of a data word. Retired by dot habu-resolve-a-data-a1c8067f, when the chain asks the engine what a data word is instead of the harness stating it. | `tools/codegen-compare-test.f`, `tools/codegen-compare.f` | tools/codegen-compare-corpus.f | 2026-08-01 | test-metaprog | habu-resolve-a-data-a1c8067f |
| TV-ADDR | `-- n` | Code-generator comparison boundary: the address of the second corpus's own `create`d type-variable binding table as the number a code generator has to materialise. The same boundary as `BUMP-ADDR` above, for the same reason - the checker gives a created word the type `ptr a` so that a pointer cannot be done arithmetic on by accident, and there is no checked way to name the number that pointer is, which is exactly what a compiler needs of a data word. Retired by dot habu-resolve-a-data-a1c8067f, when the chain asks the engine what a data word is instead of the harness stating it. | `tools/codegen-compare-test.f`, `tools/codegen-compare.f` | tools/codegen-compare-corpus2.f | 2026-08-02 | test-metaprog | habu-resolve-a-data-a1c8067f |
...
| TAKE-BUFFER | `WSTORE:buffer -- ptr u8` | Consumes a buffer owner and yields the allocation for disposal; the single crossing that ends a buffer's life, used only by DISPOSE's buffer free. Symmetric with MINT-BUFFER: the token IS the `ptr u8` allocation it was minted from, so the consume gives the same bytes back and MEM:RELEASE-BYTES takes them directly. Disposal reads the record's CELLS from BUF>REC before consuming. Package-private to WSTORE. | `maki/infer/weight-store-test.f` | maki/infer/weight-store.f | 2026-07-26 | stdlib-boundary | habu-epic-gb10-uma-391d12e8 |
...
| SND-DEAD-HEAP-END | `-- n` | Reads the first DP-heap offset the live engine generation owns, so the writer can zero the heap the refresh prelude abandoned. The anchor IMK-NDICT0 is the first variable of the first prefix source file and the whole prefix loads inside the prelude's check-off window, so it carries no charted effect and only a trusted boundary can name it - the same reason src/habu/snap.f reaches CHECKER-SNAPSHOT-PREPARE through one. | `test/run.f snap`, `tools/build-fixpoint-test.f` | src/habu/snap-lib.f | 2026-07-30 | builder-emit | habu-fix-persisted-dangling-a520f7b4 |
| SND-XT-CELL@ | `n -- n` | Reads one cell of the snapshot data scratch copy by DATA offset, so the writer can canonicalise the cells that were declared to hold a JIT-region address. | `test/run.f snap`, `test/snapshot-xt-cell-decl.f` | src/habu/snap-lib.f | 2026-07-29 | builder-emit | habu-relocate-persisted-defer-7aa681c4 |
| SND-XT-CELL! | `n n --` | Writes one canonicalised region address back into the snapshot data scratch copy by DATA offset. | `test/run.f snap`, `test/snapshot-xt-cell-decl.f` | src/habu/snap-lib.f | 2026-07-29 | builder-emit | habu-relocate-persisted-defer-7aa681c4 |
