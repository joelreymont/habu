...
| DEV-FAM-PROVISIONAL? | `n -- bool` | Type-family owner seam: asks the temporary protected owner whether a family lies in the live top rollback frame's provisional range before declaration-event state can bind it. | `test/decl-event-suite.f`, `test/run.f` | src/core/decl-event.f | 2026-07-24 | stdlib-boundary | habu-type-dsl-unify-b65d46c1 |
| DEV-FAM-RESET-ALLOWED? | `-- bool` | Type-family owner seam: asks the temporary protected owner whether no checker rollback frame is live before declaration-event RESET can mutate any state. | `test/decl-event-suite.f`, `test/run.f` | src/core/decl-event.f | 2026-07-24 | stdlib-boundary | habu-type-dsl-unify-b65d46c1 |
