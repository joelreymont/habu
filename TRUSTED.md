...
| linux-stat-fix | `n --` | Linux stat syscall layout shim copies the kernel `mode` and `size` fields into the engine's portable `stat64` offsets; raw field writes are outside checker inference. | `lib/fs-test.f`, `test/run.f` | src/habu/habu1.f:1134 | 2026-06-27 |
| emit-prims | `--` | Emits the engine's whole primitive table as raw ARM64. | `test/run.f` | src/habu/habu1.f:1361 | 2026-06-27 |
| emit-fp-prims | `--` | Emits the floating-point prim table as raw asm via FPRIM-L. | `test/run.f` | src/habu/habu1.f:1435 | 2026-06-27 |
...
| c-bp-scan | `label label label label --` | Breakpoint table scan emits branches to caller-provided labels and leaves hit-slot state in generated registers. | `test/proc-pty.f`, `test/gate-debug.f`, `test/run.f` | src/habu/habu2.f:223 | 2026-06-27 |
| c-bp-stack-dump | `label label --` | Breakpoint stack dump emits a caller-labelled loop over raw data-stack cells. | `test/proc-pty.f`, `test/gate-debug.f`, `test/run.f` | src/habu/habu2.f:232 | 2026-06-27 |
| c-bp-watch-dump | `label label --` | Breakpoint watch dump emits a caller-labelled loop over raw watched cells. | `test/proc-pty.f`, `test/gate-debug.f`, `test/run.f` | src/habu/habu2.f:243 | 2026-06-27 |
...
| c-local-ref | `label label --` | Compile-mode local-reference emitter: branches to the caller's not-local continuation or emits local loads, and rejects quotation-local captures with raw exit code 75. | `test/engine-suite.f`, `test/run.f` | src/habu/habu2.f:1528 | 2026-06-27 |
...
| em-interpret-colon | `label --` | Emits interpreter-mode colon-definition setup and jumps to the caller-provided not-colon label on non-definitions. | `test/run.f` | src/habu/habu2.f:1791 | 2026-06-27 |
...
| em-interpret-number | `label --` | Emits interpreter-mode number parsing and branches to the caller's not-number label on failure. | `tools/compiler-dispatch-test.f`, `test/run.f` | src/habu/habu2.f:1812 | 2026-06-27 |
...
| em-compile-semi | `label --` | Emits semicolon close handling and binds the caller-provided not-semi continuation label. | `test/run.f` | src/habu/habu2.f:1906 | 2026-06-27 |
...
| vop-entry | `label ptr a n n n --` | JIT binop case: fold-vs-emit split with two indirect xts and raw asm, returning to the main-loop label. | `test/run.f` | src/habu/jit.f:373 | 2026-06-27 |
| vopi-entry | `label ptr a n n n n --` | JIT binop-immediate case: fold/register/immediate split with three indirect xts and raw asm, returning to the main-loop label. | `test/run.f` | src/habu/jit.f:372 | 2026-06-27 |
| vshuf-entry | `label ptr a n n n --` | JIT reg-aware stack-shuffle case (dup/over/swap/drop/nip as register moves); `sxt` runs through `JIT-XT-EXECUTE`. | `test/run.f` | src/habu/jit.f:773 | 2026-06-27 |
| vun-entry | `label ptr a n n n --` | JIT unary-op case: con-fold vs in-place reg op via `foldxt`/`emitxt` and raw asm, returning to the main-loop label. | `test/run.f` | src/habu/jit.f:806 | 2026-06-27 |
...
| ROW | `-- rowidx<e>` | PTX tile-DSL M6: blockIdx.x as a row index proven < R under the launch ABI; a primitive the checker cannot infer. | `lib/ptx-collective-test.f` | lib/ptx-collective.f:25 | 2026-06-27 |
| ROW-SPAN | `matrix<space-global,t,e,k> rowidx<e> -- span<space-global,t,k>` | PTX tile-DSL M6: row r of a dense matrix as a span over its columns (base r*C, checked); a primitive the checker cannot infer. | `lib/ptx-collective-test.f` | lib/ptx-collective.f:28 | 2026-06-27 |
| ROW-CTX | `span<space-global,t,k> -- rowctx<b,k,m>` | PTX tile-DSL M6: one-block-per-row context (lane = tid, mask = tid < N); a primitive the checker cannot infer. | `lib/ptx-collective-test.f` | lib/ptx-collective.f:31 | 2026-06-27 |
| ROW-LOAD | `span<space-global,t,k> rowctx<b,k,m> -- tile<t,b,m>` | PTX tile-DSL M6: masked row-local load of a tile; lowers to PTX ld.global the checker cannot infer. | `lib/ptx-collective-test.f` | lib/ptx-collective.f:34 | 2026-06-27 |
| ROW-STORE | `tile<t,b,m> span<space-global,t,k> rowctx<b,k,m> --` | PTX tile-DSL M6: masked row-local store (active lanes only); lowers to PTX st.global the checker cannot infer. | `lib/ptx-collective-test.f` | lib/ptx-collective.f:37 | 2026-06-27 |
| BLOCK-MAX | `tile<f32,b,m> -- uniform<f32>` | PTX tile-DSL M6: mask-aware all-block max reduction (inactive lanes seed -inf); lowers to warp shfl + shared staging the checker cannot infer. | `lib/ptx-collective-test.f` | lib/ptx-collective.f:40 | 2026-06-27 |
| BLOCK-SUM | `tile<f32,b,m> -- uniform<f32>` | PTX tile-DSL M6: mask-aware all-block sum reduction (inactive lanes seed 0); lowers to warp shfl + shared staging the checker cannot infer. | `lib/ptx-collective-test.f` | lib/ptx-collective.f:43 | 2026-06-27 |
| B- | `tile<t,b,m> uniform<t> -- tile<t,b,m>` | PTX tile-DSL M6: tile minus a broadcast uniform scalar; lowers to PTX sub.rn the checker cannot infer. | `lib/ptx-collective-test.f` | lib/ptx-collective.f:46 | 2026-06-27 |
| B/ | `tile<t,b,m> uniform<t> -- tile<t,b,m>` | PTX tile-DSL M6: tile divided by a broadcast uniform scalar; lowers to PTX div.rn the checker cannot infer. | `lib/ptx-collective-test.f` | lib/ptx-collective.f:49 | 2026-06-27 |
| EXP. | `tile<f32,b,m> -- tile<f32,b,m>` | PTX tile-DSL M6: elementwise exp (ex2.approx.ftz(x*log2e), tolerance acceptance-gated); a primitive the checker cannot infer. | `lib/ptx-collective-test.f` | lib/ptx-collective.f:52 | 2026-06-27 |
...
| BROADCAST | `uniform<f32> -- tile<f32,b,m>` | PTX tile-DSL AD: fills a tile from a uniform (named form of the broadcast in B-/B/); the mutual adjoint of BLOCK-SUM for reverse-mode AD; a primitive the checker cannot infer. | `lib/ptx-autograd-test.f` | lib/ptx-collective.f:58 | 2026-06-27 |
| BLOCK-MAX-SELECT | `uniform<f32> tile<f32,b,m> uniform<f32> -- tile<f32,b,m>` | PTX tile-DSL AD: the BLOCK-MAX adjoint - a masked scatter routing the cotangent to the arg-max lane (deterministic lowest-lane tie-break), 0 elsewhere; a primitive the checker cannot infer. | `lib/ptx-collective-test.f` | lib/ptx-collective.f:66 | 2026-06-27 |
