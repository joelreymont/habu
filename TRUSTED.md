...
| N>HANDLE | `n -- process-pty-handle` | Private PROCESS-PTY linear-token mint: converts a packed slot/generation only after the registry has reserved or validated that identity; no public raw mint exists. Retire when TVK-RAW (`habu-nominal-storage-raw-a3430ef2`) lands. | `lib/process-pty-handle-test.f` | lib/process-pty-handle.f | 2026-07-16 |
| HANDLE>N | `process-pty-handle -- n` | Private PROCESS-PTY proof erasure: consumes the linear handle so OPEN can validate its slot, generation, active bit, and creating pid before registry access; no public raw projection exists. Retire when TVK-RAW (`habu-nominal-storage-raw-a3430ef2`) lands. | `lib/process-pty-handle-test.f` | lib/process-pty-handle.f | 2026-07-16 |
