...
| CLEAR-MEAS | `R n -- n` | Property-test oracle drains the arbitrary residual data-stack tail left by a generated program while preserving the measured count; this is exactly the value-agnostic depth boundary. | `test/prop-test.f`, `test/gate-debug.f`, `test/run.f` | test/prop-test.f:15 | 2026-06-22 |
| ERR@ | `-- n` | Reads the engine `evaluate` recovery cell from the raw DATA header so the in-process property oracle can distinguish clean execution from recovered traps. | `test/prop-test.f`, `test/gate-debug.f`, `test/run.f` | test/prop-test.f:20 | 2026-06-22 |
| CHK | `ptr u8 n --` | Temporarily installs the property check hook and evaluates generated source; recursive checker invocation and hook restoration are dynamic interpreter state. | `test/prop-test.f`, `test/gate-debug.f`, `test/run.f` | test/prop-test.f:146 | 2026-06-22 |
| RUN-MEAS | `n n --` | Builds and evaluates the generated measurement program, records `LAST-MEAS` or `LAST-TRAP`, and normalizes dynamic evaluation paths that the checker cannot express directly. | `test/prop-test.f`, `test/gate-debug.f`, `test/run.f` | test/prop-test.f:148 | 2026-06-22 |
| CONFIRM-FR? | `-- bool` | False-reject oracle deliberately compiles one generated program with checking disabled, restores the hook, and measures runtime behavior to prove a rejection was real incompleteness. | `test/prop-test.f`, `test/gate-debug.f`, `test/run.f` | test/prop-test.f:226 | 2026-06-22 |
...
| VEC-ALLOC-CELLS | `count -- ptr a` | Refines an OS-backed byte allocation whose size was computed from a checked cell count into typed cell-vector storage. | `lib/vector-test.f`, `test/run.f` | lib/vector.f:30 | 2026-06-22 |
| VEC-DATA@ | `ptr a -- ptr a` | Reads the vector header's typed cell-storage pointer from a generic cell field; tracked by dot `caf-0452ed33fe8672aa` until the checker can express typed pointer fields. | `lib/vector-test.f`, `test/run.f` | lib/vector.f:33 | 2026-06-22 |
| VEC-DATA! | `ptr a ptr a --` | Stores a typed cell-storage pointer into the vector header's generic cell field; tracked by dot `caf-0452ed33fe8672aa` until the checker can express typed pointer fields. | `lib/vector-test.f`, `test/run.f` | lib/vector.f:36 | 2026-06-22 |
...
| JSON-A@ | `-- ptr u8` | Reads the JSON parser source pointer stored in a raw variable; callers need a typed byte pointer after parser setup. | `tools/json-test.f`, `tools/json-only-test.f`, `test/run.f` | tools/json.f:110 | 2026-06-22 |
| JSON-GKA@ | `-- ptr u8` | Reads the JSON object-key scratch pointer stored in a raw variable; preserves byte-pointer type for key comparisons. | `tools/json-test.f`, `tools/json-only-test.f`, `test/run.f` | tools/json.f:112 | 2026-06-22 |
| JSONL-A@ | `-- ptr u8` | Reads the JSONL input pointer stored in a raw variable; the streaming cursor keeps pointer state outside the checked stack. | `tools/json-test.f`, `tools/json-only-test.f`, `test/run.f` | tools/json.f:114 | 2026-06-22 |
| JSONL-LA@ | `-- ptr u8` | Reads the current JSONL line pointer stored in a raw variable; preserves byte-pointer type for per-line parsing. | `tools/json-test.f`, `tools/json-only-test.f`, `test/run.f` | tools/json.f:116 | 2026-06-22 |
| JSON-STR-BUF | `-- ptr u8` | Reads the growable JSON parser string arena pointer stored in a raw variable after boot storage or OS-backed growth. | `tools/json-test.f`, `tools/json-only-test.f`, `test/run.f` | tools/json.f:118 | 2026-06-22 |
| JSON-ALLOC-STR-PTR | `n -- ptr u8` | Refines an anonymous `mmap` result into a typed byte pointer for growable JSON parser string storage after size and failure checks. | `tools/json-test.f`, `tools/json-only-test.f`, `test/run.f` | tools/json.f:183 | 2026-06-22 |
| JSONL-CATCH-LINE | `-- i64` | Recovery boundary around one JSONL parse line; the checker cannot model `catch` over dynamic parser state yet. | `tools/json-test.f`, `tools/json-only-test.f`, `bench/llm/report-test.f`, `test/run.f` | tools/json.f:799 | 2026-06-22 |
| LV-RESULT-BUF | `-- ptr u8` | Reads the validator's growable result-line buffer pointer stored in a raw variable after OS-backed allocation. | `bench/llm/validate-results-test.f`, `test/run.f` | bench/llm/validate-results.f:234 | 2026-06-22 |
| LV-CATCH-PARSE | `ptr u8 n -- n` | Validator row parser recovery boundary: stores successful JSON roots and converts parser throws into row-scoped diagnostics; checked quotations cannot model this dynamic `catch` yet. | `bench/llm/validate-results-test.f`, `test/run.f` | bench/llm/validate-results.f:356 | 2026-06-22 |
...
| SS-DISP$ | `-- ptr u8 n` | Reads the stale-status lint display path pointer and length stored in raw variables for findings. | `tools/stale-status-lint-test.f`, `test/run.f` | tools/stale-status-lint.f:54 | 2026-06-22 |
| LEX-A@ | `-- ptr u8` | Reads the source-lexer input pointer stored in a raw variable; the lexer tables are checked vectors, but the checker still cannot reload a typed byte pointer from a generic variable. Tracked by `caf-0452ed33fe8672aa`. | `tools/lint/text-foundation-test.f`, `bench/llm/drive-forth-test.f`, `test/run.f` | tools/lint/source-lex.f:22 | 2026-06-22 |
| BF-TMP-OVERRIDE$ | `-- ptr u8 n` | Reads the build-fixpoint temp override pointer and length stored in raw variables. | `tools/build-fixpoint-test.f`, `tools/hb-build-test.f`, `test/run.f` | tools/build-fixpoint.f:43 | 2026-06-21 |
| BF-TMP! | `ptr u8 n --` | Stores the build-fixpoint temp override pointer and length in raw variables for child build paths. | `tools/build-fixpoint-test.f`, `tools/hb-build-test.f`, `test/run.f` | tools/build-fixpoint.f:46 | 2026-06-21 |
...
| DS-SET$ | `ptr u8 n ptr n ptr n --` | Stores driver string pointer and length cells through raw address parameters; checker lacks typed field references. | `bench/llm/drive-stdlib-test.f`, `bench/llm/drive-forth-test.f`, `test/run.f` | bench/llm/drive-stdlib-lib.f:123 | 2026-06-21 |
| DS-NAME$ | `-- ptr u8 n` | Reads the driver task-name pointer and length stored in raw variables. | `bench/llm/drive-stdlib-test.f`, `bench/llm/drive-forth-test.f`, `test/run.f` | bench/llm/drive-stdlib-lib.f:145 | 2026-06-21 |
| DS-SIG$ | `-- ptr u8 n` | Reads the driver signature pointer and length stored in raw variables. | `bench/llm/drive-stdlib-test.f`, `bench/llm/drive-forth-test.f`, `test/run.f` | bench/llm/drive-stdlib-lib.f:148 | 2026-06-21 |
| DS-CATEGORY$ | `-- ptr u8 n` | Reads the driver category pointer and length stored in raw variables. | `bench/llm/drive-stdlib-test.f`, `bench/llm/drive-forth-test.f`, `test/run.f` | bench/llm/drive-stdlib-lib.f:151 | 2026-06-21 |
| DS-TESTS$ | `-- ptr u8 n` | Reads the driver tests-description pointer and length stored in raw variables. | `bench/llm/drive-stdlib-test.f`, `bench/llm/drive-forth-test.f`, `test/run.f` | bench/llm/drive-stdlib-lib.f:154 | 2026-06-21 |
| DS-SPEC$ | `-- ptr u8 n` | Reads the driver task-spec pointer and length stored in raw variables. | `bench/llm/drive-stdlib-test.f`, `bench/llm/drive-forth-test.f`, `test/run.f` | bench/llm/drive-stdlib-lib.f:157 | 2026-06-21 |
| DS-SEED$ | `-- ptr u8 n` | Reads the driver seed pointer and length stored in raw variables. | `bench/llm/drive-stdlib-test.f`, `bench/llm/drive-forth-test.f`, `test/run.f` | bench/llm/drive-stdlib-lib.f:160 | 2026-06-21 |
| DS-LINE! | `ptr u8 n --` | Stores the current manifest row pointer and length in raw variables during driver row scanning. | `bench/llm/drive-stdlib-test.f`, `bench/llm/drive-forth-test.f`, `test/run.f` | bench/llm/drive-stdlib-lib.f:336 | 2026-06-21 |
| DS-LINE$ | `-- ptr u8 n` | Reads the current manifest row pointer and length from raw variables during driver row scanning. | `bench/llm/drive-stdlib-test.f`, `bench/llm/drive-forth-test.f`, `test/run.f` | bench/llm/drive-stdlib-lib.f:340 | 2026-06-21 |
