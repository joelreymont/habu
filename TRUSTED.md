...
| MAP>N | `SAFET:mapping -- n` | Parks the mapped storage path's file-mapping owner in the residency block as one raw cell. A DEFLINEAR carries no fields, so a single-cell owner cannot be stored as a typed value; a store cannot be a record field either, because it is three cells wide and a field names one value. From this crossing until N>MAP the checker cannot see the mapping, so WSTORE's structure enforces single ownership — one park site in HOLD's mapped variant, one recovery site in RESIDENT-DISPOSE, no accessor — and the SAFET:LIVE-OWNERS and SAFET-MAP:LIVE assertions cover every load. Package-private to WSTORE, no public inverse. Retires with the linear-scope combinator habu-checker-linear-scope-6218899c. | `maki/infer/weight-store-test.f` | maki/infer/weight-store.f | 2026-07-26 | prim-axiom | habu-epic-gb10-uma-391d12e8 |
...
| BLOCK>PREPARED | `ptr u8 -- GPT2LOAD:prepared-load` | Gives a loader block its linear prepared-load type in two exact roles: initial construction after GPT2LOAD has filled a new block, and restoration of the same unchanged block after `CHECK-MAPPED` gets an empty result from `SAFET:DETACH-MAPPING`. The value is the block address, but the checker can prove neither the initial construction nor the identity-preserving restoration. Package-private; no public raw constructor. Retires with habu-checker-ptr-lifetime-f59d1e9d. | `maki/infer/gpt2-prepare-test.f` | maki/infer/gpt2-load.f | 2026-07-26 | stdlib-boundary | habu-epic-gb10-uma-391d12e8 |
| VIEW-PREPARED-BLOCK | `GPT2LOAD:prepared-load -- GPT2LOAD:prepared-load ptr n` | Preserves a prepared load while exposing its private block address to GPT2LOAD accessors. Package-private; no caller can obtain the address. Retires with habu-checker-ptr-lifetime-f59d1e9d. | `maki/infer/gpt2-prepare-test.f` | maki/infer/gpt2-load.f | 2026-07-26 | stdlib-boundary | habu-epic-gb10-uma-391d12e8 |
| TAKE-PREPARED-BLOCK | `GPT2LOAD:prepared-load -- ptr n` | Consumes a prepared load and returns its private block for `DISCARD-PREPARED`, `RETURN-TENSOR-INDEX`, or a checked state transition. Package-private. Retires with habu-checker-ptr-lifetime-f59d1e9d. | `maki/infer/gpt2-prepare-test.f` | maki/infer/gpt2-load.f | 2026-07-26 | stdlib-boundary | habu-epic-gb10-uma-391d12e8 |
| BLOCK>BYTE-POINTER | `ptr n -- ptr u8` | Gives GPT2LOAD's validated private block the byte-pointer role required by the memory release and state-conversion paths. Package-private. Retires with habu-checker-ptr-lifetime-f59d1e9d. | `maki/infer/gpt2-prepare-test.f` | maki/infer/gpt2-load.f | 2026-07-26 | stdlib-boundary | habu-epic-gb10-uma-391d12e8 |
| TENSOR-INDEX>CELL | `SAFET:census -- n` | Erases the parsed tensor index only in three GPT2LOAD roles: initial storage in the prepared-load block, writing the restored owner back after `COPY-WEIGHTS`, and the defensive empty `DETACH-MAPPING` result where the restored owner is converted and dropped because the unchanged block cell already contains that representation. The private inverse below restores block-held owners; leak counters expose a stranded owner. Retires with habu-checker-linear-scope-6218899c. | `maki/infer/gpt2-prepare-test.f` | maki/infer/gpt2-load.f | 2026-07-26 | prim-axiom | habu-epic-gb10-uma-391d12e8 |
| CELL>TENSOR-INDEX | `n -- SAFET:census` | Restores the parsed tensor index from the sole block cell. Each private state transition then consumes that owner or writes the same owner representation back before returning; no public accessor exists. Retires with habu-checker-linear-scope-6218899c. | `maki/infer/gpt2-prepare-test.f` | maki/infer/gpt2-load.f | 2026-07-26 | prim-axiom | habu-epic-gb10-uma-391d12e8 |
| TABLE>CELL | `WSTORE:table -- n` | Erases a sealed weight table only in three GPT2LOAD roles: the prepared-load block, the pending-table cell while PREPARE guards block allocation, and the copied-table cell while LOAD-COPIED completes. The private inverse below restores those tables; WSTORE's live counter exposes a stranded table. Retires with habu-checker-linear-scope-6218899c. | `maki/infer/gpt2-prepare-test.f` | maki/infer/gpt2-load.f | 2026-07-26 | prim-axiom | habu-epic-gb10-uma-391d12e8 |
| CELL>TABLE | `n -- WSTORE:table` | Restores the sealed table stored by `TABLE>CELL`. It is package-private and reached only by GPT2LOAD state transitions and exits. Retires with habu-checker-linear-scope-6218899c. | `maki/infer/gpt2-prepare-test.f` | maki/infer/gpt2-load.f | 2026-07-26 | prim-axiom | habu-epic-gb10-uma-391d12e8 |
| BLOCK>MAPPED-READY | `ptr u8 -- GPT2LOAD:mapped-ready` | Converts the prepared-load block into mapped-ready state after `CHECK-MAPPED` has verified the configuration and moved the file mapping into it. Package-private; `CHECK-MAPPED` is the only caller. Retires with habu-checker-ptr-lifetime-f59d1e9d. | `maki/infer/gpt2-mapped-test.f` | maki/infer/gpt2-load.f | 2026-07-26 | stdlib-boundary | habu-epic-gb10-uma-391d12e8 |
| TAKE-MAPPED-BLOCK | `GPT2LOAD:mapped-ready -- ptr n` | Consumes mapped-ready state and returns its private block for `DISCARD-MAPPED` or `LOAD-MAPPED`. Package-private. Retires with habu-checker-ptr-lifetime-f59d1e9d. | `maki/infer/gpt2-mapped-test.f` | maki/infer/gpt2-load.f | 2026-07-26 | stdlib-boundary | habu-epic-gb10-uma-391d12e8 |
| BLOCK>COPY-READY | `ptr u8 -- GPT2LOAD:copy-ready` | Converts the prepared-load block into copy-ready state after `CHECK-COPY` has verified the configuration. The parsed tensor index remains in the block because `LOAD-COPIED` still needs the checkpoint bytes. Package-private. Retires with habu-checker-ptr-lifetime-f59d1e9d. | `maki/infer/gpt2-copy-test.f` | maki/infer/gpt2-load.f | 2026-07-26 | stdlib-boundary | habu-epic-gb10-uma-391d12e8 |
| TAKE-COPY-BLOCK | `GPT2LOAD:copy-ready -- ptr n` | Consumes copy-ready state and returns its private block for `DISCARD-COPY` or `LOAD-COPIED`. Package-private. Retires with habu-checker-ptr-lifetime-f59d1e9d. | `maki/infer/gpt2-copy-test.f` | maki/infer/gpt2-load.f | 2026-07-26 | stdlib-boundary | habu-epic-gb10-uma-391d12e8 |
| MAPPING>CELL | `SAFET:mapping -- n` | Stores the owned checkpoint mapping in the mapped-ready block as one raw cell. GPT2LOAD has one storing path and only the private inverse below; mapping and tensor-owner counters expose a stranded mapping. Retires with habu-checker-linear-scope-6218899c. | `maki/infer/gpt2-mapped-test.f` | maki/infer/gpt2-load.f | 2026-07-26 | prim-axiom | habu-epic-gb10-uma-391d12e8 |
| CELL>MAPPING | `n -- SAFET:mapping` | Restores the mapping stored by `MAPPING>CELL`. It is package-private and reached only by `DISCARD-MAPPED` and `LOAD-MAPPED`, so the same mapping cannot be restored twice. Retires with habu-checker-linear-scope-6218899c. | `maki/infer/gpt2-mapped-test.f` | maki/infer/gpt2-load.f | 2026-07-26 | prim-axiom | habu-epic-gb10-uma-391d12e8 |
| BUFFER>CELL | `WSTORE:buffer -- n` | Stores the owned copy buffer while `LOAD-COPIED` completes later fallible steps. The buffer has one storing path and only the private inverse below; WSTORE's live counter exposes a stranded owner. Retires with habu-checker-linear-scope-6218899c. | `maki/infer/gpt2-copy-test.f` | maki/infer/gpt2-load.f | 2026-07-26 | prim-axiom | habu-epic-gb10-uma-391d12e8 |
| CELL>BUFFER | `n -- WSTORE:buffer` | Restores the buffer stored by `BUFFER>CELL` for cleanup or successful copied-store construction. It is package-private and each path restores it exactly once. Retires with habu-checker-linear-scope-6218899c. | `maki/infer/gpt2-copy-test.f` | maki/infer/gpt2-load.f | 2026-07-26 | prim-axiom | habu-epic-gb10-uma-391d12e8 |
| MAKE-MODEL-PROOF | `-- model-proof` | Creates GPT2LOAD's private model-construction proof. A raw cell cannot replace this value, so code outside GPT2LOAD cannot construct a model from nothing. Existing generated `UNMAKE` access remains tracked by habu-checker-sealed-destructure-d967fc03. | `maki/infer/gpt2-mapped-test.f` | maki/infer/gpt2-load.f | 2026-07-26 | prim-axiom | habu-epic-gb10-uma-391d12e8 |
...
| RUN | `--` | Brackets one offline source-verification pass with the private package-mirror provider, then restores live package authority through both normal return and `catch`. Direct mirror writes remain inert outside this boundary. | `test/cast-negative-suite.f`, `test/decl-replay-verify-source.f`, `test/run.f` | src/habu/verify-source.f | 2026-07-27 | builder-emit | habu-cast-v2-family-741e7bae |
