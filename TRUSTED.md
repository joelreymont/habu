...
| linux-stat-fix | `n --` | Linux stat syscall layout shim copies the kernel `mode` and `size` fields into the engine's portable `stat64` offsets; raw field writes are outside checker inference. | `lib/fs-test.f`, `test/run.f` | src/habu/habu1.f:1049 | 2026-06-24 |
| emit-prims | `--` | Emits the engine's whole primitive table as raw ARM64. | `test/run.f` | src/habu/habu1.f:1273 | 2026-06-24 |
| emit-fp-prims | `--` | Emits the floating-point prim table as raw asm via FPRIM-L. | `test/run.f` | src/habu/habu1.f:1347 | 2026-06-24 |
...
| ASM-CODE | `-- asm` | Target image-builder bridge that snapshots the current assembler code length and produces the checked image-build phase token. | `test/run.f`, `tools/build-fixpoint-test.f`, `test/engine-suite.f` | src/os/linux/elf.f:24 | 2026-06-26 |
| BUILD-IMAGE | `asm -- img` | Target image-builder boundary that consumes the checked assembled-code phase token and produces the executable-image phase token. | `test/run.f`, `tools/build-fixpoint-test.f`, `test/engine-suite.f` | src/os/linux/elf.f:64 | 2026-06-26 |
| BUILD-SNAP-HDR | `n -- snap n` | Target snapshot-header builder; consumes text payload length, produces the snapshot-header phase token, and returns padded executable text size. | `test/run.f`, `tools/build-fixpoint-test.f`, `test/engine-suite.f` | src/os/linux/elf.f:74 | 2026-06-26 |
...
| CODESIG2 | `img -- img` | Target signing pass boundary; Linux is a no-op and macOS mutates the image buffer while preserving the executable-image phase token. | `test/run.f`, `tools/build-fixpoint-test.f`, `test/engine-suite.f` | src/os/linux/sign.f:7 | 2026-06-26 |
...
| ASM-CODE | `-- asm` | Target image-builder bridge that snapshots the current assembler code length and produces the checked image-build phase token. | `test/run.f`, `tools/build-fixpoint-test.f`, `test/engine-suite.f` | src/os/macos/macho.f:24 | 2026-06-26 |
| BUILD-IMAGE | `asm -- img` | Target image-builder boundary that consumes the checked assembled-code phase token and produces the executable-image phase token. | `test/run.f`, `tools/build-fixpoint-test.f`, `test/engine-suite.f` | src/os/macos/macho.f:89 | 2026-06-26 |
| BUILD-SNAP-HDR | `n -- snap n` | Target snapshot-header builder; consumes text payload length, produces the snapshot-header phase token, and returns padded executable text size. | `test/run.f`, `tools/build-fixpoint-test.f`, `test/engine-suite.f` | src/os/macos/macho.f:104 | 2026-06-26 |
...
| CODESIG2 | `img -- img` | Target signing pass boundary; Linux is a no-op and macOS mutates the image buffer while preserving the executable-image phase token. | `test/run.f`, `tools/build-fixpoint-test.f`, `test/engine-suite.f` | src/os/macos/sign2.f:76 | 2026-06-26 |
...
| HB-TARGET-LINUX? | `-- bool` | Target selector is a trusted runtime target fact used by checked multi-platform source lists. | `test/run.f`, `tools/build-fixpoint-test.f` | src/core/roles.f:7 | 2026-06-26 |
| HB-TARGET-MACOS? | `-- bool` | Target selector is a trusted runtime target fact used by checked multi-platform source lists. | `test/run.f`, `tools/build-fixpoint-test.f` | src/core/roles.f:8 | 2026-06-26 |
| HB-TARGET-KNOWN? | `-- bool` | Target selector aggregate is trusted so checked tools reject unsupported targets instead of defaulting. | `test/run.f`, `tools/build-fixpoint-test.f` | src/core/roles.f:9 | 2026-06-26 |
...
| STR-CHECK-REJECTS | `ptr u8 n --` | String negative checker assertion temporarily suppresses diagnostics and calls `CHECK!` to prove typed append/copy helpers reject wrong roles. | `lib/string-test.f`, `test/run.f` | lib/string-test.f:37 | 2026-06-26 |
...
| DRV-WRITE-IMAGE | `img ptr u8 n --` | Driver image write boundary consumes the executable-image phase token and writes the finalized `MBUF`/`MLEN` bytes to the requested output path. | `test/run.f`, `tools/build-fixpoint-test.f`, `test/engine-suite.f` | src/habu/driver-io.f:46 | 2026-06-26 |
| SNAP-STALE | `snap --` | Snapshot builder explicitly invalidates the provisional header phase token before recomputing the final padded snapshot header. | `test/run.f`, `tools/build-fixpoint-test.f` | src/habu/snap.f:32 | 2026-06-26 |
| SNAP-WRITE | `snap --` | Snapshot writer consumes the final snapshot-header phase token before streaming header, text, regions, trailer, and padding to disk. | `test/run.f`, `tools/build-fixpoint-test.f` | src/habu/snap.f:66 | 2026-06-26 |
| SNAP-CHECK-HOOK | `ptr u8 n -- n` | Snapshot image installs the fail-closed checker hook into the emitted image; recursive `CHECK!` hook bodies are trusted boundaries. | `test/run.f`, `test/gate-debug.f` | src/habu/snap.f:76 | 2026-06-26 |
| SNAP-INSTALL-HOOK | `--` | Snapshot image mutates the checker hook cell only through a named trusted installer. | `test/run.f`, `test/gate-debug.f` | src/habu/snap.f:78 | 2026-06-26 |
| SBUF@ | `-- ptr u8` | Reads the stage2 source buffer pointer stored in a raw variable. | `test/run.f` | src/habu/stage2.f:13 | 2026-06-26 |
