...
| N>HANDLE | `n -- process-pty-handle` | Private PROCESS-PTY linear-token mint: COMMIT converts one validated consumed reservation into one live handle; successful read-only handle operations remint the same consumed authority. Confined by refine-lint; no public raw mint exists. Retire when TVK-RAW (`habu-nominal-storage-raw-a3430ef2`) lands. | `lib/process-pty-handle-test.f` | lib/process-pty-handle.f | 2026-07-16 |
| HANDLE>N | `process-pty-handle -- n` | Private PROCESS-PTY proof erasure: consumes the linear handle before slot, generation, live-state, and creating-pid validation; no public raw projection exists. Retire when TVK-RAW (`habu-nominal-storage-raw-a3430ef2`) lands. | `lib/process-pty-handle-test.f` | lib/process-pty-handle.f | 2026-07-16 |
