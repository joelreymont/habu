...
| MINT-MDL-PROOF | `-- mdl-proof` | Mints the bound model's private-mint proof, an arity-0 nominal family whose only constructor is this leaf (the MDLCFG:MINT-CFG-PROOF shape). It is what makes GPT2TX:gpt2-model unforgeable from outside the package: a raw n in the proof slot is a checker reject, so only the commit path can produce a model. Package-private to GPT2TX. Scope of the guarantee, shared with MDLCFG:cfg-proof and maki/typestate.f ART:built: a holder of a real model can UNMAKE and re-MAKE it with the same proof; closing that needs the sealed-destructure capability. | `maki/infer/gpt2-bind-test.f` | maki/infer/gpt2-bind.f | 2026-07-26 | prim-axiom | habu-epic-gb10-uma-391d12e8 |
