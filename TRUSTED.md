...
| JSON-A@ | `-- ptr u8` | Reads the JSON parser source pointer stored in a raw variable; callers need a typed byte pointer after parser setup. | `tools/json-test.f`, `tools/json-only-test.f`, `test/run.f` | tools/json.f:119 | 2026-06-22 |
| JSON-GKA@ | `-- ptr u8` | Reads the JSON object-key scratch pointer stored in a raw variable; preserves byte-pointer type for key comparisons. | `tools/json-test.f`, `tools/json-only-test.f`, `test/run.f` | tools/json.f:121 | 2026-06-22 |
| JSONL-A@ | `-- ptr u8` | Reads the JSONL input pointer stored in a raw variable; the streaming cursor keeps pointer state outside the checked stack. | `tools/json-test.f`, `tools/json-only-test.f`, `test/run.f` | tools/json.f:123 | 2026-06-22 |
| JSONL-LA@ | `-- ptr u8` | Reads the current JSONL line pointer stored in a raw variable; preserves byte-pointer type for per-line parsing. | `tools/json-test.f`, `tools/json-only-test.f`, `test/run.f` | tools/json.f:125 | 2026-06-22 |
| JSON-STR-BUF | `-- ptr u8` | Reads the growable JSON parser string arena pointer stored in a raw variable after boot storage or OS-backed growth. | `tools/json-test.f`, `tools/json-only-test.f`, `test/run.f` | tools/json.f:127 | 2026-06-22 |
