...
| PTR-VARIABLE | `--` | Pointer variables are created through `CREATE`/`DOES>` with a pointer-valued runtime effect that the checker cannot infer from the definer body. | `test/pointer-storage-test.f`, `test/gate-dictionary.f`, `test/run.f` | src/core/pointer-storage-effects.f | 2026-07-13 |
