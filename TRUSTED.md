...
| TTHROWS-RAW | `a n --` | Test assertion boundary around `catch`; the checker has no model for applying an arbitrary execution token and observing its throw code. | `lib/test-test.f`, `test/run.sh` | lib/test.f:61 | 2026-06-18 |
| BUILD-CHECK-RAW | `ptr u8 n -- n` | Build helper boundary around `CHECK!`; the checker cannot certify a source definition by evaluating its own checker recursively. | `lib/build-test.f`, `test/run.sh` | lib/build.f:47 | 2026-06-18 |
