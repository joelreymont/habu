...
| ST-N>PTR | `n -- ptr u8` | Reinterprets an `mmap` numeric address as a typed byte pointer for zero-copy header/tensor reads (safetensors loader); the checker cannot express this syscall-result refinement. | `maki/infer/safetensors-test.f` | maki/infer/safetensors.f | 2026-07-21 | stdlib-boundary | habu-epic-gb10-uma-391d12e8 |
| ST-PTR>N | `ptr u8 -- n` | Reinterprets a byte pointer as its numeric address for data-pointer arithmetic over an in-memory image (PARSE-SPAN); inverse of ST-N>PTR, same soundness boundary. | `maki/infer/safetensors-test.f` | maki/infer/safetensors.f | 2026-07-21 | stdlib-boundary | habu-epic-gb10-uma-391d12e8 |
