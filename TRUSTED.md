...
| XREF-PATCH32 | `n ptr a --` | Namespace finalization and explicit `undefine` patch fixed 32-bit fields inside live dictionary records; the record layout and protected write bracket are outside checked pointer inference. | `test/type-ctor-suite.f`, `tools/xref-test.f`, `test/gate-dictionary.f`, `test/run.f` | src/habu/xref.f | 2026-07-31 | builder-emit | habu-create-absolute-pkg-2856a6d0 |
...
| emit-qualify-def | `--` | Emits the shared LQUALIFYDEF helper: calls the common last-separator classifier and full-prefix package ensure leaf, rejects malformed or type-target definitions before publication, rewrites the pending token to its tail, and applies duplicate and capacity walls. | `test/gate-dictionary.f`, `tools/xref-test.f`, `test/run.f` | src/habu/habu2.f | 2026-07-31 |  | habu-resolve-native-full-52296eac |
...
| C-FIND-CHECKER | `ptr n n label --` | Bootstrap package keywords may omit a missing checker bridge only while the friend latch is open; the same path fails closed after sealing. | `tools/bootstrap-codegen-test.f`, `test/engine-suite.f` | src/habu/habu2.f | 2026-07-14 |  |  |
| SEAL-VIOLATION | `-- n` | Reopens the reserved early HB-ERROR package after checker startup solely to register the immutable constant's exact output effect. | `tools/bootstrap-codegen-test.f`, `test/engine-suite.f`, `test/seal.f` | src/core/engine-error-effects.f | 2026-07-14 | prim-axiom | habu-primitive-effect-axiom-1119f176 |
| PROTECTED-WID | `-- n` | Reopens the reserved early HB-ERROR package after checker startup solely to register the immutable constant's exact output effect. | `tools/bootstrap-codegen-test.f`, `test/engine-suite.f`, `test/seal.f` | src/core/engine-error-effects.f | 2026-07-14 | prim-axiom | habu-primitive-effect-axiom-1119f176 |
| BAD-TAG | `-- n` | Reopens the reserved early HB-ERROR package after checker startup solely to register the immutable constant's exact output effect. | `tools/bootstrap-codegen-test.f`, `test/engine-suite.f`, `test/gate-engine.f` | src/core/engine-error-effects.f | 2026-07-14 | prim-axiom | habu-primitive-effect-axiom-1119f176 |
| CALLABLE-ABI | `-- n` | Reopens the reserved early HB-ERROR package after checker startup solely to register the immutable constant's exact output effect. | `tools/bootstrap-codegen-test.f`, `test/engine-suite.f`, `test/gate-engine.f` | src/core/engine-error-effects.f | 2026-07-14 | prim-axiom | habu-primitive-effect-axiom-1119f176 |
| CATCH-STACK | `-- n` | Reopens the reserved early HB-ERROR package after checker startup solely to register the immutable constant's exact output effect. | `tools/bootstrap-codegen-test.f`, `test/engine-suite.f`, `test/gate-engine.f` | src/core/engine-error-effects.f | 2026-07-14 | prim-axiom | habu-primitive-effect-axiom-1119f176 |
| CODE-CERT | `-- n` | Reopens the reserved early HB-ERROR package after checker startup solely to register the immutable constant's exact output effect. | `tools/bootstrap-codegen-test.f`, `test/engine-suite.f`, `test/gate-engine.f` | src/core/engine-error-effects.f | 2026-07-14 | prim-axiom | habu-primitive-effect-axiom-1119f176 |
...
| c-package-path-guard | `--` | Package path guard calls the common emitted last-separator classifier and rejects leading, trailing, or doubled namespace separators before checker or dictionary mutation. | `test/gate-dictionary.f`, `test/run.f` | src/habu/habu2.f | 2026-07-31 |  | habu-resolve-native-full-52296eac |
...
| c-package-new-record | `--` | Package creation helper stores the caller's exact absolute prefix through the shared inline/EXT name path, then publishes one namespace record with nonzero public/private wordlist roles. | `test/gate-dictionary.f`, `test/run.f` | src/habu/habu2.f | 2026-07-31 |  | habu-create-absolute-pkg-2856a6d0 |
| c-package-prefix | `--` | Absolute-package prefix helper calls the shared namespace leaf, rejects a non-package kind, creates a missing exact prefix, and returns its record and role WIDs. | `test/gate-dictionary.f`, `test/run.f` | src/habu/habu2.f | 2026-07-31 |  | habu-create-absolute-pkg-2856a6d0 |
| emit-ns-ensure | `--` | Emits one LNSENSURE leaf that walks every exact prefix, reuses the shared namespace lookup and package-row creator, returns the final package record and role WIDs, and rejects a type row at any package position. | `test/gate-dictionary.f`, `test/run.f` | src/habu/habu2.f | 2026-07-31 |  | habu-resolve-native-full-52296eac |
| c-package-ensure | `--` | Package keyword wrapper calls the common LNSENSURE full-prefix leaf with the current token and receives its final package record and role WIDs. | `test/gate-dictionary.f`, `test/run.f` | src/habu/habu2.f | 2026-07-31 |  | habu-resolve-native-full-52296eac |
...
| c-export | `--` | Interpreter `EXPORT` keyword: at top level consumes a directive name; inside a package it resolves the source, derives the alias tail from the common last-separator classifier, checks dictionary/name capacity and duplicate targets, syncs the checker alias with protected-source refusal, and publishes through the shared protected-WID target guard while copying the source code pointer, body span, and immediate/wide/min-input flags. | `test/type-export-suite.f`, `test/run.f` | src/habu/habu2.f | 2026-07-31 |  | habu-resolve-native-full-52296eac |
...
| c-die-bad-tag | `--` | Emits the MATCH invalid-tag die INLINE into the user word: a jump over the message, "hb: bad <family> tag\n" copied inline (the name bytes travel with the word), then a self-contained write(2) + exit_group(HB-ERROR:BAD-TAG). | `test/run.f` | src/habu/habu2.f | 2026-07-09 |  |  |
...
| forth | `ptr u8 n --` | Top-level engine builder: consumes source bytes, allocates every forward-ref label, and emits the complete image. | `test/run.f` | src/habu/habu2.f | 2026-06-25 |  |  |
...
| FAM-PUBLIC? | `n -- bool` | Type-family registry read: forwards to the pre-hook `TFAM-PUBLIC?` for the visibility half of the generation gate; a private family exports nothing and records no constructor namespace. | `test/enum-decl-suite.f`, `test/generated-declaration-transaction-suite.f`, `test/run.f` | src/core/generated-declaration.f | 2026-07-25 | stdlib-boundary | habu-enum-generate-named-1f3261a3 |
...
| CTOR-PUBLISH | `n n n -- ` | Constructor-metadata seam: forwards to the pre-hook `TDECL-CTOR-PUBLISH`, which records the exact family namespace on every variant row. | `test/enum-decl-suite.f`, `test/generated-declaration-transaction-suite.f`, `test/run.f` | src/core/generated-declaration.f | 2026-07-25 | stdlib-boundary | habu-enum-generate-named-1f3261a3 |
...
| NS-EXISTS? | `n -- bool` | Exact-child preflight bridge: derives the canonical constructor namespace through the pre-hook family registry and asks `XREF:NAMESPACE?`, xref's owning pure package/type-row lookup. Unified ENUM and STRUCTURE use the checked catchable require above this read before their first constructor publication. | `test/generated-declaration-transaction-suite.f`, `test/run.f` | src/core/generated-declaration.f | 2026-07-31 | stdlib-boundary | habu-nest-generated-family-70b2f31a |
...
| FAM-ARITY@ | `n -- n` | Shared declaration-term family read: forwards to `TFAM-ARITY@` so `TERM` accepts only closed arity-zero families. | `test/decl-replay-verify-source.f`, `test/structure-decl-suite.f`, `test/enum-decl-suite.f`, `test/run.f` | src/core/generated-declaration.f | 2026-07-31 | stdlib-boundary | habu-factor-declaration-term-4cd6ab45 |
| FAM-LAYOUT? | `n -- bool` | Shared declaration-term family read: forwards to `TFAM-LAYOUT?` so `TERM` recognizes concrete aggregate families through the registry authority. | `test/decl-replay-verify-source.f`, `test/structure-decl-suite.f`, `test/enum-decl-suite.f`, `test/run.f` | src/core/generated-declaration.f | 2026-07-31 | stdlib-boundary | habu-factor-declaration-term-4cd6ab45 |
| FAM-CELL? | `n -- bool` | Shared declaration-term family read: forwards to `TFAM-CELL?` so `TERM` recognizes nominal scalar families through the registry authority. | `test/decl-replay-verify-source.f`, `test/structure-decl-suite.f`, `test/enum-decl-suite.f`, `test/run.f` | src/core/generated-declaration.f | 2026-07-31 | stdlib-boundary | habu-factor-declaration-term-4cd6ab45 |
| SIG-RESOLVE | `ptr u8 n ptr u8 n -- n bool` | Shared declaration-term lookup: forwards to `TFAM-SIG-RESOLVE` for binder-aware family resolution in the active declaration package. | `test/decl-replay-verify-source.f`, `test/structure-decl-suite.f`, `test/enum-decl-suite.f`, `test/run.f` | src/core/generated-declaration.f | 2026-07-31 | stdlib-boundary | habu-factor-declaration-term-4cd6ab45 |
| ACTIVE-PKG$ | `-- ptr u8 n` | Shared declaration-term package read: forwards to `TFAM-ACTIVE-PKG$` so `TERM` resolves a token in the declaration's package scope. | `test/decl-replay-verify-source.f`, `test/structure-decl-suite.f`, `test/enum-decl-suite.f`, `test/run.f` | src/core/generated-declaration.f | 2026-07-31 | stdlib-boundary | habu-factor-declaration-term-4cd6ab45 |
| CON-CODE | `ptr u8 n -- n` | Shared declaration-term concrete lookup: forwards to `CON-OF`, returning zero when the token is not a concrete type. | `test/decl-replay-verify-source.f`, `test/structure-decl-suite.f`, `test/enum-decl-suite.f`, `test/run.f` | src/core/generated-declaration.f | 2026-07-31 | stdlib-boundary | habu-factor-declaration-term-4cd6ab45 |
| CON-N | `-- n` | Shared declaration-term constant seam: reads `CC-N` for the single-letter `n` concrete type. | `test/decl-replay-verify-source.f`, `test/structure-decl-suite.f`, `test/enum-decl-suite.f`, `test/run.f` | src/core/generated-declaration.f | 2026-07-31 | stdlib-boundary | habu-factor-declaration-term-4cd6ab45 |
| CON-F | `-- n` | Shared declaration-term constant seam: reads `CC-BOOL` for the single-letter `f` concrete type. | `test/decl-replay-verify-source.f`, `test/structure-decl-suite.f`, `test/enum-decl-suite.f`, `test/run.f` | src/core/generated-declaration.f | 2026-07-31 | stdlib-boundary | habu-factor-declaration-term-4cd6ab45 |
| CON-R | `-- n` | Shared declaration-term constant seam: reads `CC-R` for the single-letter `r` concrete type. | `test/decl-replay-verify-source.f`, `test/structure-decl-suite.f`, `test/enum-decl-suite.f`, `test/run.f` | src/core/generated-declaration.f | 2026-07-31 | stdlib-boundary | habu-factor-declaration-term-4cd6ab45 |
| SCH-CON | `n -- n` | Shared declaration-term schema constructor: forwards to `SCHEMA-CON` and returns the new concrete schema node. | `test/decl-replay-verify-source.f`, `test/structure-decl-suite.f`, `test/enum-decl-suite.f`, `test/run.f` | src/core/generated-declaration.f | 2026-07-31 | stdlib-boundary | habu-factor-declaration-term-4cd6ab45 |
| SCH-PARAM | `n -- n` | Shared declaration-term schema constructor: forwards to `SCHEMA-PARAM` and returns the binder's ordinal schema node. | `test/decl-replay-verify-source.f`, `test/structure-decl-suite.f`, `test/enum-decl-suite.f`, `test/run.f` | src/core/generated-declaration.f | 2026-07-31 | stdlib-boundary | habu-factor-declaration-term-4cd6ab45 |
| SCH-PTR | `n -- n` | Shared declaration-term schema constructor: forwards to `SCHEMA-PTR` and returns the pointer schema node around its pointee. | `test/decl-replay-verify-source.f`, `test/structure-decl-suite.f`, `test/enum-decl-suite.f`, `test/run.f` | src/core/generated-declaration.f | 2026-07-31 | stdlib-boundary | habu-factor-declaration-term-4cd6ab45 |
| SCH-APP | `n n n -- n` | Shared declaration-term schema constructor: forwards to `SCHEMA-APP` and returns the closed family application node. | `test/decl-replay-verify-source.f`, `test/structure-decl-suite.f`, `test/enum-decl-suite.f`, `test/run.f` | src/core/generated-declaration.f | 2026-07-31 | stdlib-boundary | habu-factor-declaration-term-4cd6ab45 |
| SCH-LINEAR? | `n -- bool` | Shared declaration-term linearity read: forwards to `TFCL-NODE?` so `TERM` rejects a pointer whose pointee reaches a linear value. | `test/decl-replay-verify-source.f`, `test/structure-decl-suite.f`, `test/enum-decl-suite.f`, `test/run.f` | src/core/generated-declaration.f | 2026-07-31 | stdlib-boundary | habu-factor-declaration-term-4cd6ab45 |
...
| FAM-PUBLIC? | `n -- bool` | Type-family registry read: forwards to the pre-hook `TFAM-PUBLIC?`; a private family renders with an empty constructor namespace, which the probe reports rather than hides. | `test/enum-decl-suite.f`, `test/run.f` | tools/decl-gen-probe.f | 2026-07-25 | test-metaprog | habu-enum-generate-named-1f3261a3 |
...
| SM-PUBLIC? | `n -- bool` | STRUCTURE generator seam: forwards to the pre-hook `TFAM-PUBLIC?` (family is public, so it owns a reserved constructor namespace); the checker cannot type the sealed pre-hook registry word from a post-hook checked body. Re-points to the shared constructor-gen module at the type-DSL cutover. | `test/structure-make-suite.f`, `test/run.f` | src/core/structure-make.f | 2026-07-21 | stdlib-boundary | habu-structure-generate-make-872a6e75 |
...
| SM-EMIT-ROWS | `n n n n -- ` | STRUCTURE registration seam: consumes the live declaration token, family, field-range start, and count; every provisional schema read is revalidated by DECL-EVENT before the sealed mutation rebuilds roots, adds the make/unmake variant rows, sets the variant range, and stamps the constructor namespace. Split from the word rendering below so a replayed declaration (`tools/check-core.f`, `src/habu/verify-source.f`) can register a STRUCTURE without defining anything — those rows are how a later `FAMILY:MAKE` in the same source resolves, so they are registration, not generation. | `test/structure-make-suite.f`, `test/structure-decl-suite.f`, `test/generated-declaration-transaction-suite.f`, `test/run.f` | src/core/structure-make.f | 2026-07-26 | stdlib-boundary | habu-atomic-generated-declaration-4c1e8b7a |
...
| DLOPEN-RAW | `ptr u8 n -- n` | Private exact `dlopen` boundary: the path is read-only, flags are scalar, and the sealed `FFI` package fixes both directions before the trusted-only bounded call. | `lib/ffi-test.f`, `lib/task-test.f`, `lib/ptx/cuda-driver-test.f` | lib/ffi-abi.f | 2026-07-11 | stdlib-boundary | habu-ptx-m1-c-1df1d6e7 |
| DLSYM-RAW | `n ptr u8 -- n` | Private exact `dlsym` boundary: handle is scalar, symbol is read-only, and the sealed `FFI` package prevents replacement or extension of the call surface. | `lib/ffi-test.f`, `lib/task-test.f`, `lib/ptx/cuda-driver-test.f` | lib/ffi-abi.f | 2026-07-11 | stdlib-boundary | habu-ptx-m1-c-1df1d6e7 |
...
| c-package-record-match | `label label --` | Legacy single-component package matcher retained only for the out-of-scope `using` resolver; absolute package creation and protected reopen use the shared namespace leaf. | `test/using-test.f`, `test/run.f` | src/habu/habu2.f | 2026-07-31 |  | habu-create-absolute-pkg-2856a6d0 |
| c-package-prot-guard | `--` | Generated exact-package reopen guard calls the shared namespace leaf and rejects the matched package when its public wordlist is protected; ancestor prefixes are deliberately outside the guard. | `test/type-ctor-suite.f`, `test/run.f` | src/habu/habu2.f | 2026-07-31 |  | habu-create-absolute-pkg-2856a6d0 |
...
| TWX-MK-PUSH | `n n -- n` | Test-only checker-term bridge that builds the real row-push node used by the stored-effect persistence fixture; it grants no production authority. | `test/type-decl-suite.f`, `test/candidate-validation.f` | test/type-decl-suite.f | 2026-07-31 | test-metaprog | habu-enforce-one-cell-8a183ca1 |
| TWX-MK-PARAM0 | `n -- n` | Test-only nullary-family term builder over the checker's private parameter scratch, used to probe structural cell kinds without reparsing source; it grants no production authority. | `test/type-decl-suite.f`, `test/candidate-validation.f` | test/type-decl-suite.f | 2026-07-31 | test-metaprog | habu-enforce-one-cell-8a183ca1 |
| TWX-MK-PARAM1 | `n n -- n` | Test-only unary-family term builder over the checker's private parameter scratch, used for carrier-schema and bypass-unification fixtures; it grants no production authority. | `test/type-decl-suite.f`, `test/candidate-validation.f` | test/type-decl-suite.f | 2026-07-31 | test-metaprog | habu-enforce-one-cell-8a183ca1 |
| TWX-MK-PARAM2 | `n n n -- n` | Test-only binary-family term builder over the checker's private parameter scratch, used to construct the payload-bearing sum control; it grants no production authority. | `test/type-decl-suite.f`, `test/candidate-validation.f` | test/type-decl-suite.f | 2026-07-31 | test-metaprog | habu-enforce-one-cell-8a183ca1 |
| TWX-T-RES | `n -- n` | Test-only read bridge to the checker's resolved type-term representative, used to inspect the kind on the actual unification root; it grants no production authority. | `test/type-decl-suite.f`, `test/candidate-validation.f` | test/type-decl-suite.f | 2026-07-31 | test-metaprog | habu-enforce-one-cell-8a183ca1 |
| TWX-PAY | `n -- n` | Test-only encoded-term payload decoder used after resolution to recover the type-variable id under test; it grants no production authority. | `test/type-decl-suite.f`, `test/candidate-validation.f` | test/type-decl-suite.f | 2026-07-31 | test-metaprog | habu-enforce-one-cell-8a183ca1 |
| TWX-TVK@ | `n -- n` | Test-only read bridge to the private type-variable kind arena, used to assert CELL and RAW propagation and restoration; it grants no production authority. | `test/type-decl-suite.f`, `test/candidate-validation.f` | test/type-decl-suite.f | 2026-07-31 | test-metaprog | habu-enforce-one-cell-8a183ca1 |
| TWX-TVK-MEET | `n n --` | Test-only bridge to the real trailed kind meet, used to drive the CELL-to-RAW trial transition under test; it grants no production authority. | `test/type-decl-suite.f`, `test/candidate-validation.f` | test/type-decl-suite.f | 2026-07-31 | test-metaprog | habu-enforce-one-cell-8a183ca1 |
| TWX-TVK-CELL! | `n --` | Test-only untrailed CELL initializer for fresh variables before unification, trial, and effect-persistence probes; it grants no production authority. | `test/type-decl-suite.f`, `test/candidate-validation.f` | test/type-decl-suite.f | 2026-07-31 | test-metaprog | habu-enforce-one-cell-8a183ca1 |
| TWX-TVK-RAW! | `n --` | Test-only untrailed RAW initializer for the CELL-plus-RAW unification control; it grants no production authority. | `test/type-decl-suite.f`, `test/candidate-validation.f` | test/type-decl-suite.f | 2026-07-31 | test-metaprog | habu-enforce-one-cell-8a183ca1 |
| TWX-TVK-ANY | `-- n` | Test-only bridge exposing the private ANY kind constant so the fixture pins its exact encoded value; it grants no production authority. | `test/type-decl-suite.f`, `test/candidate-validation.f` | test/type-decl-suite.f | 2026-07-31 | test-metaprog | habu-enforce-one-cell-8a183ca1 |
| TWX-TVK-CELL | `-- n` | Test-only bridge exposing the private CELL kind constant for kind-arena assertions; it grants no production authority. | `test/type-decl-suite.f`, `test/candidate-validation.f` | test/type-decl-suite.f | 2026-07-31 | test-metaprog | habu-enforce-one-cell-8a183ca1 |
| TWX-TVK-RAW | `-- n` | Test-only bridge exposing the private RAW kind constant for meet and kind-arena assertions; it grants no production authority. | `test/type-decl-suite.f`, `test/candidate-validation.f` | test/type-decl-suite.f | 2026-07-31 | test-metaprog | habu-enforce-one-cell-8a183ca1 |
| TWX-TRIAL-SAVE | `--` | Test-only trial-frame opener that increments the private trial depth and invokes the production save path before a kind transition; it grants no production authority. | `test/type-decl-suite.f`, `test/candidate-validation.f` | test/type-decl-suite.f | 2026-07-31 | test-metaprog | habu-enforce-one-cell-8a183ca1 |
| TWX-TRIAL-REST | `--` | Test-only trial-frame closer that invokes production restoration and balances the private trial depth after the kind-transition probe; it grants no production authority. | `test/type-decl-suite.f`, `test/candidate-validation.f` | test/type-decl-suite.f | 2026-07-31 | test-metaprog | habu-enforce-one-cell-8a183ca1 |
| TWX-E-BUILD-EFFECT | `n n n n bool -- n` | Test-only bridge to the real stored-effect builder, used to prove EN.B persists CELL through the production record path; it grants no production authority. | `test/type-decl-suite.f`, `test/candidate-validation.f` | test/type-decl-suite.f | 2026-07-31 | test-metaprog | habu-enforce-one-cell-8a183ca1 |
| TWX-E-PTR | `n -- ptr a` | Test-only effect-offset decoder used to address the stored record built by the persistence fixture; it grants no production authority. | `test/type-decl-suite.f`, `test/candidate-validation.f` | test/type-decl-suite.f | 2026-07-31 | test-metaprog | habu-enforce-one-cell-8a183ca1 |
| TWX-E-INST-RESET | `ptr a --` | Test-only bridge to the production effect-instantiation map reset for the persisted record under test; it grants no production authority. | `test/type-decl-suite.f`, `test/candidate-validation.f` | test/type-decl-suite.f | 2026-07-31 | test-metaprog | habu-enforce-one-cell-8a183ca1 |
| TWX-E-DIN@ | `ptr a -- n` | Test-only reader for the stored effect's private data-input node offset, used immediately before production instantiation; it grants no production authority. | `test/type-decl-suite.f`, `test/candidate-validation.f` | test/type-decl-suite.f | 2026-07-31 | test-metaprog | habu-enforce-one-cell-8a183ca1 |
...
| ALLOC-BYTES>N | `CAD-NUM:alloc-byte-len -- n` | Private MEM projection for raw allocation extents. `MEM:ALLOC-BYTES` uses it for `mmap`; `MEM:RELEASE-BYTES` uses it for whole-allocation `munmap`. No raw value crosses the public boundary. Retire when both primitives accept `CAD-NUM:alloc-byte-len`. | `lib/memory-test.f` | lib/memory.f | 2026-07-28 | prim-axiom | habu-epic-model-cad-70b629a9 |
...
| BYTE-LEN>N | `CAD-NUM:byte-len -- n` | Private MEM projection for the raw `munmap` length operand. `MEM:UNMAP` is the only caller; no raw value crosses the public boundary. Retire when `munmap` accepts `CAD-NUM:byte-len`. | `lib/memory-test.f` | lib/memory.f | 2026-07-28 | prim-axiom | habu-epic-model-cad-70b629a9 |
| WB-SCOPE | `R CAD-NUM:alloc-byte-len [ R ptr u8 CAD-NUM:alloc-byte-len -- S ] -- S` | Exception-safe body of `MEM:WITH-BYTES`: allocates a mapping, catches the body, releases directly on both paths, restores the outer frame after successful release, and rethrows the body code. The arbitrary result row cannot cross a fetched execution token through checked `catch`, so the public checked word delegates to this private boundary. Retire under linear owner types. | `lib/memory-test.f` | lib/memory.f | 2026-07-28 | stdlib-boundary | habu-epic-type-habu-a34713f0 |
...
| WB-REL-CUR | `--` | Private `MEM:WITH-BYTES` frame op: reads the parked fat pointer and allocation length, calls fatal `MEM:RELEASE-BYTES`, then clears the frame only after success. Release failure exits before returning. | `lib/memory-test.f` | lib/memory.f | 2026-07-28 | stdlib-boundary | habu-epic-type-habu-a34713f0 |
...
test/decl-replay-verify-source.f:V-NS$ test-metaprog habu-cut-global-enum-56ca54e2
...
test/decl-replay-verify-source.f:N-SCH test-metaprog habu-factor-declaration-term-4cd6ab45
test/decl-replay-verify-source.f:N-CON test-metaprog habu-factor-declaration-term-4cd6ab45
test/decl-replay-verify-source.f:SCH-HIGH test-metaprog habu-factor-declaration-term-4cd6ab45
test/decl-replay-verify-source.f:ROOT-HIGH test-metaprog habu-factor-declaration-term-4cd6ab45
...
test/generated-declaration-transaction-suite.f:NS-VAR-NS$ test-metaprog habu-nest-generated-family-70b2f31a
test/generated-declaration-transaction-suite.f:NS-VAR-SYM@ test-metaprog habu-nest-generated-family-70b2f31a
...
prim - sumv-ctor-ns$ - pe-n pe-in pe-ptr-u8 pe-out pe-n pe-out
