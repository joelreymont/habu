...
| emit-qualify-def | `--` | Emits the shared LQUALIFYDEF helper: resolves or creates the namespace wordlist under dictionary/name-capacity checks, rewrites the pending token to its qualified tail, and rejects malformed qualification or a duplicate target definition. Saves the link register across its internal hash-index branch-with-link. | `test/gate-dictionary.f`, `tools/xref-test.f`, `test/run.f` | src/habu/habu2.f | 2026-07-21 |  |  |
...
| c-package-path-guard | `--` | Package path guard scans the current token in generated registers and rejects leading, trailing, or doubled namespace separators before checker or dictionary mutation. | `test/gate-dictionary.f`, `test/run.f` | src/habu/habu2.f | 2026-07-31 |  | habu-create-absolute-pkg-2856a6d0 |
...
| c-package-ensure | `--` | Package keyword path walker derives each ancestor by separator chopping and ensures every exact absolute prefix through the shared namespace lookup seam. | `test/gate-dictionary.f`, `test/run.f` | src/habu/habu2.f | 2026-07-31 |  | habu-create-absolute-pkg-2856a6d0 |
...
| c-export | `--` | Interpreter `EXPORT` keyword: at top level consumes a directive name; inside a package it resolves the source, checks dictionary/name capacity and duplicate targets, syncs the checker alias with protected-source refusal, and publishes through the shared protected-WID target guard while copying the source code pointer, body span, and immediate/wide/min-input flags. | `test/type-export-suite.f`, `test/run.f` | src/habu/habu2.f | 2026-07-10 |  |  |
