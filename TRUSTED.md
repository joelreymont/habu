...
| MINT-MAPPING | `ptr u8 -- SAFET:mapping` | Mints the linear file-mapping owner from the three-cell record that SAFET:DETACH-MAPPING just allocated. A mapping token IS its record, so the leaf is an identity; the checker cannot express "this pointer is a freshly filled SAFET mapping record". Package-private to SAFET. | `maki/infer/safetensors-test.f` | maki/infer/safetensors.f | 2026-07-26 | stdlib-boundary | habu-epic-gb10-uma-391d12e8 |
