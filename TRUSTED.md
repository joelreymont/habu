...
| IMAGE-TEXT-SIZE-OFF | `-- n` | Linux executable header offset where the snapshot writer reads the mapped text size. | `test/run.f`, `tools/build-fixpoint-test.f` | src/os/linux/layout.f:12 | 2026-06-26 |
| IMAGE-TEXT-CONTENT-ADJ | `-- n` | Linux text-size field adjustment from segment size to code-content size for snapshot streaming. | `test/run.f`, `tools/build-fixpoint-test.f` | src/os/linux/layout.f:13 | 2026-06-26 |
| IMAGE-TEXT-TRAILER-ADJ | `-- n` | Linux trailer address adjustment for snapshot restore when the text-size field includes the code offset. | `test/run.f`, `tools/build-fixpoint-test.f` | src/os/linux/layout.f:14 | 2026-06-26 |
| DATA-VA | `-- ptr a` | Linux fixed DATA virtual address used by snapshot and AOT startup writers as both cell-address and byte-span base. | `test/run.f`, `tools/build-fixpoint-test.f` | src/os/linux/layout.f:15 | 2026-06-26 |
| DATA-SIZE | `-- n` | Linux fixed DATA mapping size used by snapshot validation and image inspection. | `test/run.f`, `tools/build-fixpoint-test.f` | src/os/linux/layout.f:16 | 2026-06-26 |
...
| CODE-OFF | `-- n` | Linux executable code offset used by checked snapshot streaming code. | `test/run.f`, `tools/build-fixpoint-test.f` | src/os/linux/layout.f:17 | 2026-06-26 |
| DLOPEN-SLOT | `-- ptr n` | Linux dynamic ELF fixed GOT cell where ld.so resolves `dlopen` before Habu FFI reads the function pointer. | `test/run.f` | src/os/linux/layout.f:18 | 2026-06-26 |
| DLSYM-SLOT | `-- ptr n` | Linux dynamic ELF fixed GOT cell where ld.so resolves `dlsym` before Habu FFI reads the function pointer. | `test/run.f` | src/os/linux/layout.f:19 | 2026-06-26 |
| ASM-CODE | `-- asm` | Target image-builder bridge that snapshots the current assembler code length and produces the checked image-build phase token. | `test/run.f`, `tools/build-fixpoint-test.f`, `test/engine-suite.f` | src/os/linux/elf.f:56 | 2026-06-26 |
| BUILD-IMAGE | `asm -- img` | Target image-builder boundary that consumes the checked assembled-code phase token and produces the executable-image phase token. | `test/run.f`, `tools/build-fixpoint-test.f`, `test/engine-suite.f` | src/os/linux/elf.f:195 | 2026-06-26 |
| BUILD-SNAP-HDR | `n -- snap n` | Target snapshot-header builder; consumes text payload length, produces the snapshot-header phase token, and returns padded executable text size. | `test/run.f`, `tools/build-fixpoint-test.f`, `test/engine-suite.f` | src/os/linux/elf.f:206 | 2026-06-26 |
| SNAP-EXTRA-PTR | `-- ptr u8` | Linux snapshot writer stages the dynamic RW segment after the header buffer and streams it after the padded live text. | `test/run.f`, `test/engine-suite.f` | src/os/linux/elf.f:178 | 2026-06-26 |
| SNAP-EXTRA-SIZE | `-- n` | Linux snapshot writer appends the fixed-size `.dynamic` plus GOT segment after the padded text payload. | `test/run.f`, `test/engine-suite.f` | src/os/linux/elf.f:181 | 2026-06-26 |
...
| SNAP-EXTRA-PTR | `-- ptr u8` | macOS snapshot images have no post-text extra segment; the shared snapshot writer still needs a typed zero-length byte span. | `test/run.f`, `test/engine-suite.f` | src/os/macos/macho.f:108 | 2026-06-26 |
| SNAP-EXTRA-SIZE | `-- n` | macOS snapshot images append no target extra bytes after the padded text payload. | `test/run.f`, `test/engine-suite.f` | src/os/macos/macho.f:111 | 2026-06-26 |
...
| SNAP-WRITE | `snap --` | Snapshot writer consumes the final snapshot-header phase token before streaming header, text, regions, trailer, and padding to disk. | `test/run.f`, `tools/build-fixpoint-test.f` | src/habu/snap.f:67 | 2026-06-26 |
| SNAP-CHECK-HOOK | `ptr u8 n -- n` | Snapshot image installs the fail-closed checker hook into the emitted image; recursive `CHECK!` hook bodies are trusted boundaries. | `test/run.f`, `test/gate-debug.f` | src/habu/snap.f:77 | 2026-06-26 |
| SNAP-INSTALL-HOOK | `--` | Snapshot image mutates the checker hook cell only through a named trusted installer. | `test/run.f`, `test/gate-debug.f` | src/habu/snap.f:79 | 2026-06-26 |
