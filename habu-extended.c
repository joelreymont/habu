...
habu_value_t PARSE_OP(habu_value_t, habu_value_t);
...
habu_value_t PARSE_NUMBER(habu_value_t STR, habu_value_t IDX, habu_value_t ACC) {
    return (is_nil((value_to_fixnum(IDX) >= value_to_fixnum(fixnum_to_value(habu_string_length_raw(STR))) ? fixnum_to_value(1) : NIL)) ? ({
    habu_value_t CH = habu_string_ref(STR, value_to_fixnum(IDX));
    (is_nil(IS_DIGIT_P(CH)) ? habu_cons(ACC, IDX) : PARSE_NUMBER(STR, fixnum_to_value(value_to_fixnum(IDX) + value_to_fixnum(fixnum_to_value(1))), fixnum_to_value(value_to_fixnum(fixnum_to_value(value_to_fixnum(ACC) * value_to_fixnum(fixnum_to_value(10)))) + value_to_fixnum(fixnum_to_value(value_to_fixnum(CH) - value_to_fixnum(fixnum_to_value(48)))))));
    }) : habu_cons(ACC, IDX));
}

habu_value_t PARSE_OP(habu_value_t STR, habu_value_t IDX) {
    return ({
    habu_value_t CH = habu_string_ref(STR, value_to_fixnum(IDX));
    (is_nil((value_to_fixnum(CH) == value_to_fixnum(fixnum_to_value(43)) ? fixnum_to_value(1) : NIL)) ? (is_nil((value_to_fixnum(CH) == value_to_fixnum(fixnum_to_value(45)) ? fixnum_to_value(1) : NIL)) ? (is_nil((value_to_fixnum(CH) == value_to_fixnum(fixnum_to_value(42)) ? fixnum_to_value(1) : NIL)) ? (is_nil((value_to_fixnum(CH) == value_to_fixnum(fixnum_to_value(47)) ? fixnum_to_value(1) : NIL)) ? habu_cons(NIL, IDX) : habu_cons(habu_make_symbol_from_string(habu_make_string("/", 1)), fixnum_to_value(value_to_fixnum(IDX) + value_to_fixnum(fixnum_to_value(1))))) : habu_cons(habu_make_symbol_from_string(habu_make_string("*", 1)), fixnum_to_value(value_to_fixnum(IDX) + value_to_fixnum(fixnum_to_value(1))))) : habu_cons(habu_make_symbol_from_string(habu_make_string("-", 1)), fixnum_to_value(value_to_fixnum(IDX) + value_to_fixnum(fixnum_to_value(1))))) : habu_cons(habu_make_symbol_from_string(habu_make_string("+", 1)), fixnum_to_value(value_to_fixnum(IDX) + value_to_fixnum(fixnum_to_value(1)))));
    });
}
...
habu_value_t PARSE_ONE(habu_value_t STR, habu_value_t IDX) {
    ...
    (is_nil((value_to_fixnum(IDX2) >= value_to_fixnum(fixnum_to_value(habu_string_length_raw(STR))) ? fixnum_to_value(1) : NIL)) ? ({
      ...
      (is_nil((value_to_fixnum(CH) == value_to_fixnum(fixnum_to_value(40)) ? fixnum_to_value(1) : NIL)) ? (is_nil(IS_DIGIT_P(CH)) ? PARSE_OP(STR, IDX2) : PARSE_NUMBER(STR, IDX2, fixnum_to_value(0))) : PARSE_LIST(STR, fixnum_to_value(value_to_fixnum(IDX2) + value_to_fixnum(fixnum_to_value(1)))));
      }) : habu_cons(NIL, IDX2));
    });
}
...
habu_value_t PARSE_LIST(habu_value_t STR, habu_value_t IDX) {
    ...
    habu_value_t IDX2 = SKIP_WS(STR, IDX);
    ({
      habu_value_t OP_RESULT = PARSE_ONE(STR, IDX2);
      ({
        habu_value_t OP = habu_car(OP_RESULT);
        ({
          habu_value_t IDX3 = habu_cdr(OP_RESULT);
          ({
            habu_value_t ARG1_RESULT = PARSE_ONE(STR, IDX3);
            ({
              habu_value_t ARG1 = habu_car(ARG1_RESULT);
              ({
                habu_value_t IDX4 = habu_cdr(ARG1_RESULT);
                ({
                  habu_value_t ARG2_RESULT = PARSE_ONE(STR, IDX4);
                  ({
                    habu_value_t ARG2 = habu_car(ARG2_RESULT);
                    ({
                      habu_value_t IDX5 = SKIP_WS(STR, habu_cdr(ARG2_RESULT));
                      ({
                        habu_value_t IDX6 = fixnum_to_value(value_to_fixnum(IDX5) + value_to_fixnum(fixnum_to_value(1)));
                        habu_cons(habu_cons(OP, habu_cons(ARG1, habu_cons(ARG2, NIL))), IDX6);
                        });
                      });
                    });
                  });
...
habu_value_t REPL_START() {
    return ({
  habu_print_value(habu_make_string("Habu REPL - List Support", 24));
  ({printf("\n"); NIL;});
  REPL_LOOP();
});
}
...
habu_value_t REPL_LOOP() {
  ...
  ({
        ...
        ({
          ...
          ({
            habu_value_t RESULT = EVAL_EXPR(EXPR);
            ({
