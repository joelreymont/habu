;;;; Minimal Runtime REPL
;;;; Complete Habu REPL with predicates, comparisons, reader, eval in Lisp
;;;; Minimal C runtime - only primitives like get-tag, string-ref, cons, etc.
...
(defun repl-loop ()
  (progn
    (print (quote "Habu REPL - Minimal Runtime"))
    (println)
            ...
            (if (> (string-length line) (quote 0))
                (let ((input-str (make-string-from-cstr line)))
                  (let ((expr (read-from-string input-str)))
                    (let ((result (eval expr (quote nil))))
                      (progn
                        (print-value result)
                        (println)))))
                (quote nil))
