...
(defun wrap-bytecode-with-mmap-heap (code-bytes heap-size)
  "Wrap bytecode with mmap-based heap initialization.
   HEAP-SIZE: total heap size in bytes.
   Uses mmap syscall at runtime instead of pre-allocated __DATA segment.
   Returns prologue + original code."
  (let* ((init-bytes (mmap-heap-init-code heap-size))
         ;; Calculate BL offset: from BL instruction to user code
         ;; After BL comes epilogue (7 instructions): lsr, 4x ldr, add, ret
         ;; So BL offset = 7 + 1 = 8 (skip epilogue to reach user code)
         (user-code-offset 8))
    (append
...
;;; ============================================================
;;; Mach-O Builder Without Heap Segment (uses mmap at runtime)
;;; ============================================================

(defun build-macho-executable-mmap-heap (code-bytes imports &optional local-fns)
  "Build Mach-O executable without __DATA heap segment.
   Heap is allocated via mmap at runtime instead of pre-mapped segment.
   CODE-BYTES should already be wrapped with mmap-heap-init-code.
   Returns executable as byte list."
  (let* ((num-imports (length imports))
         (num-locals (if local-fns (length local-fns) 0))
         (stub-size 12)
         (stubs-total-size (* num-imports stub-size))
         ...
         ;; 13 load commands (no __DATA segment)
         (ncmds 13)
         ...
         ;; Page calculations for stubs
         (stub-page (ash stubs-offset -12))
         (got-page (ash data-const-fileoff -12))
         (got-page-diff (- got-page stub-page))
         (got-base-offset 0)

         ;; __LINKEDIT segment - comes right after __DATA_CONST (no __DATA)
         ...
         (nlist-size (* nsyms 16))
         (strtab-offset (+ symtab-offset nlist-size))
         (indirect-offset (align-up (+ strtab-offset string-table-size) 8))
         (num-indirect-syms (* 2 num-imports))  ; one for __stubs, one for __got
         (indirect-size (* num-indirect-syms 4))
         (fixups-offset (align-up (+ indirect-offset indirect-size) 8))

         ;; Chained fixups - generate actual data to get correct size
         (got-vm-offset text-vmsize)
         (fixups-data (build-chained-fixups-data imports num-segments 2 got-vm-offset))
         (fixups-size (length fixups-data))
         (aligned-fixups-size (align-up fixups-size 8))
         ...
         ;; Total LINKEDIT size
         (linkedit-size (align-up (+ (- exports-offset linkedit-fileoff)
                                    aligned-exports-size) +PAGE-SIZE+))

         ;; Entry point
         (entry-offset code-offset)

         ;; UUID (HABULISP MMAPHEAP)
    ...
    (append
     ...
     ;; LC #1: __TEXT with __text and __stubs
     (buf-segment-command-64 "__TEXT" +VM-BASE+ text-vmsize
                             0 text-filesize
                             (logior +VM-PROT-READ+ +VM-PROT-EXECUTE+)
                             (logior +VM-PROT-READ+ +VM-PROT-EXECUTE+)
                             2 0)
     ...
     (buf-section-64 "__stubs" "__TEXT"
                    (+ +VM-BASE+ stubs-offset) stubs-total-size stubs-offset 2 0 0
                    (logior +S-SYMBOL-STUBS+ +S-ATTR-PURE-INSTRUCTIONS+)
                    num-imports stub-size)

     ;; LC #2: __DATA_CONST with __got
     (buf-segment-command-64 "__DATA_CONST" data-const-vmaddr data-const-vmsize
                             data-const-fileoff data-const-filesize
                             (logior +VM-PROT-READ+ +VM-PROT-WRITE+)
                             (logior +VM-PROT-READ+ +VM-PROT-WRITE+)
     ...
     ;; LC #3: __LINKEDIT (no __DATA segment!)
     (buf-segment-command-64 "__LINKEDIT" linkedit-vmaddr +PAGE-SIZE+
                             linkedit-fileoff linkedit-size
                             +VM-PROT-READ+ +VM-PROT-READ+
                             0 0)
     ...
     ;; LC #12: dysymtab
     (buf-dysymtab-command-full 0 num-locals       ; local symbols
                                num-locals 1       ; external defined
                                (1+ num-locals) num-imports  ; undefined
                                (/ indirect-offset 4) num-imports)
     ...
     ;; Padding to LINKEDIT
     (buf-zeros (- linkedit-fileoff (+ data-const-fileoff got-total-size)))

     ;; Symbol table entries
     (if local-fns
         (build-symbol-table-with-locals local-fns imports (+ +VM-BASE+ code-offset) wrapper-size)
         (build-symbol-table imports (+ +VM-BASE+ code-offset)))
     ...
     ;; Chained fixups (pre-generated above)
     fixups-data

     ;; Padding to exports
     (buf-zeros (- exports-offset (+ fixups-offset fixups-size)))

     ;; Exports trie
     exports-trie-data

     ;; Final padding to reach page boundary
     (buf-zeros (- linkedit-size
                  (+ nlist-size string-table-size
                     (- indirect-offset strtab-offset string-table-size)
                     indirect-size
                     (- fixups-offset indirect-offset indirect-size)
                     fixups-size
                     (- exports-offset fixups-offset fixups-size)
                     exports-trie-size))))))

(defun write-macho-executable-mmap-heap (output-path code-bytes imports &optional local-fns)
  "Generate Mach-O executable with mmap heap, write to file.
   CODE-BYTES should already be wrapped with mmap-heap-init-code."
  (let* ((exe-buf (build-macho-executable-mmap-heap code-bytes imports local-fns))
         (exe-str (buf-to-string exe-buf)))
    (native-write-executable output-path exe-str)))
