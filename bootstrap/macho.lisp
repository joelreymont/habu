...
;;; ============================================================
;;; These constants MUST match the actual wrapper code generated by
;;; wrap-bytecode-with-heap-for-imports and wrap-bytecode-with-mmap-heap.
...
(defconstant +mmap-wrapper-size+ 204
  "Size of mmap heap wrapper in bytes (51 instructions * 4).
   Used by wrap-bytecode-with-mmap-heap for runtime mmap allocation.")
...
(defun wrap-bytecode-with-mmap-heap (code-bytes heap-size)
  ...
  (let* ((init-bytes (mmap-heap-init-code heap-size))
    ...
    (append
     ...
     ;; ADR uses byte offset: BL (4 bytes) + epilogue (3*4=12 bytes) = 16 bytes
     ;; Plus 4 bytes for ADR instruction itself = 20 bytes to user code
     ;; BL offset: skip epilogue (3) + 1 = 4 instructions to reach user code
     (buf-append-all
      (list (arm64:adr :code-base 20)               ; adr x26, +20 (code base = user code start)
            (arm64:bl 4)))                          ; bl user_code (skip 4 instructions)

     ;; Epilogue: untag result and call _exit syscall
     ;; For standalone executable, we exit instead of returning
     (buf-append-all
      (list (arm64:lsr :x0 :x0 4 :imm t)          ; lsr x0, x0, #4 (untag fixnum for exit code)
            (arm64:movz :x16 1)                    ; mov x16, #1 (_exit syscall number)
            (arm64:svc 0)))                        ; svc 0 (syscall)
...
(defun build-macho-executable-mmap-heap (code-bytes imports &optional local-fns)
  ...
  (let* ((num-imports (length imports))
         ...
         ;; Wrapper size for symbol table
         (wrapper-size +mmap-wrapper-size+))
