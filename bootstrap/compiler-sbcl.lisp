...
(defun sys:compile (expr env fenv)
  (cond
    ...
    ((consp expr)
     ...
     ;; (incf/decf/loop are macros but we compile them directly)
     (if (and (symbolp (car expr))
              (macro-function (car expr))
              (not (member (car expr) '(incf decf loop))))
         ...
         ((eq op 'decf)
          (let* ((place (cadr expr))
                 (delta (if (cddr expr) (caddr expr) 1)))
            (sys:compile `(setq ,place (- ,place ,delta)) env fenv)))
         ;; loop - expand common LOOP patterns to simpler forms
         ((eq op 'loop)
          (let ((clauses (cdr expr)))
            (cond
              ;; (loop for var across str collect body)
              ((and (>= (length clauses) 5)
                    (eq (first clauses) 'for)
                    (eq (third clauses) 'across)
                    (eq (fifth clauses) 'collect))
               (let ((var (second clauses))
                     (seq-form (fourth clauses))
                     (body (sixth clauses))
                     (seq-sym (gensym "SEQ"))
                     (len-sym (gensym "LEN"))
                     (idx-sym (gensym "IDX"))
                 ...
                 (sys:compile
                  `(let* ((,seq-sym ,seq-form)
                     ...
                     (tagbody
                      ...
                      (if (>= ,idx-sym ,len-sym)
                          (go loop-end))
                      (let ((,var (string-ref ,seq-sym ,idx-sym)))
                        (setq ,acc-sym (cons ,body ,acc-sym)))
                      (setq ,idx-sym (+ ,idx-sym 1))
                      (go loop-start)
                      loop-end)
                     (reverse ,acc-sym))
                  env fenv)))
              ;; (loop for var in list collect body)
              ((and (>= (length clauses) 5)
                    (eq (first clauses) 'for)
                    (eq (third clauses) 'in)
                    (eq (fifth clauses) 'collect))
               (let ((var (second clauses))
                     (list-form (fourth clauses))
                     (body (sixth clauses))
                     (lst-sym (gensym "LST"))
                     (acc-sym (gensym "ACC")))
                 (sys:compile
                  `(let* ((,lst-sym ,list-form)
                          (,acc-sym nil))
                     (tagbody
                      loop-start
                      (if (null ,lst-sym)
              ...
              ((and (>= (length clauses) 7)
                    (eq (first clauses) 'for)
                    (eq (third clauses) 'from)
                    (eq (fifth clauses) 'to)
                    (eq (seventh clauses) 'do))
               (let ((var (second clauses))
                     (start-form (fourth clauses))
                     (end-form (sixth clauses))
                     (body (nthcdr 7 clauses))
                     (end-sym (gensym "END")))
                 (sys:compile
                  `(let ((,var ,start-form)
                         (,end-sym ,end-form))
                     ...
                     nil)
                  env fenv)))
              ;; (loop for i from start below end do body...)
              ((and (>= (length clauses) 7)
                    (eq (first clauses) 'for)
                    (eq (third clauses) 'from)
                    (eq (fifth clauses) 'below)
                    (eq (seventh clauses) 'do))
               (let ((var (second clauses))
                     (start-form (fourth clauses))
                     (end-form (sixth clauses))
                 ...
                 (sys:compile
                  `(let ((,var ,start-form)
                     ...
                     nil)
                  env fenv)))
              ;; (loop repeat n collect body)
              ((and (>= (length clauses) 3)
                    (eq (first clauses) 'repeat)
                    (eq (third clauses) 'collect))
               (let ((n-form (second clauses))
                     (body (fourth clauses))
                     (n-sym (gensym "N"))
                     (i-sym (gensym "I"))
                     (acc-sym (gensym "ACC")))
              ...
              ((and (>= (length clauses) 3)
                    (eq (first clauses) 'while)
                    (eq (third clauses) 'do))
               (let ((cond-form (second clauses))
                     (body (nthcdr 3 clauses)))
                 (sys:compile
                  `(tagbody
                    loop-start
                    (if (not ,cond-form)
                    ...
                    nil)
                  env fenv)))
              ;; Simple infinite loop: (loop body...) - not commonly used but just in case
              (t
               (error "Unsupported LOOP form: ~S" expr)))))
