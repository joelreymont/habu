...
: FPRIM-L ( ptr u8 xt -- ) {: na nu xt -- :}          \ body preserves LR; stack guards frame their own BL
   LBL LBL {: lbl elbl :}
...
: STACK-GUARD:EXIT-BOUNDS ( -- )
   LBL {: msg :}
   0 2 MOVZ, 1 msg ADR, 2 25 MOVZ, NR-WRITE SYS,
   0 ENGINE-ERROR:STACK-BOUNDS MOVZ, NR-EXIT-GROUP SYS,
   msg LBL, s" hb: stack bounds exceeded" BYTES, ;

: STACK-GUARD:ALIGNED ( reg fail -- ) {: reg fail :}
   $F240081F reg 5 lshift or STACK-GUARD:WORD, C-NE fail BCOND, ;

: STACK-GUARD:DESCRIPTOR ( base cap fail -- ) {: base cap fail :}
   base fail CBZ, base fail STACK-GUARD:ALIGNED
   $AA2003E0 base 16 lshift or base or STACK-GUARD:WORD,
   cap base CMP, C-HI fail BCOND,
   $AA2003E0 base 16 lshift or base or STACK-GUARD:WORD, ;

: STACK-GUARD:CHECK-CURSOR ( above fail -- ) {: above fail :}
   14 10 fail STACK-GUARD:DESCRIPTOR
   12 fail STACK-GUARD:ALIGNED
...
: B.S ( -- )
   ...
   LBL {: sl :}  LBL {: sd :}
   sl LBL,
      9 DATA SSCR-CELL LDR,  9 XDS CMP,  C-CS sd BCOND,
      9 9 0 LDR,  G-PRINT9
...
: C-EXIT-DIAG ( ptr u8 n rc -- ) {: a u rc :}
   LBL LBL {: msg after :}
   after B,
   msg LBL,  a u BYTES,
   after LBL,
...
: BTHROW ( -- )
   ...
   10 11 40 LDR, 14 STACK-ABI:RETURN-CELLS MOVZ, 10 14 CMP, C-HI lcorrupt BCOND,
   10 11 48 LDR, 14 STACK-ABI:LOOP-FRAMES MOVZ, 10 14 CMP, C-HI lcorrupt BCOND,
   14 11 STACK-ABI:CATCH-BASE LDR, 10 11 STACK-ABI:CATCH-CAP LDR, 12 11 8 LDR,
   8 lcorrupt STACK-GUARD:CHECK-CURSOR
...
: BSETPREFLIGHT ( -- )
   LBL LBL LBL LBL LBL LBL LBL {: bad invalid empty emit done msg invalid-msg :}
   A G-POP
...
\ folded into its immediate field, through LCEMIT. Recovery mirror of
\ src/habu/habu2.f EMIT-ADDSUB-IMM: every variable-width stack transfer offset
\ in the pass-2 emitters reaches the code window here, in both engines.
...
\ against the destination. Below 4096 it stays ONE word, which is the shape
\ test/bootstrap-wide-memory-src.f's goldens decode.
...
: EMIT-ADDSUB-IMM ( -- )
   ...
   low LBL,
      9 7 10 LSLI,  9 9 8 ORR,  LCEMIT @ BL,          \ the single folded word
   done LBL,
...
: EMIT-BCAP ( -- )
   ...
   LBCS @ LBL,
   LBL LBL LBL LBL {: bok bcp bcd go :}
...
: EMIT-FIND ( -- )
   ...
   {: qscan qunqual qpkg qloop qnext qcmp qmatch qinl qmiss floop fdone fnext frank fcmp fscan fhit finl pri pub have :}
...
: EMIT-FIND-USED ( -- )
   ...
   {: qscan qnone ret uloop mloop member ninl ncmp nmatch unext udone amb ambmsg :}
   13 0 MOVZ,
   17 0 MOVZ,
   qscan LBL,
...
: EMIT-TRAPH ( -- )
   ...
   0 stackbad STACK-GUARD:CHECK-CURSOR
   12 stackempty CBZ,                                \ a valid empty stack has no top cell
   ...
   tno LBL,
   LCRASHH @ B,
   stackbad LBL, STACK-GUARD:EXIT-BOUNDS
...
\ names no scratch register: the emitted code runs while x9..x15 (regalloc.fs
\ VRPACK) may still hold live values.
: C-EMIT-DROP-X12 ( -- )   \ mirrors src/habu/habu2.f C-EMIT-DROP-X12
   LBL {: done :}
   12 done CBZ,
      8 $910003FF LIT64,  7 12 0 ADDI,  LADDSUBIMM @ BL,   \ add sp,sp,#bytes
   done LBL, ;
...
: C-DOES-REC ( -- )
   LBL LBL LBL LBL {: cpy cpd pad pend :}
   C-DOES-NAME$
   10 CP 0 ADDI,
...
: C-QUALIFY-DEF ( -- )
   ...
   {: qscan qnone qhas qtail qseal qsealed qlookup nloop nnext ncmp nmatch ninl nmake nroom qapply qbad done :}
...
: C-PACKAGE-CHECK-CALL ( n n -- ) {: lvar len :}
   LBL {: done :}
...
: C-PACKAGE-PROT-GUARD ( n -- )
   {: bad :}
   LBL LBL LBL LBL {: loop miss hit done :}
   5 DBASE 0 ADDI,  6 NDICT 0 ADDI,
   loop LBL,
      6 done CBZ,
...
: C-PACKAGE ( -- )
   LBL LBL LBL {: inactive hastok checkdone :}
...
: C-USING-NAME-GUARD ( -- )   \ consume the next token; reject a missing name / a ':' in it
   LBL LBL LBL LBL LBL LBL {: mmiss hastok cscan cbad cmsg cok :}
   LTOK @ BL,  0 hastok CBNZ,
...
: C-USING-WID ( -- )   \ TKA/TKL name a package -> x2 = its public WID; die unknown otherwise
   LBL LBL LBL LBL LBL LBL {: loop miss hit notfound umsg done :}
   5 DBASE 0 ADDI,  6 NDICT 0 ADDI,
   loop LBL,
      6 notfound CBZ,
...
: EM-P2-CARVE ( -- )
   ...
   {: ql qd wl wd wok pl pd jl jd sok :}
   SP SP 32 SUBI,
   ...
   frameok LBL,
   8 $D10003FF LIT64,  7 5 0 ADDI,  LADDSUBIMM @ BL,   \ sub sp,sp,#frame
...
: C-LBRACE-CARVE-FRAME ( -- )
   ...
   8 $D10003FF LIT64,  7 5 0 ADDI,  LADDSUBIMM @ BL,         \ sub sp,sp,#frame (SP stays 16-aligned)
...
: C-ESC-HEX-X9 ( label -- ) {: bad :}
   LBL LBL LBL {: lower upper done :}
...
: C-ESC-DECODE-BASIC ( label label -- ) {: hex bad :}
   LBL LBL LBL LBL LBL LBL LBL LBL LBL LBL LBL LBL {: dq bs bel bs8 esc lf ff cr tab vt nul done :}
...
: EMIT-ESC-SCAN ( -- )
   LBL LBL LBL LBL LBL {: scan done esc hex bad :}
   LESCSCAN @ LBL,
   SP SP 16 SUBI,  30 SP 0 STR,
   11 0 MOVZ,
...
: EMIT-ESC-COPY ( -- )
   LBL LBL LBL LBL LBL {: copy done esc hex bad :}
   LESCCOPY @ LBL,
...
: EMIT-SEED-DICT ( -- )
   LBL LBL LBL LBL {: scopy scdone pkg fields :}
...
: EMIT-SNAPSHOT-VALIDATE-WIDS ( n -- ) {: bad :}
   LBL LBL LBL LBL LBL
   {: prot-loop prot-max prot-inner prot-next widn :}
...
: EMIT-SNAPSHOT-RESTORE ( -- )
   ...
   {: snomag snbad snok snnew snhave snbadver snpresent :}
   24 0 MOVZ,
...
: EMIT-P2-COPY ( -- )
   ...
   valid LBL,
   8 $D2800009 LIT64,  7 5 5 LSLI,  9 8 7 ORR,  LCEMIT @ BL,        \ movz x9,#len
...
: EMIT-P2-DROPN ( -- )
   LP2DROPN @ LBL,
   SP SP 16 SUBI,  30 SP 0 STR,
   8 $D1000273 LIT64,  7 5 3 LSLI,  LADDSUBIMM @ BL,                \ sub x19,x19,#n*8
   30 SP 0 LDR,  SP SP 16 ADDI,  RET, ;

\ An empty/1-cell span is a noop the emitted `cmp x10,x11 ; b.hs` takes at run
\ time, so this emitter has no compile-time early-out of its own.
: EMIT-P2-REV ( -- )
   LP2REV @ LBL,
   SP SP 16 SUBI,  30 SP 0 STR,
   8 $D100026A LIT64,  7 5 0 ADDI,  LADDSUBIMM @ BL,                \ sub x10,x19,#lo
   8 $D100026B LIT64,  7 6 0 ADDI,  LADDSUBIMM @ BL,                \ sub x11,x19,#hi
...
: EMIT-P2-ROT ( -- )
   ...
   LP2REV @ BL,                                  \ reverse the top T-k cells
   5 SP 8 LDR,  5 5 3 LSLI,  6 8 MOVZ,
   LP2REV @ BL,                                  \ reverse the whole span
   done LBL,
   30 SP 0 LDR,  SP SP 32 ADDI,  RET, ;

: EMIT-P2-RS ( -- )
   LBL LBL LBL {: rsto rsdone valid :}
   LP2RS @ LBL,
   SP SP 32 SUBI,  30 SP 0 STR,  5 SP 8 STR,  6 SP 16 STR,
   5 rsdone CBZ,
   ...
   10 20 RSP-CELL W-LDRX C-EMITW
   8 $D100014A LIT64,  5 SP 8 LDR,  7 5 0 ADDI,  LADDSUBIMM @ BL,            \ sub x10,x10,#T
   RSTK-SLOT-ADDR,
   ...
   10 20 RSP-CELL W-LDRX C-EMITW
   RSTK-SLOT-ADDR,
   8 $D100026C LIT64,  5 SP 8 LDR,  7 5 3 LSLI,  LADDSUBIMM @ BL,            \ sub x12,x19,#T*8
   8 $D2800009 LIT64,  5 SP 8 LDR,  7 5 5 LSLI,  9 8 7 ORR,  LCEMIT @ BL,    \ movz x9,#T
   ...
   $F1000529 C-EMITW                                  \ subs x9,x9,#1
   $54FFFF61 C-EMITW                                  \ b.ne loop (-5)
   8 $D1000273 LIT64,  5 SP 8 LDR,  7 5 3 LSLI,  LADDSUBIMM @ BL,            \ sub x19,x19,#T*8
   8 $9100014A LIT64,  5 SP 8 LDR,  7 5 0 ADDI,  LADDSUBIMM @ BL,            \ add x10,x10,#T
   10 20 RSP-CELL W-STRX C-EMITW
   rsdone LBL,
   30 SP 0 LDR,  SP SP 32 ADDI,  RET, ;
...
: EMIT-P2-STORE ( -- )
   ...
   8 $D280000B LIT64,  7 5 8 LSLI,  9 8 7 ORR,  LCEMIT @ BL,    \ movz x11,#W*8
   11 LPROTSPAN @ ADR,
   C-CALL-EMIT-ABSOLUTE                              \ registered protection helper in fixed engine text
   ...
   5 SP 8 LDR,
   8 $D1000273 LIT64,  7 5 3 LSLI,  LADDSUBIMM @ BL,            \ sub x19,x19,#W*8
   done LBL,
   30 SP 0 LDR,  SP SP 16 ADDI,  RET, ;
...
: EMIT-WIDTH ( -- )
   LBL LBL LBL LBL {: missing keyeq found drift :}
   LP2CWAT @ LBL,
   14 10 0 ADDI,
   11 DATA TXN-CERT-A-CELL LDR,
   12 11 WF-COUNT-CELL cells LDR,
...
: EMIT-DESC ( -- )
   LBL LBL LBL {: missing found drift :}
   LP2CDESC @ LBL,
...
: EMIT-DESC ( -- )
   LBL LBL LBL LBL LBL {: outer guards next done bad :}
   VDESC-LABEL @ LBL,
...
: ROW-END ( n n n n n -- ) {: count stride cur total bad :}
   16 stride MOVZ,
   15 count 16 MUL,
...
: VALIDATE-WF ( n -- ) {: bad :}
   LBL LBL LBL LBL LBL LBL LBL {: loop ordered flags fetch advance done xdone :}
...
: VALIDATE-FETCHES ( n -- ) {: bad :}
   LBL LBL LBL LBL {: loop scan linked done :}
   5 11 WF-COUNT-CELL cells LDR,
   6 11 BIND-COUNT-CELL cells LDR,
...
variable P2SK
: P2W-ENTRY ( n n n n n n -- ) {: lmain kwvar kwlen k hxt :}
   LBL P2SK !
   0 kwvar @ ADR,  1 kwlen MOVZ,  LKWCMP @ BL,
   0 P2SK @ CBZ,
...
: P2F-ENTRY ( n n n n n -- ) {: lmain kwvar kwlen hxt :}
   LBL P2SK !
   0 kwvar @ ADR,  1 kwlen MOVZ,  LKWCMP @ BL,
   0 P2SK @ CBZ,
...
: EM-ADT-CON-VAR ( n -- ) {: lmain :}
   LBL LBL LBL {: vmsg vok nox :}
   LBCAP @ BL,                          \ operand reaches the checker's body too
   2 5 MOVZ,  LPROT @ BL,               \ region -> RX: checker-call window
...
: EM-ADT-MATCH-OF ( n -- ) {: lmain :}
   LBL LBL LBL {: emsg eok noxm :}
   LBCAP @ BL,
   ...
   9 C-CALL-MOVZ-X16 LIT64,  14 14 5 LSLI,  9 9 14 ORR,  LCEMIT @ BL,   \ movz x16,#tag
   $F85F8269 C-EMITW
   $EB10013F C-EMITW
   $9A9F17E9 C-EMITW
   C-PUSHCP
   ...
   8 $D1000273 LIT64,  7 14 3 LSLI,  LADDSUBIMM @ BL,   \ sub x19,x19,#(8*(1+pads))
   14 DATA CMBK-CELL LDR,  14 14 1 LSLI,  14 14 1 ORRI,  14 DATA CMBK-CELL STR,
   12 0 MOVZ,  12 DATA CMM-CELL STR,
   lmain B, ;

: EMIT-COMPILE-ADT-MODE ( n -- ) {: lmain :}
   LBL LBL LBL LBL LBL {: s2 s3 s4 s5 off :}
...
: EMIT-EVAL-THROW-RECOVER ( -- )
   ...
   loop LBL,
      ...
      14 13 24 LDR,
      11 pop CBZ,
      11 14 CMP,  C-CC deliver BCOND,
   pop LBL,
      14 13 STACK-ABI:EVAL-BASE LDR, 10 13 STACK-ABI:EVAL-CAP LDR,
      12 13 32 LDR, 0 bad STACK-GUARD:CHECK-CURSOR
      14 DATA STACK-ABI:BASE-CELL STR,
...
: EMIT-LABEL-JIT ( -- )
   ...
   LBL LVSPILL !  LBL LVLITPUSH !  LBL LVPUSHC !  LBL LVPUSHT !
