...
\ place to look, and the one file an arch's pool definition lives in:
...
\     arm64 today: x9..x15 (scratch the engine never holds across BL, because
\     LVSPILL runs before every call) + x29 (no frame-pointer use anywhere).
\     Never: x16/x17 (call-chain veneers), x18 (Apple-reserved), x19..x28
\     (engine runtime registers), x30 (LR).
...
\   jit.fs: LVSPILL (reset to VRALL), LVDROP/LVNIPX/LVBINPREP/LVPUSHR/LVRECON
\   (free/claim via LVBIT)   forth.fs: the WHILE and J-REPEAT VRALL stores
...
: PK# ( x -- n ) 0 BEGIN over 0= 0= WHILE 1 + swap 8 rshift swap REPEAT nip ;
