...
;;; ============================================================
;;; Part 1: ARM64 Instruction Encoders (asm-*)
;;; ============================================================
...
(defun and-imm (rd rn n immr imms)
...
(defun ldrb-offset (rt rn offset)
...
(defun load-addr (rd addr)
  (let* ((lo16 (logand addr #xFFFF))
         ...
         (base (movz rd lo16))
         (p1 (if (> hi16 0) (movk rd hi16 16) nil))
         (r1 (append base p1))
         (p2 (if (> hi32 0) (movk rd hi32 32) nil))
         (r2 (append r1 p2))
         (p3 (if (> hi48 0) (movk rd hi48 48) nil)))
    (append r2 p3)))
      ...
      (let ((store-code (store-chars chars 0 nil)))
        (append-all
         (list
          ...
          (movz 1 alloc-size)
          (add-reg 28 28 1)                ; x28 += alloc size
...
(defun save-temp (depth)
  ...
  (let ((reg (temp-register depth)))
    (if reg
        (mov-reg reg 0)            ; MOV xN, x0
        (str-offset 0 31 (temp-slot depth)))))  ; STR x0, [sp, #off]
  ...
  (let ((reg (temp-register depth)))
    (if reg
        (if (= dest-reg reg)
            ...
            (mov-reg dest-reg reg)) ; MOV dest, xN
...
(defun prologue ()
  ...
  (append
   (sub-imm 31 31 #x1000)   ; SUB sp, sp, #4096 (allocate stack frame)
   (stp-offset 29 30 31 0)  ; STP x29, x30, [sp, #0]
   (stp-offset 19 20 31 16) ; STP x19, x20, [sp, #16]
   (stp-offset 21 22 31 32) ; STP x21, x22, [sp, #32]
   (stp-offset 23 24 31 48) ; STP x23, x24, [sp, #48]
   (mov-reg 19 0)           ; MOV x19, x0 (save runtime table)
   (add-imm 20 31 #x180)))  ; ADD x20, sp, #384 (env-base)

(defun epilogue ()
  (append
   (ldp-offset 23 24 31 48) ; LDP x23, x24, [sp, #48]
   (ldp-offset 21 22 31 32) ; LDP x21, x22, [sp, #32]
   (ldp-offset 19 20 31 16) ; LDP x19, x20, [sp, #16]
   (ldp-offset 29 30 31 0)  ; LDP x29, x30, [sp, #0]
   (add-imm 31 31 #x1000)    ; ADD sp, sp, #4096 (deallocate stack)
   (ret)))
    ...
    (cond
      ...
      (left-may-call
       (let* ((xs (temp-slot td))
         ...
         (append-all
          (list (str-offset 24 31 xs)   ; save x24
                lc                          ; eval left -> x0
                (str-offset 0 31 ls)    ; save left value (must use stack)
                (ldr-offset 24 31 xs)   ; restore x24
                rc                          ; eval right -> x0
                (mov-reg 1 0)           ; x1 = right
                (ldr-offset 0 31 ls)    ; x0 = left
                op-instrs))))
      ...
      (right-may-call
       (let* ((ls (temp-slot td))
         ...
         (append-all
          (list lc                          ; eval left -> x0
                (str-offset 0 31 ls)    ; save left value at temp[td]
  ...
  (cond
       ...
       (cond
         ...
         ((and (has-tag left 'var) (has-tag right 'lit)
          ...
          (let ((var-code (codegen left rtaddrs fnoffs td))
            ...
            (append var-code (add-imm 0 0 imm))))
         ...
         ((and (has-tag left 'lit) (has-tag right 'var)
          ...
          (let ((var-code (codegen right rtaddrs fnoffs td))
            ...
            (append var-code (add-imm 0 0 imm))))
         ;; General case
         (t (codegen-binop left right (add-reg 0 0 1) rtaddrs fnoffs td)))))
    ((has-tag ir 'sub)
     ...
     (let ((left (cadr ir))
       ...
       (if (and (has-tag left 'var) (has-tag right 'lit)
           ...
           (let ((var-code (codegen left rtaddrs fnoffs td))
                 (imm (ash (cadr right) 4)))
             (append var-code (sub-imm 0 0 imm)))
           (codegen-binop left right (sub-reg 0 0 1) rtaddrs fnoffs td))))
    ((has-tag ir 'mul)
     ...
     (codegen-binop (cadr ir) (caddr ir)
                       (append-all (list (lsr-imm 1 1 4)    ; untag right only
                                            (mul-reg 0 0 1)))  ; (left<<4) * right = result<<4
                       rtaddrs fnoffs td))
    ((has-tag ir 'band)
     (codegen-binop (cadr ir) (caddr ir) (and-reg 0 0 1) rtaddrs fnoffs td))
    ((has-tag ir 'bor)
     (codegen-binop (cadr ir) (caddr ir) (orr-reg 0 0 1) rtaddrs fnoffs td))
    ((has-tag ir 'bxor)
     (codegen-binop (cadr ir) (caddr ir) (eor-reg 0 0 1) rtaddrs fnoffs td))
    ((has-tag ir 'bsh)
     ...
     (let* ((val-ir (cadr ir))
            ...
            (shift-code (append-all
                         (list (asr-imm 1 0 4)
                               ...
                               (lsl-imm 0 0 4)))))
       (if may-call
           (append-all (list (str-offset 24 31 xs) vc (lsr-imm 0 0 4)
                                (str-offset 0 31 vs) (ldr-offset 24 31 xs)
                                ac shift-code))
           (append-all (list vc (lsr-imm 0 0 4) (str-offset 0 31 vs)
                                ac shift-code)))))
    ((has-tag ir 'cmp-eq)
     (codegen-binop (cadr ir) (caddr ir)
                       (append-all (list (cmp-reg 0 1)
                                            (cset 0 (cond-eq))
                                            (lsl-imm 0 0 4)))
                       rtaddrs fnoffs td))
    ((has-tag ir 'cmp-lt)
    ...
    ((has-tag ir 'cons-ir)
     ...
     (let* ((car-ir (cadr ir))
            ...
            (alloc-code (append-all
                         (list (mov-reg 1 0)             ; x1 = cdr value
                               (ldr-offset 0 31 cs)      ; x0 = car value
                               (str-offset 0 28 0)       ; [x28+0] = car
                               (str-offset 1 28 8)       ; [x28+8] = cdr
                               (mov-reg 0 28)            ; x0 = untagged ptr
                               (add-imm 28 28 16)        ; bump heap by 16
                               (movz 1 1)                ; x1 = 1
                               (orr-reg 0 0 1)))))       ; x0 = ptr | 1
       (if may-call
           (append-all (list (str-offset 24 31 xs) cc (str-offset 0 31 cs)
                                (ldr-offset 24 31 xs) dc alloc-code))
           (append-all (list cc (str-offset 0 31 cs) dc alloc-code)))))
    ((has-tag ir 'car-ir)
       ...
       (let ((ac (codegen arg-ir rtaddrs fnoffs td)))
         (append-all
          (list ac
                ...
                (movz 1 #xFFF0)                ; x1 = mask (keep upper bits)
                (movk 1 #xFFFF 16)             ; complete mask
                (movk 1 #xFFFF 32)
                (movk 1 #xFFFF 48)
                (and-reg 0 0 1)                ; x0 = ptr with tag cleared
                (ldr-offset 0 0 0))))))        ; x0 = [ptr+0] = car
    ((has-tag ir 'cdr-ir)
    ...
    ((has-tag ir 'write-file-ir)
     ...
     (let* ((path-ir (cadr ir))
            ...
            (sp (str-offset 0 31 xs))
            (cc (codegen contents-ir rtaddrs fnoffs nd))
            (m1 (mov-reg 1 0))
            (lp (ldr-offset 0 31 xs))
            (lf (ldr-offset 9 19 376))
            (bl (blr 9)))
     ...
     (let* ((str-ir (cadr ir))
       ...
       (append-all
        (list sc
              ...
              (movk 1 #xFFFF 32)
              (movk 1 #xFFFF 48)
              (and-reg 0 0 1)              ; x0 = str_ptr (untagged)
              ;; Load length from [x0+0]
              (ldr-offset 0 0 0)           ; x0 = raw length
    ...
    ((has-tag ir 'string-ref-ir)
     ...
     (let* ((str-ir (cadr ir))
            ...
            (si (str-offset 0 31 is)))
       ;; After codegen: idx saved at [sp+is], str at [sp+xs]
       (append-all
        (list sc sv ic si
              ;; Load str -> x1
              (ldr-offset 1 31 xs)         ; x1 = str (tagged)
              ;; Clear tag: x1 = x1 & ~0xF (same approach as car-ir)
              (movz 2 #xFFF0)              ; x2 = mask (keep upper bits)
              (movk 2 #xFFFF 16)           ; complete mask
              (movk 2 #xFFFF 32)
              (movk 2 #xFFFF 48)
              (and-reg 1 1 2)              ; x1 = str_ptr (untagged)
              ;; Load idx -> x0
              (ldr-offset 0 31 is)         ; x0 = idx (tagged)
              ;; Calculate offset: x0 = (idx >> 4) + 8
              (lsr-imm 0 0 4)              ; x0 = untagged idx
              (add-imm 0 0 8)              ; x0 = offset = 8 + idx
              ;; Load byte from str_ptr + offset
              (ldrb-reg 0 1 0)             ; x0 = byte value (zero-extended)
              ;; Tag as fixnum: x0 = x0 << 4
              (lsl-imm 0 0 4)))))
    ...
    ((has-tag ir 'string-equal-ir)
     ...
     (let* ((str1-ir (cadr ir))
       ...
       (append-all
        (list s1 sp s2
              ;; x2 = str2 base (untagged)
              (and-imm 2 0 1 #x3C #x3B)    ; x2 = str2 & ~0xF
              ;; x1 = str1 base (untagged)
              (ldr-offset 0 31 xs)         ; x0 = str1 (tagged)
              (and-imm 1 0 1 #x3C #x3B)    ; x1 = str1 & ~0xF
              ;; Load lengths
              (ldr-offset 3 1 0)           ; x3 = len1
              (ldr-offset 4 2 0)           ; x4 = len2
              ;; Compare lengths
              (cmp-reg 3 4)                ; cmp len1, len2
              (b-cond (cond-ne) 56)     ; if len1 != len2, jump to return_false (+14 instructions = 56 bytes)
              ;; Lengths equal, setup for loop
              ;; x1 = str1 data = x1 + 8
              (add-imm 1 1 8)              ; x1 = str1 data start
              ;; x2 = str2 data = x2 + 8
              (add-imm 2 2 8)              ; x2 = str2 data start
              ;; x4 = loop counter = 0
              (movz 4 0)                   ; x4 = 0
              ;; loop_start: (offset here, instruction 5)
              (cmp-reg 4 3)                ; cmp counter, len
              (b-cond (cond-ge) 28)     ; if counter >= len, jump to return_true (+7 instructions = 28 bytes)
              ;; Load bytes from both strings
              (ldrb-reg 5 1 4)             ; x5 = str1[counter]
              (ldrb-reg 6 2 4)             ; x6 = str2[counter]
              ;; Compare bytes
              (cmp-reg 5 6)                ; cmp char1, char2
              (b-cond (cond-ne) 20)     ; if char1 != char2, jump to return_false (+5 instructions = 20 bytes)
              ;; Increment counter
              (add-imm 4 4 1)              ; x4++
              ...
              (b-offset 8)                 ; skip return_false (+2 instructions = 8 bytes)
     ...
     (let* ((vec-ir (cadr ir))
       ...
       (append-all
        (list vc sv ic si vlc
              ...
              (ldr-offset 1 31 xs)         ; x1 = vec (tagged)
              (ldr-offset 2 31 xs2)        ; x2 = idx (tagged)
              ;; Clear tag from vec: x1 = x1 & ~0xF
              (and-imm 1 1 1 #x3C #x3B)    ; x1 = vec_ptr (untagged, clear low 4 bits)
     ...
     (let* ((vec-ir (cadr ir))
       ...
       (append-all
        (list vc
              ;; x0 = vec (tagged)
              ;; Clear tag: x0 = x0 & ~0xF
              (and-imm 0 0 1 #x3C #x3B)    ; x0 = vec_ptr (untagged)
              ;; Load length: x0 = [x0+0]
     ...
     (let* ((vec-ir (cadr ir))
       ...
       (append-all
        (list vc sv ic
              ...
              (lsr-imm 0 0 4)              ; x0 = idx_untagged (byte offset)
              (add-imm 0 0 8)              ; x0 = offset = 8 + byte_index
              ;; Load byte from vec_ptr + offset
              (add-reg 1 1 0)              ; x1 = address
              (ldrb-offset 0 1 0)          ; x0 = byte (zero-extended to 64-bit)
              ;; Tag as fixnum
              (lsl-imm 0 0 4)              ; x0 = tagged fixnum
              ))))
    ...
    ((has-tag ir 'make-string-from-vector-ir)
     ...
     (let* ((vec-ir (cadr ir))
       ...
       (append-all
        (list vc
              ;; x1 = untagged vec base
              (and-imm 1 0 1 #x3C #x3B)    ; x1 = vec & ~0xF
              ;; x5 = vec length (raw)
              ...
              (and-imm 4 4 1 #x3C #x3B)    ; x4 = (len + 23) & ~15 (clear low 4 bits)
     ...
     (let* ((buf-ir (cadr ir))
       ...
       (append-all
        (list
         ...
         (and-imm 1 1 1 #x3C #x3B)       ; x1 = buf & ~0xF (clear tag)
         (add-imm 1 1 8)                 ; x1 = buf + 8 (skip length header)
         ;; Allocate string: store length at [x28]
         (str-offset 5 28 0)             ; [x28+0] = length
         ;; x4 = alloc size = (8 + len + 15) & ~15 for 16-byte alignment
         (add-imm 4 5 23)                ; x4 = len + 23 (= len + 8 + 15)
         (and-imm 4 4 1 #x3C #x3B)       ; x4 = (len + 23) & ~15
         ;; Save string ptr (will be result), bump heap
         (mov-reg 0 28)                  ; x0 = string base (untagged)
         (add-reg 28 28 4)               ; x28 += alloc_size
         ;; x2 = string data base = x0 + 8
         ...
         (ldrb-offset 4 4 0)             ; x4 = byte at [x4]
         ;; Store byte: str_data[x3] = x4
         (strb-reg 4 2 3)                ; [x2 + x3] = x4 (byte)
         ;; x3++
         (add-imm 3 3 1)                 ; x3++
     ...
     (let* ((str-ir (cadr ir))
       ...
       (append-all
        (list
         ...
         str-code
         (str-offset 0 31 str-slot)

         ;; Get next-id from x27[0]
         (ldr-offset 3 27 0)  ; x3 = next-id (untagged)

         ;; Create (id . table) cons
         ;; id = x3 << 4 (tag as fixnum)
         (lsl-imm 4 3 4)      ; x4 = id as fixnum
         ;; table = [x27+8]
         (ldr-offset 5 27 8)  ; x5 = current table
         ...
         (mov-reg 6 28)
         (movz 9 1)
         (orr-reg 6 6 9)      ; x6 = id-next cons
         (add-imm 28 28 16)   ; bump heap

         ;; Create outer cons: (name . id-next)
         ;; name = input string
         (ldr-offset 0 31 str-slot)
         (str-offset 0 28 0)  ; [x28+0] = name
         (str-offset 6 28 8)  ; [x28+8] = id-next cons
         ;; Tag as cons
         (mov-reg 7 28)
         (orr-reg 7 7 9)      ; x7 = new entry cons
         (add-imm 28 28 16)   ; bump heap
         ...
         (movz 11 #xF)
         (bic-reg 0 4 11)     ; clear fixnum tag
         (movz 9 2)
         (orr-reg 0 0 9)))))  ; tag as symbol
     ...
     (let* ((sym-ir (cadr ir))
       ...
       (append-all
        (list
         ...
         (lsr-imm 1 0 4)           ; x1 = symbol ID (untagged)
         ;; Get table: x2 = x27[8]
         (ldr-offset 2 27 8)       ; x2 = table (list of entries)
         ;; Load mask for clearing tag bits
         (movz 11 #xF)             ; x11 = 0xF (tag mask)
         ;; loop:
         ;; Check if nil (x2 == 0)
         (cmp-imm 2 0)
         (b-cond (cond-eq) 48)  ; if nil, jump to end (+12 instructions = 48 bytes)
         ;; Get entry: x2 is cons (entry . rest), untag to get pointer
         (bic-reg 3 2 11)          ; x3 = entry pointer (untagged)
         ;; Get id-next: (cdr entry) = [x3+8]
         (ldr-offset 4 3 8)        ; x4 = (id . rest) cons
         ;; Untag and get id: (car x4) = [x4-1] after untagging
         (bic-reg 4 4 11)          ; x4 = pointer to (id . rest)
         (ldr-offset 5 4 0)        ; x5 = id (as fixnum, so id << 4)
         (lsr-imm 5 5 4)           ; x5 = id (untagged)
         ;; Compare: x5 == x1?
         (cmp-reg 5 1)
         (b-cond (cond-eq) 12)  ; if match, jump to found (+3 instructions = 12 bytes)
         ;; Not match, advance: x2 = (cdr entry) = [x3+8], then cdr of that = [x4+8]
         (ldr-offset 2 4 8)        ; x2 = rest of table
         (b-offset -44)            ; back to loop start (11 instructions = -44 bytes)
         ;; found: return (car entry) = [x3+0] (the name string)
         (ldr-offset 0 3 0)        ; x0 = name string
         ;; skip to end (past the nil case)
         (b-offset 8)              ; skip past nil case (branch + movz = 8 bytes)
    ...
    ((has-tag ir 'nthcdr-ir)
     ...
     (let* ((n-ir (cadr ir))
            ...
            (sn (str-offset 0 31 xs))
            (lc (codegen list-ir rtaddrs fnoffs nd))
            (ml (mov-reg 1 0))
            (ln (ldr-offset 2 31 xs))
            (asr (asr-imm 2 2 4))
            (cm (cmp-imm 2 0))
            (be (b-cond (cond-le) 28))
            (m0 (mov-reg 0 1))
            (lf (ldr-offset 9 19 16))
            (bl (blr 9))
            (m1 (mov-reg 1 0))
            (si (sub-imm 2 2 1))
            (bk (b-offset -20))
                 ...
                 (labels ((eval-vals (vs idx acc)
                            (if (null vs)
                                ...
                                (let* ((vc (codegen (car vs) rtaddrs fnoffs nd))
                                       ...
                                       (sv (str-offset 0 31 slot)))
                                  (eval-vals (cdr vs) (+ idx 1)
                                             (append-all (list acc vc sv)))))))
                   (let* ((evc (eval-vals irs 0 nil))
                          (l0 (if (> cnt 0) (ldr-offset 1 31 (temp-slot td)) (movz 1 0)))
                          (l1 (if (> cnt 1) (ldr-offset 2 31 (temp-slot (+ td 1))) (movz 2 0)))
                          (l2 (if (> cnt 2) (ldr-offset 3 31 (temp-slot (+ td 2))) (movz 3 0)))
                          (l3 (if (> cnt 3) (ldr-offset 4 31 (temp-slot (+ td 3))) (movz 4 0)))
                          (ct (ash cnt 4))
                          (mc (movz 0 ct))
                          (lf (ldr-offset 9 19 136))
                          (bl (blr 9))
                          (lv (ldr-offset 0 31 (temp-slot td))))
     ...
     (let* ((vars (cadr ir))
       ...
       (labels ((bind-vars (idx acc)
                  (if (>= idx nvars)
                      ...
                      (let* ((mi (movz 0 idx))
                             (lp (ldr-offset 1 31 xs))
                             (lf (ldr-offset 9 19 144))
                             (bl (blr 9))
                             ;; Store in env frame: sub x1, x20, offset; str x0, [x1]
                             (env-off (* idx 8))
                             (s1 (sub-imm 1 20 env-off))
                             (sv (str-offset 0 1 0)))
                        (bind-vars (+ idx 1)
                   ...
                   (let ((else-bytes (code-size elc)))
                     (append-all
                      (list tc
                            (movz 1 0)
                            (cmp-reg 0 1)
                            (b-cond (cond-eq) (+ then-bytes 8))  ; Skip then + B + self
                            thc
                            (b-offset (+ else-bytes 4))  ; Skip else + landing
                            elc)))))))))))
    ((has-tag ir 'let-ir)
     ;; let-ir = (let-ir vals bir count offs)
     (let* ((vals (cadr ir))
            ...
            (xs (temp-slot td))
            (nd (+ td 1))
            (acc (str-offset 24 31 xs)))
       (labels ((gb (vs os a)
                  (if (null vs) a
                      (let* ((vc (codegen (car vs) rtaddrs fnoffs nd))
                             (s1 (sub-imm 1 20 (* (car os) 8)))
                             (s2 (str-offset 0 1 0))
                             (st (append s1 s2))
                             (ld (ldr-offset 24 31 xs))
                             (t1 (append a ld))
                             (t2 (append t1 vc))
                             (t3 (append t2 st)))
                        (gb (cdr vs) (cdr os) t3)))))
         (let* ((body-code (gb vals offs nil))
                (final-ld (ldr-offset 24 31 xs))
    ...
    ((has-tag ir 'call-fn)
     ...
     (let* ((fnm (cadr ir))
       ...
       (labels ((ga (as i a)
                  ...
                  (if (null as) a
                      (let* ((rs (if (> i 0) (ldr-offset 24 31 xs) nil))
                             (ac (codegen (car as) rtaddrs fnoffs nd))
                             (st (str-offset 0 31 (spill-slot td i)))
                             (t1 (append a rs))
                             (t2 (append t1 ac))
                             (t3 (append t2 st)))
                        (ga (cdr as) (+ i 1) t3))))
                (gl-reg (i a)
                  ...
                  (if (>= i (min na 8)) a
                      (let* ((adjusted-off (+ (spill-slot td i) stack-space))
                             (ld (ldr-offset i 31 adjusted-off))
                             (t1 (append a ld)))
                        (gl-reg (+ i 1) t1))))
                (store-stack-args (i a)
                  ...
                  (if (>= i na) a
                      (let* ((adjusted-off (+ (spill-slot td i) stack-space))
                             (ld (ldr-offset 0 31 adjusted-off))
                             (stack-off (* (- i 8) 8))
                             (st (str-offset 0 31 stack-off))
                             (t1 (append a ld))
                             (t2 (append t1 st)))
                        (store-stack-args (+ i 1) t2)))))
         (let* ((save-x24 (str-offset 24 31 xs))
                (args-code (ga airs 0 nil))
                (restore-x24 (ldr-offset 24 31 xs))
                ;; Allocate stack space for args 8+ (if any)
                (alloc-stack (if (> stack-args 0)
                                 (sub-imm 31 31 stack-space)
                                 nil))
     ...
     (let* ((fnm (cadr ir))
       ...
       (labels ((ga (as i a)
                ...
                (gl-reg (i a)
                  ...
                  (if (>= i (min na 8)) a
                      (let* ((ld (ldr-offset i 31 (spill-slot td i)))
                             (t1 (append a ld)))
                        (gl-reg (+ i 1) t1)))))
         (let* ((save-x24 (str-offset 24 31 xs))
                (args-code (ga airs 0 nil))
       ...
       (labels ((eval-args (args idx acc)
                  (if (null args)
                      ...
                      (let* ((arg-code (codegen (car args) rtaddrs fnoffs nd))
                             (store (str-offset 0 31 (spill-slot td idx))))
                ...
                (copy-to-params (idx acc)
                  ...
                  (if (>= idx nargs)
                      acc
                      (let* ((load (ldr-offset 0 31 (spill-slot td idx)))
                             (param-addr (sub-imm 1 20 (* idx 8)))
                             (store (str-offset 0 1 0)))
    ...
    ((has-tag ir 'lambda-ref)
     ...
     (let* ((name (cadr ir))
            ...
            (code-slot (temp-slot td))
            (env-slot (temp-slot (+ td 1))))
       (if (= capture-count 0)
           ...
           (let ((tagged-offset (ash offset-bytes 4)))  ; tag as fixnum
             (append-all
              (list
               ...
               (str-offset 9 28 0)           ; [x28+0] = car = fn-offset
               ;; Store nil in [x28+8]
               (movz 10 0)                   ; x10 = nil
               (str-offset 10 28 8)          ; [x28+8] = cdr = nil
               ;; Result = x28 | 5 (closure tag)
               (mov-reg 0 28)                ; x0 = x28
               (movz 9 5)                    ; x9 = closure tag
               (orr-reg 0 0 9)                   ; x0 = x28 | 5
                             ...
                             (if (null offs)
                                 ...
                                 (let* ((off (car offs))
                                        ...
                                        (load-cap
                                         (append-all
                                          (list
                                           (sub-imm 1 20 (* off 8)) ; x1 = &captured
                                           (ldr-offset 0 1 0)       ; x0 = captured value
                                           (str-offset 0 31 val-slot)))) ; save value
                                        ;; Allocate cons: (value . prev-env)
                                        (alloc-cons
                                         (append-all
                                          (list
                                           (ldr-offset 9 31 val-slot)  ; car = captured value
                                           (str-offset 9 28 0)         ; [x28+0] = car
                                           ;; cdr = previous env acc
                                           (if (null env-acc)
                                               (movz 9 0)              ; first: cdr = nil
                                               (ldr-offset 9 31 env-acc)) ; else: load prev env
                                           (str-offset 9 28 8)         ; [x28+8] = cdr
                                           ;; Result = x28 | 1 (cons tag)
                                           (mov-reg 0 28)
                                           (movz 9 1)
                                           (orr-reg 0 0 9)                 ; x0 = cons ptr
                                           ;; Save and bump
             ...
             (let* ((env-code (car capture-code))
               ...
               (append-all
                (list
                 ...
                 (str-offset 9 28 0)             ; [x28+0] = car
                 (ldr-offset 9 31 env-result-slot) ; cdr = env cons list
                 (str-offset 9 28 8)             ; [x28+8] = cdr
                 ;; Result = x28 | 5 (closure tag)
                 (mov-reg 0 28)
                 (movz 9 5)
    ...
    ((has-tag ir 'funcall-ir)
     ...
     (let* ((fn-ir (cadr ir))
            ...
            (closure-slot (temp-slot (+ td 3)))
            (code-slot (temp-slot (+ td 4)))
            (env-slot (temp-slot (+ td 5)))
            (arg-base (+ td 6))
       ...
       (labels ((gen-args (airs idx acc)
                  (if (null airs)
                      acc
                      (let* ((rs (if (> idx 0) (ldr-offset 24 31 x24-slot) nil))
                             (ac (codegen (car airs) rtaddrs fnoffs nested-td))
                             (st (str-offset 0 31 (temp-slot (+ arg-base idx)))))
                ...
                (load-reg-args (idx total-offset acc)
                  ...
                  (if (>= idx (min num-args 8))
                      ...
                      (let* ((adjusted-off (+ (temp-slot (+ arg-base idx)) total-offset))
                             (ld (ldr-offset idx 31 adjusted-off)))
         ...
         (let ((total-offset (+ stack-space param-space)))
           (append-all
            (list
             ;; Save x24 and x20
             (str-offset 24 31 x24-slot)
             (str-offset 20 31 x20-slot)
             ;; Evaluate closure into x0
             fn-code
             ;; Clear closure tag (5) to get heap address: x9 = x0 & ~0xF
             (movz 11 #xF)                     ; x11 = 0xF
             (bic-reg 9 0 11)                  ; x9 = x0 & ~0xF
             ...
             (add-reg 10 26 10)
             (str-offset 10 31 code-slot)      ; save code address
             ;; Load cdr = env: x11 = [x9+8]
             (ldr-offset 11 9 8)
             (str-offset 11 31 env-slot)       ; save env
             ...
             (sub-imm 31 31 param-space)
             ;; Set x20 for lambda's param-stores: x20 = sp + param-space - 8
             (if (> param-space 8)
                 (add-imm 20 31 (- param-space 8))
                 (mov-reg 20 31))  ; If param-space <= 8, set x20 = sp
     ...
     (let* ((var (cadr ir))
       ...
       (append-all
        (list
         ...
         ;; Increment counter
         (ldr-offset 0 31 counter-slot)
         (add-imm 0 0 #x10)  ; add tagged 1
         (str-offset 0 31 counter-slot)
         ;; Branch back to loop start
         ;; Distance: -(loop test (4) + store var (4) + body + incr (3))
         (b-offset (- (* (+ body-instrs 11) 4)))
     ...
     (let* ((var (cadr ir))
       ...
       (append-all
        (list
         ...
         (ldr-offset 0 31 list-slot)
         (ldr-offset 9 19 16)  ; cdr function at offset 16
         (blr 9)
         (str-offset 0 31 list-slot)
         ;; Branch back to loop start
         ;; Distance: -(null check (3) + get car (5) + body + get cdr (4))
         (b-offset (- (* (+ body-instrs 12) 4)))
     ...
     (let* ((fd-ir (cadr ir))
       ...
       (append-all
        (list fd-code save-fd buf-code save-buf len-code save-len
              ...
              (and-imm 1 1 1 #x3C #x3B)            ; clear string tag, get ptr
     ...
     (let* ((fd-ir (cadr ir))
       ...
       (append-all
        (list fd-code save-fd buf-code save-buf len-code save-len
              ...
              (lsr-imm 0 0 4)                      ; untag fd
              (ldr-offset 1 31 (temp-slot (+ td 1)))
              (and-imm 1 1 1 #x3C #x3B)            ; clear vector tag
              (add-imm 1 1 8)                      ; skip length field
              (ldr-offset 2 31 (temp-slot (+ td 2)))
              (lsr-imm 2 2 4)                      ; untag len
     ...
     (let* ((path-ir (cadr ir))
       ...
       (append-all
        (list path-code save-path flags-code save-flags mode-code save-mode
              (ldr-offset 0 31 (temp-slot td))
              (and-imm 0 0 1 #x3C #x3B)            ; clear string tag
      ...
      (let* ((frame-size (if leaf #x1000 #x1000))  ; Must match fn-prologue - now 4KB for all functions
             (st (if (< idx 8)
                     ...
                     (let ((stack-off (+ frame-size (* (- idx 8) 8))))
                       (append (ldr-offset 22 31 stack-off)
                               (sub-imm 21 20 (* (+ base idx) 8))
                               (str-offset 22 21 0))))))
...
(defun fn-prologue (frame-size x20-offset &key leaf)
  ...
  (if leaf
      ...
      (append
       (sub-imm 31 31 frame-size)   ; SUB sp, sp, #frame-size
       (stp-offset 20 30 31 0)      ; STP x20, lr, [sp, #0] (save x20 and return addr)
       (add-imm 20 31 x20-offset))  ; ADD x20, sp, #x20-offset (env base)
      ;; Non-leaf function: full frame with x24 save
      (append
       (sub-imm 31 31 frame-size)   ; SUB sp, sp, #frame-size (allocate function frame)
       (stp-offset 20 30 31 0)      ; STP x20, lr, [sp, #0] (save caller's x20 and return addr)
       (str-offset 24 31 16)        ; STR x24, [sp, #16] (save caller's closure env)
       (add-imm 20 31 x20-offset)))) ; ADD x20, sp, #x20-offset (env base past spill area)
...
(defun fn-epilogue (frame-size &key leaf)
  ...
  (if leaf
      ...
      (append
       (ldp-offset 20 30 31 0)    ; LDP x20, lr, [sp, #0] (restore x20 and lr)
       (add-imm 31 31 frame-size))  ; ADD sp, sp, #frame-size (deallocate leaf frame)
      ;; Non-leaf function: full restore
      (append
       (ldr-offset 24 31 16)       ; LDR x24, [sp, #16] (restore caller's closure env)
       (ldp-offset 20 30 31 0)     ; LDP x20, lr, [sp, #0] (restore caller's x20 and lr)
       (add-imm 31 31 frame-size)))) ; ADD sp, sp, #frame-size (deallocate function frame)
  ...
  (if (null params)
      ...
      (let* ((temp-slot (+ 200 idx))
             ...
             (restore-code (append-all
                            (list
                             ;; Load from temp slot
                             (sub-imm 21 20 temp-off)
                             (ldr-offset 22 21 0)
                             ;; Store to final slot
                             (sub-imm 21 20 final-off)
                             (str-offset 22 21 0)))))
