    ...
    ((consp expr)
     (let ((op (car expr)))
       (cond
         ...
         ;;                 (make-string-from-vector vec)))
         ((eq op 'string-append)
          ;; Use gensyms for labels functions to avoid collisions, fixed names for variables
          (let ((copy1-fn (gensym "COPY1"))
                (copy2-fn (gensym "COPY2")))
            (nc-compile
             (list 'let* (list (list 's1 (cadr expr))
                               (list 's2 (caddr expr))
                               (list 'len1 (list 'string-length 's1))
                               (list 'len2 (list 'string-length 's2))
                               (list 'total (list '+ 'len1 'len2))
                               (list 'vec (list 'make-vector 'total)))
                   (list 'labels (list (list copy1-fn (list 'i)
                                             (list 'if (list '< 'i 'len1)
                                                   (list 'progn
                                                         (list 'vector-set 'vec 'i (list 'string-ref 's1 'i))
                                                         (list copy1-fn (list '+ 'i 1)))))
                                       (list copy2-fn (list 'i)
                                             (list 'if (list '< 'i 'len2)
                                                   (list 'progn
                                                         (list 'vector-set 'vec (list '+ 'len1 'i) (list 'string-ref 's2 'i))
                                                         (list copy2-fn (list '+ 'i 1))))))
                         (list copy1-fn 0)
                         (list copy2-fn 0)
                         (list 'make-string-from-vector 'vec)))
             env fenv)))
