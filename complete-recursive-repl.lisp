;;;; Complete Recursive REPL - v1.3 Features
;;;; Adds: variadic ops, let*, begin, append, reverse, length, nth, atom?, pair?, when, unless
...
;;; NEW: Additional type predicates
(defun pair? (x) (cons? x))
(defun null? (x) (nil? x))
(defun atom? (x) (if (cons? x) (quote nil) (quote 1)))
...
;;;; Evaluator with quote, if, let, let*, lambda, and, or, cond, begin
                      ...
                      (if (symbol=? first (make-symbol (quote "and")))
                        ...
                        (if (symbol=? first (make-symbol (quote "or")))
                          ...
                          (if (symbol=? first (make-symbol (quote "cond")))
                            ...
                            (if (symbol=? first (make-symbol (quote "begin")))
...
;;; NEW: let* - sequential binding
(defun eval-let* (args env)
  (let ((bindings (car args)))
    (let ((body (car (cdr args))))
      (let ((new-env (eval-bindings-seq bindings env)))
        (eval-expr body new-env)))))

(defun eval-bindings-seq (bindings env)
  (if (nil? bindings) env
    (let ((binding (car bindings)))
      (let ((sym (car binding)))
        (let ((val-expr (car (cdr binding))))
          (let ((val (eval-expr val-expr env)))
            (let ((new-env (env-extend sym val env)))
              (eval-bindings-seq (cdr bindings) new-env))))))))

;;; NEW: begin - sequential evaluation
(defun eval-begin (exprs env)
...
;;; NEW: Variadic arithmetic helpers
(defun add-all (vals)
  (if (nil? vals) (quote 0)
    (+ (car vals) (add-all (cdr vals)))))

(defun sub-all (first rest)
  (if (nil? rest) (- (quote 0) first)
    (- first (add-all rest))))

(defun mul-all (vals)
  (if (nil? vals) (quote 1)
    (* (car vals) (mul-all (cdr vals)))))

(defun div-all (first rest)
  (if (nil? rest) (/ (quote 1) first)
    (/ first (mul-all rest))))

;;; NEW: Variadic comparison helpers
(defun all-equal? (first rest)
  (if (nil? rest) (quote 1)
    (if (= first (car rest))
      (all-equal? first (cdr rest))
      (quote nil))))

(defun all-less? (prev rest)
  (if (nil? rest) (quote 1)
    (let ((curr (car rest)))
      (if (< prev curr)
        (all-less? curr (cdr rest))
        (quote nil)))))

(defun all-greater? (prev rest)
  (if (nil? rest) (quote 1)
    (let ((curr (car rest)))
      (if (> prev curr)
        (all-greater? curr (cdr rest))
        (quote nil)))))

(defun all-less-eq? (prev rest)
  (if (nil? rest) (quote 1)
    (let ((curr (car rest)))
      (if (<= prev curr)
        (all-less-eq? curr (cdr rest))
        (quote nil)))))

(defun all-greater-eq? (prev rest)
  (if (nil? rest) (quote 1)
    (let ((curr (car rest)))
      (if (>= prev curr)
        (all-greater-eq? curr (cdr rest))
        (quote nil)))))
...
(defun nth-helper (lst n)
  (if (= n (quote 0)) (car lst)
    (nth-helper (cdr lst) (- n (quote 1)))))

(defun eval-apply (op args env)
  ;;; NEW: Variadic arithmetic
  (if (symbol=? op (make-symbol (quote "+")))
    (let ((vals (eval-list args env)))
      (add-all vals))
    (if (symbol=? op (make-symbol (quote "-")))
      (let ((vals (eval-list args env)))
        (if (nil? vals) (quote 0)
          (sub-all (car vals) (cdr vals))))
      (if (symbol=? op (make-symbol (quote "*")))
        (let ((vals (eval-list args env)))
          (mul-all vals))
        (if (symbol=? op (make-symbol (quote "/")))
          (let ((vals (eval-list args env)))
            (if (nil? vals) (quote 1)
              (div-all (car vals) (cdr vals))))
          ;;; NEW: Variadic comparisons
          (if (symbol=? op (make-symbol (quote "=")))
            (let ((vals (eval-list args env)))
              (if (nil? vals) (quote 1)
                (all-equal? (car vals) (cdr vals))))
            (if (symbol=? op (make-symbol (quote "<")))
              (let ((vals (eval-list args env)))
                (if (nil? vals) (quote 1)
                  (if (nil? (cdr vals)) (quote 1)
                    (all-less? (car vals) (cdr vals)))))
              (if (symbol=? op (make-symbol (quote ">")))
                (let ((vals (eval-list args env)))
                  (if (nil? vals) (quote 1)
                    (if (nil? (cdr vals)) (quote 1)
                      (all-greater? (car vals) (cdr vals)))))
                (if (symbol=? op (make-symbol (quote "<=")))
                  (let ((vals (eval-list args env)))
                    (if (nil? vals) (quote 1)
                      (if (nil? (cdr vals)) (quote 1)
                        (all-less-eq? (car vals) (cdr vals)))))
                  (if (symbol=? op (make-symbol (quote ">=")))
                    (let ((vals (eval-list args env)))
                      (if (nil? vals) (quote 1)
                        (if (nil? (cdr vals)) (quote 1)
                          (all-greater-eq? (car vals) (cdr vals)))))
                    ;;; not operator
                    (if (symbol=? op (make-symbol (quote "not")))
                      (if (nil? (eval-expr (car args) env))
                        ...
                        (quote nil))
                      ;;; NEW: List operations
                      (if (symbol=? op (make-symbol (quote "append")))
                        (let ((vals (eval-list args env)))
                          (if (nil? vals) (quote nil)
                            (if (nil? (cdr vals)) (car vals)
                              (append-two (car vals) (car (cdr vals))))))
                        (if (symbol=? op (make-symbol (quote "reverse")))
                          (reverse-list (eval-expr (car args) env))
                          (if (symbol=? op (make-symbol (quote "length")))
                            (length-helper (eval-expr (car args) env) (quote 0))
                            (if (symbol=? op (make-symbol (quote "nth")))
                              (let ((vals (eval-list args env)))
                                (nth-helper (car (cdr vals)) (car vals)))
                              ;;; Original operators
                                ...
                                (if (symbol=? op (make-symbol (quote "car")))
                                  ...
                                  (if (symbol=? op (make-symbol (quote "cdr")))
                                    ...
                                    (if (symbol=? op (make-symbol (quote "list")))
                                      (eval-list args env)
                                      ;;; NEW: Type predicates as functions
                                      (if (symbol=? op (make-symbol (quote "atom?")))
                                        (if (atom? (eval-expr (car args) env)) (quote 1) (quote nil))
                                        (if (symbol=? op (make-symbol (quote "pair?")))
                                          (if (pair? (eval-expr (car args) env)) (quote 1) (quote nil))
                                          (if (symbol=? op (make-symbol (quote "null?")))
                                            (if (null? (eval-expr (car args) env)) (quote 1) (quote nil))
...
(defun repl-start ()
  (progn
    (print (quote "Habu REPL - Complete (v1.3)"))
    (println)
    (print (quote "Features: variadic ops, let*, begin, append, reverse, length, nth, atom?, pair?"))
    (println)
    (repl-loop (quote nil))))
