              ...
              (if (symbol=? first (make-symbol (quote "if")))
                ...
                (if (symbol=? first (make-symbol (quote "let")))
                  ...
                  (if (symbol=? first (make-symbol (quote "lambda")))
                    ...
                    (if (symbol=? first (make-symbol (quote "progn")))
...
(defun eval-progn (exprs env)
  (if (nil? exprs)
    ...
    (if (nil? (cdr exprs))
      ...
      (let ((dummy (eval-expr (car exprs) env)))
                  ...
                  (if (symbol=? op (make-symbol (quote "car")))
                    ...
                    (if (symbol=? op (make-symbol (quote "cdr")))
                      ...
                      (if (symbol=? op (make-symbol (quote "list")))
                        (eval-list args env)
                        (if (symbol=? op (make-symbol (quote "get-tag")))
                          (get-tag (eval-expr (car args) env))
                          (if (symbol=? op (make-symbol (quote "symbol=?")))
                            (if (symbol=? (eval-expr (car args) env)
                                         (eval-expr (car (cdr args)) env))
                              (quote 1)
                              (quote nil))
                            (if (symbol=? op (make-symbol (quote "make-symbol")))
                              (eval-expr (car args) env)
                              (if (symbol=? op (make-symbol (quote "write-file")))
                                (write-file (eval-expr (car args) env)
                                           (eval-expr (car (cdr args)) env))
                                (if (symbol=? op (make-symbol (quote "read-file")))
                                  (read-file (eval-expr (car args) env))
                                  (if (symbol=? op (make-symbol (quote "string-concat")))
                                    (habu-string-concat (eval-expr (car args) env)
                                                       (eval-expr (car (cdr args)) env))
                                    (if (symbol=? op (make-symbol (quote "fixnum-to-string")))
                                      (habu-fixnum-to-string (eval-expr (car args) env))
                                      (if (symbol=? op (make-symbol (quote "symbol-name")))
                                        (habu-symbol-name (eval-expr (car args) env))
...
;;; Build initial environment with helper functions
(defun make-initial-env ()
  ;;; Create environment with essential predicates and helpers
  ;;; We make these available by defining them as closures
  (let ((env (quote nil)))
    ;;; Add type predicates
    (let ((env2 (env-extend (make-symbol (quote "fixnum?"))
                           (cons (make-symbol (quote "closure"))
                                (cons (quote nil)
                                     (cons (cons (make-symbol (quote "x")) (quote nil))
                                          (cons (cons (make-symbol (quote "="))
                                                     (cons (cons (make-symbol (quote "get-tag"))
                                                                (cons (make-symbol (quote "x")) (quote nil)))
                                                          (cons (quote 0) (quote nil))))
                                               (quote nil)))))
                           env)))
      (let ((env3 (env-extend (make-symbol (quote "cons?"))
                             (cons (make-symbol (quote "closure"))
                                  (cons (quote nil)
                                       (cons (cons (make-symbol (quote "x")) (quote nil))
                                            (cons (cons (make-symbol (quote "="))
                                                       (cons (cons (make-symbol (quote "get-tag"))
                                                                  (cons (make-symbol (quote "x")) (quote nil)))
                                                            (cons (quote 1) (quote nil))))
                                                 (quote nil)))))
                             env2)))
        (let ((env4 (env-extend (make-symbol (quote "symbol?"))
                               (cons (make-symbol (quote "closure"))
                                    (cons (quote nil)
                                         (cons (cons (make-symbol (quote "x")) (quote nil))
                                              (cons (cons (make-symbol (quote "="))
                                                   ...
                                                   (quote nil)))))
                               env3)))
          (let ((env5 (env-extend (make-symbol (quote "nil?"))
                                 (cons (make-symbol (quote "closure"))
                                      (cons (quote nil)
                                           (cons (cons (make-symbol (quote "x")) (quote nil))
                                                (cons (cons (make-symbol (quote "="))
                                                           (cons (make-symbol (quote "x"))
                                                                (cons (quote 0) (quote nil))))
                                                     (quote nil)))))
                                 env4)))
            env5))))))
