...
MERGED 2026-08-04 (dot-groom). Duplicate dot habu-verify-effects-doc-c67960b0 (same
title, filed five minutes earlier) is closed into this one. Its one unique clause: the
verdict-spelling leg is specifically E-UNCHECKABLE *versus the old E-UNCHECKED spelling*,
so the fixture has to show which of the two the structured emitter produces, not merely
that some verdict is emitted. Path correction from the same pass: the spelling now lives
at src/core/render.f:586, not render.f:558. Everything else in the two dots was identical.
