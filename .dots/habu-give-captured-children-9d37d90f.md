...
Claim: alder, .jj-ws/alder-capture-stdin, base b4efad25. Own the default
captured-child stdin path and its focused regression; explicit stdin remains
owned by the caller. Coordinate any wider process API change before coding.

Implementation: every default capture spawn (path, argv, environment, cwd)
opens /dev/null into the existing capture-owned stdin descriptor slot, marks
it close-on-exec, and closes it after spawn or through capture cleanup on
failure. Explicit-stdin and raw-IO entry points are unchanged. The regression
keeps the parent's idle stdin pipe writer open and checks a bare engine exits
cleanly through all four default entry points. Unmodified code times out with
E-PROC-TIMEOUT (-2502); the changed library passes in both tiers.

Validation: process argv/env/cwd, runner, stdlib-process-fixtures and all
tool reader rows pass, with the following ordering requirement. Loading the
new NUL literal through the linker reproduces the existing pool leak:
HBT-SIZE-AOT sees one zero data byte. The pending pool-isolation commit
b2e149a6 fixes it unchanged: a fresh private b4efad25 product with that seam
and this patch passes the complete hb-build-fixtures row. Land after the
alias/pool pair. Astra review is clear. build-fixpoint-fixtures is deferred
to Hazel's controlled gate because it invokes the prohibited install --force;
no full gate or install was run here. Dot remains active until integration.
