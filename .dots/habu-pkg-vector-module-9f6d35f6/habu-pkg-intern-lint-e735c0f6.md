...
PARTIAL 2026-08-23 (lane habu-lint-intern-table-85ae462f, commit 83d08b5e): tools/lint/intern.f is now package LINT-INTERN (public: E-LINT-INTERN-CAP, INTERN, INTERN?, INTERN$, INTERN-FIND, INTERN-RESET, INTERN-FOLD, INTERN-FOLD?; everything else private), its five consumers import it with `using LINT-INTERN`, set-test reopens the package, and package-diff-lint accepts a body edit to INTERN - the whitespace-probe half of this leaf's acceptance is satisfied. NOT done: the raw-vector conversion ('zero raw vector references in the file') and the four recorded store defects (half-published entry on a failed second VEC:PUSH, non-recoverable INTERN-INIT-ONCE, three parallel ownership vectors with no disposal, bare-pointer VEC:INIT) - they stay this leaf's, still behind the design parent habu-own-nominal-linear-491d11e4. The parent's 'no leaf may be dispatched' rule was read before the lane ran: the minimal form moves a boundary and redesigns nothing, and each of the four recorded blockers was checked against it (none applies); the verbatim seven-blocker verdict (blackboard 20260727-154724.143-codex-da26) is no longer on disk.
