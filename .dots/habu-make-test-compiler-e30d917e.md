...
Claim: alder, .jj-ws/alder-native-suite-setup, base b4efad25.

Measured cause: STDLIB-GATE:SUITE-ARG+ prepends test/compiler/aot-mode.f
for native-* suites. Standalone loads stay at tier 0, so native-trap examines
tier-0 instruction shapes and native-rename-rows misses native elaboration's
width refusals. Reproduced four trap failures and three rename failures on a
private release-engine copy; both files pass unchanged with the gate prelude.
Each fixture now requires that same mode itself, before its definitions.
All assertions and the gate registry remain unchanged.

Validation: both standalone commands and both registry-row commands (aot-mode
plus the single suite file) pass on the private release copy. The expected
native-rename refusal diagnostics remain. Astra review: no findings. Logs:
/tmp/alder-native-suite-setup/{trap,rename}-{before,after,gate-before,gate-after}.log.
Hazel runs the full gate on integration; this change contains no engine code.
