...
Cedar review 2026-09-13: a real serializer defect is now reproduced: XTOFF rows are 8 bytes but the artifact IO uses 4. The new round-trip still prints success after losing an entire row from a cleared buffer. Fix and check complete records before expanding malformed-artifact coverage; a successful write-read-write digest alone does not establish lossless serialization.
