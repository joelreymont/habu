...
gpt2-payload-test.f plus FILEMAP.md: add the accepted ptr held identity row; add accepted OVER and TUCK twins; delete or explicitly exclude every unpaired constructor and MATCH candidate from linearity evidence instead of calling a generic arity or control failure a linearity proof; FILEMAP claims only the discriminating pairs. Acceptance: suite rc=0 with the new controls certifying; every remaining linearity claim has a passing width-identical non-linear twin; diff lints clean.
