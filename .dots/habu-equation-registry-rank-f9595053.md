...
2026-07-20 SERIALIZED behind the extroles lane (spark): same file (maki/spec.f validators); dispatch after habu-extent-roles-b-df9d232f merges.

2026-07-20 serialization released (extent-roles landed 2b6ad8f8).
Claim: agent=rank0reg workspace=.jj-ws/fable-rank0reg machine=spark (owns maki/spec.f validators/derive pipeline + spec-test.f; the cross-seq fixture lane owns its own NEW test file and must not touch spec.f)
