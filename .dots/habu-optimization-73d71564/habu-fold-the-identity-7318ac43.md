...
Added 2026-09-17 (pointer-read lane, 47eb605f..af155607): the `0 ptr-field` in the pointer definers is gone at the source, because an empty does> clause now leaves a created word alone at both tiers (the definers' clauses are empty); what this dot still covers is folding `0 ptr-field` wherever else it is spelled, e.g. the checker's `X 0 ptr-field` field words.
