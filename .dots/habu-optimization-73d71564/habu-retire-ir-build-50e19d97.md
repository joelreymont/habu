...
Added 2026-09-17 (offset-handle lane, 630dc6e5): IR-BUILD:FIND-B is now the largest registry cost in the compiler at 2.3 percent of a trivial tier-1 definition, a sixteen-row scan per builder resolution, one per append; the same fix one layer down applies (pack the slot into the builder handle, or move the builder record into the region as arena descriptors now are). Acceptance extended: FIND-B's scan goes with it, measured the same way.
