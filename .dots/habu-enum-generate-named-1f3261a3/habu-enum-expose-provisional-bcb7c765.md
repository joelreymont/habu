...
Behavior: every call requires the exact live top declaration token and its bound family; the variant must be a variant event owned by that frame; index is declaration-order among that variants field events. Schema and width come from the sole provisional TYPE-FIELD row selected by the event. PAYLOAD-CELLS is the checked sum of widths. Stale, nested-foreign, wrong-family, wrong-variant, non-field, negative, and one-past indices reject before reading an unauthorized row. Published-only or rolled-back rows are never visible. Preserve empty-variant count and zero width. The checker cannot resolve the sealed pre-hook owner from checked DECL-EVENT code, so extend the existing audited field-owner boundary with exactly one `TRUSTED: DEV-FLD-TX-CELLS-FOR ( n n n -- n ) TYPE-FIELD-OWNER:TX-CELLS-FOR ;`, inventory it beside DEV-FLD-TX-SCHEMA-FOR, and bind it to the same type-DSL-unification retirement owner. Do not add any other trust site or raw-memory access. No copied descriptor, positional fallback, public cast, legacy SUMTYPE/PRODUCT edit, constructor generation, reflection, or caller migration.
