...
Concrete load contract: /home/joel/Work/maki/maki.f requires src/... from the application project and lib/... from the selected Habu checkout. Remove the need for its temporary absolute-require/provided-alias manifest; application build source resolution must handle both roots (Maki 15:01).


Tender supplied an existing source-discovery defect: a valid local named required is mistaken for the loader (-4101). Root fix in .jj-ws/cedar-source tracks byte-exact local bindings and block/quotation scopes; expanded tools/source-discovery-test.f passes. Independent review and integration pending. Native --repl bypasses DISCOVER in the app candidate, but AOT still requires this correction.
