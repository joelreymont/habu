...
Frozen interface: `OPEN ( CAD-NUM:byte-len -- result<MODEL-ASSET:ws,n> )`; `PATH-SPAN ( MODEL-ASSET:ws -- MODEL-ASSET:ws ptr u8 CAD-NUM:byte-len )`; `BYTE-SPAN` has the same stack shape; `RELEASE ( MODEL-ASSET:ws -- result<n,n> )`. Each span is an owner-threaded borrow valid only while that exact workspace remains live; callers must not retain it past a consuming call. The raw-buffer ban means no ambient or lifetime-free buffer, not no owner-threaded pointer. No callback, copy-out, limits type, or special result type exists.
