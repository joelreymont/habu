...
Ratchet the global-disable escape hatch to zero: every '0 set-check' site (outside the builder prefix, which staged fixpoint checking dissolves) must be retired as capabilities land, or carry source-local rationale naming an owning capability dot. Enforce with a gate check: set-check count monotonically non-increasing, hard-fail on new sites without a dot reference. End state: set-check exists only for the seed/bootstrap path.
...
- src/habu/aot-closure.f:5, aot-lib.f:15, build.f:13, maker.f:8, snap.f:25
  ...
  Each site carries the dissolution cross-ref comment; hide.f:BFR-CHECK-OFF
  is owned by the staged-fixpoint dot.
...
source-local rationale, an owning capability dot, and an in-file cross-ref to
the capability that retires it. This dot STAYS OPEN pinned to the builder
