...
Add a checked path lint rejecting new test-shaped Forth modules outside the canonical tree and validate collision, case, duplicate-basename, direct-load, dependency-cycle, and stale-path failures. Prove every moved test still runs standalone and through its owning suite, every canonical inventory member runs exactly once, cache invalidation reflects the new path, and package, suite-coverage, host, bootstrap, fixpoint, Maki, PTX standard library, and full native gates pass on each leaf and the integrated tree. Measure source-path duplication, manifest rows, discovery time, and gate wall time before and after; require no unexplained growth.
