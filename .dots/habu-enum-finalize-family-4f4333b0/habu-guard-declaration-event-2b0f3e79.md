...
Ratchet closure: the accepted guard adds exactly four top-level colon
definitions in `src/core/decl-event.f` and two in `src/core/type-family.f`.
The assembled linux-arm64 stage2 census therefore measures 4062 versus the
committed 4056. Update only the linux-arm64 Certified row in `STATUS.md` to
4062; uncheckable and rejected remain zero, the macOS row remains owed, and
`Last verified` remains unchanged until the combined full tree is green.
Acceptance: the build-fixpoint census on the exact integration parent reports
linux-arm64 4062/0/0, status/stale-status checks accept the row, and removing
any one of the six new definitions makes the exact ratchet fail. No source,
date, other-target, timing, or code-size edit.
