...
VERDICT (2026-08-07, refuted-inlining lane): the chain-side half of this dot's concern is DONE — inline.f SMALL? decides per call site from measured post-selection size vs the convention-derived overhead (in+out+3), the value-heuristic byte bound deliberately rejected (LESSONS records it). The remaining scope is the ENGINE's INL-MAX copy policy (habu2.f:38) and the AOT/direct-BL default — machinery habu-delete-the-old-679cfd35 deletes at the cut. This dot closes with that deletion unless AOT retains an inliner; reconcile there, build nothing here.
