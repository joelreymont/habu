...
STEP-1 CORRECTION 2026-07-18: the xt<effect> cell capability already landed via habu-typed-xt-storage-ddad4af8 (CHECKER-STORAGE-INFO admits T-QUOT at checker.f:2879; TYPED-VARIABLE/TYPED-BUFFER; test/xt-cell-test.f, which also pins the raw-variable launder fixture ready for the step-3 flip). habu-typed-xt-cells-08e1dc2c is re-scoped to the one genuine remainder: the tick-store retype (`['] W XV !` falsely rejects today; only quotation-stores work). Step 2's migration will use idiomatic tick-stores once that lands.
