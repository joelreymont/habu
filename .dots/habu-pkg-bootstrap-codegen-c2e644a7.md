...
Claim: agent=bcgpkg workspace=.jj-ws/habu-add-using-to-d815f0ab (RELEASED 2026-08-21: workspace gone, no live lane - gc)

MALFORMED LEAF, found by the claim-release pass 2026-08-21: this file holds TWO
complete dot documents concatenated, and it is the only leaf in .dots that does.
The `dot` CLI reads the first one, so the second block's frontmatter is inert -
which is exactly why its `status: active` survived the release sweep and had to
be flipped by hand. The two blocks describe the same work at different stages:
the first is the original contract, the second repeats it and appends the
bcgpkg lane's measured report. Merging them into one document needs a judgement
about which contract text is authoritative and is left to the tracker owner;
the release pass only removed the stale `active`.
