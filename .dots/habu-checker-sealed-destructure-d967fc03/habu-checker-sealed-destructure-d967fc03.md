...
The campaign closes when owner construction works through the production compiler, public products are unchanged, OWNER-WID is absent, inference proof-mint ceremony is deleted (package sealing itself dies in the type conversion, habu-convert-the-type-d5aad352), NEWTYPE no longer exists as a defining word, foreign construction by qualified name rejects at check time, and native fixpoint plus declaration and inference suites pass.
