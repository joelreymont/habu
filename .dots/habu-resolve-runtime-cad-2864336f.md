...
Design-reference (2026-07-18, tfinite): implements `docs/effects.md` § R8-4 (the runtime resolver). Acceptance: `maki/effect-bindings.f` combines a sealed static row with typed operands/attributes/capability tokens, the stable semantic site path, and canonical Artifact metadata into a sorted `(atom, site-path, slot-kind, slot-index, semantic-fact)` set or a typed uncacheable/unresolved reason; `param-read` resolves an immutable digest, `state-write`/`random` bind owner plus generation/sequence, and the remaining classes bind exact authority facts or return a typed reason; only an exact repeated full tuple is idempotent, a divergent fact on one atom/site/slot is a typed conflict, distinct sites/slots stay distinct despite matching slot numbers or digests, site paths derive from revision/node/call structure with no address or insertion counter, and no cache owner may project the set privately — mutation-matrix rows 24-32.
