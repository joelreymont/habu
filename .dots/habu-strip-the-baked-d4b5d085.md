...
Claim: agent=alder workspace=.jj-ws/alder-strip-baked.

Current evidence on d3f8be78: the check-only recovery chain passes before
changes. Its stdin stage2-src is 2,845,761 bytes. The checked lexer compactor
prototype produces 1,671,302 bytes while preserving multiline strings, paren
comments and primitive registry rows; focused fixtures pass. A two-pass
recovery experiment (raw stdin image, compaction, rebuilt maker/image) also
passes the check-only chain, but still requires the raw payload to boot first.
That dependency does not satisfy the long-term arena-growth requirement, so
the experiment is not proposed for landing. The prototype remains WIP: move
compaction before the first large baked payload is executed, then measure the
actual generated payload in the assembled-stream budget check. A pinned
historical payload length is not a growth check.
