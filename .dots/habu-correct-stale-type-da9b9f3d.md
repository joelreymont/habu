...
Claim: alder. Update only the Structures And Enums section of docs/forth.md.
The current repository rules retire LESSONS.md, so no lesson file is added.

Validation: private b4efad25 host compiles and executes the section's examples
and the named generated operations (MAKE/UNMAKE, addr fields/AT/BYTES/CELLS,
eq/hash, full and compact enums, private constructors, parametric locals) in
tiers 0 and 1. Comment probes for both unified forms return the documented
E-BAD-DECLARATION JSON. The forth-card-doc registry row passes. Astra review
clear. No code or other documentation section changed; closure is with Hazel.
