...
Drift note 2026-09-12 15:05 UTC (hazel): af24000e moved src/core/dynamic-storage.f into native's core LOAD group (after src/core/bytes.f); the seed's mirror in bootstrap/cg/forth.fs still carries it in its three-row stdlib group and has no bytes.f row, so the LOAD mirror is behind native again; both gforth-hosted checks stay green. Re-mirror when the chain is worked on.
