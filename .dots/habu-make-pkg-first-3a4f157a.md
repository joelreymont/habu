...
Full context: AGENTS.md Forth style summarizes casing and typing but omits docs/forth.md:109-123 package-first rule, so agents repeatedly generate raw namespace stems such as LRD-* despite the canonical standard. Fix: add a concise BLOCKING AGENTS rule requiring every new library, tool, test-support, and subsystem module to use a real package by default; permit globals only for documented core/prelude language surfaces and E-* error constants; require short private/public tails and cross-package qualification; forbid raw prefix stems as namespace substitutes. Acceptance: AGENTS points to the canonical package section, the rule is unambiguous at first-session scan, LESSONS records why indirect documentation was insufficient, and host/filemap/dot/maki/ptx gates pass.
