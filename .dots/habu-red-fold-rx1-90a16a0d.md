...
LEGS LANDED 2026-07-16 (rx1fold lane; claim released): host-executor
consistency leg in maki/executor-test.f (BIAS 2x2 + col 2x1=[10,30] ->
[[11,12],[33,34]] proving EX-BC@ reads element r per row-block lane, distinct
from the scalar case) and the device golden in maki/lower-red-device-test.f
(COL->RMSNORM 4x8 + 4x1 hand-built MIR; a shared checked source-text builder
LRD-COL-SRC feeds BOTH the parent host reference (EVALUATE) and the child
driver - one source of truth, off-device load validates the child text; device
compare wired behind CUDA:OPEN?, skips clean off-device). The runtime fold
itself landed 2026-07-08 (EMIT-ROW-SPAN-STRIDE1 + BC-COL routing + positive
PTX-text fixture). REMAINING ACCEPTANCE (device-gated, hence the blocks:
edge): run maki/lower-red-device-test.f on zed, expect V-PASS for the COL
case; then close. SHP-LEGAL? capture legalization intentionally untouched -
separate decision.
