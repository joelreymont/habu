...
Campaign only; do not dispatch. Its leaves add one linear scheduler around the shared INFER engine: nominal request identity, strict immediate KV admission, FIFO order, synchronous cancellation, alternating one-token prefill and decode-batch selection, and one bounded TICK that writes caller-owned result rows. Package KV performs every cache mutation; INFER is its only external owner and invokes cache construction in START, admission in OPEN-SEQ, provisional execution in RUN-ROWS, aggregate reclamation in CLOSE-MANY, and teardown in STOP. SCHED calls only INFER and receives no cache carrier. Engine, sequence, tokenizer, sampling, and model ownership remain in INFER; transport state remains outside SCHED.
