...
blocks:
  - habu-snapshot-rebase-persisted-4bd33351
---
...
REMAINING SCOPE (why this dot stays open) - three parts, first two are new
sub-problems the original framing missed:
...
`@ set-top-check`;
(3) checker sig-store persistence: find and rebase the field at DATA 0x378729
feeding FIND-SIG's sig-less-token scan (start from CHECKER-RECORD-SYM /
CHECKER-FIND-USIG-SYM interning and CHECKER-SNAPSHOT-PREPARE; the value is a
raw __text base with no offset - a cached text base or baked table pointer,
not a name pointer). Only after (3) is tracker-on-snapshot safe for arbitrary
user tokens; then test/top-row-snapshot-test.f asserts identical p1/p2/p3
...
CROSS-REF 2026-07-15: part (3) above (the un-rebased DATA pointer, stale
engine-text base + 0x7220) is the SAME defect sol hit independently via
...
investigation). THIS dot retains parts (1) arm timing and (2) the native
re-arm mechanism, and blocks on 4bd33351 for tracker-on-snapshot.
