...
## Follow-on (2026-07-07, from closed habu-batch-candidate-src-57288926)

When the raw NR-EXIT-GROUP sites here become catchable throws, the source-misuse
negatives they enable (duplicate definition, colon/dict overflow, package
misuse) should NOT be added as new process sentinels: fold them into the
test/engine-suite.f probe table (independent T= probes on the shared
GE-ENGINE-SUITE-ON candidate launch, with the evaluator result object making a
probe error non-fatal to the batch) - the batching mechanism that dot built.
Only contract-bound sentinels (hardware traps, exit-contract tests, profiler
isolation, CLI end-to-end) stay as spawns per docs/gate.md Boundary Rule.


Claim: agent=rawexit workspace=.jj-ws/fable-rawexit
