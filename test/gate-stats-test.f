...
15000 constant GST-GUARD-TIMEOUT-MS
...
: GST-GUARD-RUN ( -- n )
   ...
   GST-GUARD-TIMEOUT-MS >MS RUN-ARGV-CAPTURE
   MATCH result
...
: GST-GEN-ENV-RUN ( ptr u8 n -- n n ) {: val:ptr valu:n :}
   ...
   s" bin/hb" >LEN GST-GUARD-OUT GST-GUARD-CAP >LEN GST-GUARD-ERR GST-GUARD-CAP >LEN
   GST-GUARD-TIMEOUT-MS >MS RUN-ARGV-ENV-CAPTURE
   MATCH result
...
: RAW-CWD ( -- )
   ...
   -1 >FD -1 >FD -1 >FD PROC-CWD:SPAWN-ARGV-ENV-CWD-RAW WAIT-CLEAN
   PROC-CWD:ARGV-ENV-CWD-RESET ;
...
: STDIN-EXEC ( -- )
   ...
   GST-GUARD-TIMEOUT-MS >MS RUN-ARGV-STDIN-CAPTURE
   MATCH result
