...
\ contract.
\
\ The module lives in `package RELOC-PROOF`. Its subject is the pair of emitted
\ passes that make a snapshot image portable - `SNAP-RELOC:EMIT-CALLS` and
\ `SNAP-RELOC:EMIT-XT` in src/habu/habu2.f, with the writer's address-cell half
\ `SND-CANON-XT-CELL` in src/habu/snap-lib.f and the band constants in
