...
\ shape of the vector table, and every call, address-cell and address-literal
\ chain row driven through the shipped `SNAP-RELOC:EMIT-CALLS`,
\ `SNAP-RELOC:EMIT-XT` and `SNAP-RELOC:EMIT-ADDRS` instruction sequences.
\
