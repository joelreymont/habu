\ ir-intern-proof.f - the compiler interning parity gate.
\
\ `formal/Common/Interning.v` proves the interning invariant that IR-SYM, IR-TYPE
\ and IR-ATTR share: a table is an append-only list of distinct rows under a
\ committed ceiling, interning a key answers the ordinal of the row that matches
\ it or appends one, a rejected intern leaves the table exactly as it was, and a
\ key may only refer to ordinals strictly below its own. Until this gate exists
\ those are theorems about a model, and `src/compiler/ir/{symbol,type,attr}.f` is
\ a separate description of the same design, with nothing stopping the two
\ drifting apart.
\
\ One artifact, two readers. `test/compiler/ir-intern-schema.f` holds the intern
\ sequences and the frozen structural rows. `test/compiler/ir-intern-cases.f`
\ asks the shipped compiler about them and `test/compiler/ir-intern-obligations.f`
\ asks Rocq about the same rows. Neither side carries a copy.
\
\ What the gate refuses:
\
\   - a changed scan-predicate field list, a capacity check that moved after the
\     first arena push, or a weakened reference guard, through the structural
\     reader in the cases file;
\   - a step of any intern sequence, through the Habu run and the generated Rocq
...
\     structural inventory of what `Interning.v` declares and what it queries;
\   - ANY assumption at all. This is stricter than the identity gate, which names
\     two assumptions and holds the observed set equal to them. The interning
\     proofs rest on nothing, so this gate refuses an assumption twice over: the
\     manifest is read with assumption rows forbidden, so one cannot be written
\     down, and every statement must additionally report "Closed under the global
\     context" with no `Axioms:` header anywhere in what Rocq printed.
\
\ Focused command: `bin/hb --load test/compiler/ir-intern-proof.f`. The gate
...
require test/compiler/ir-intern-obligations.f

package COMPILER-INTERN-PROOF-GATE
...
: MANIFEST-PATH$ ( -- ptr u8 n )
   s" test/compiler/ir-intern-axioms.txt" ;
...
\ The grammar's shape rules - an orphan statement, a second statement for one
\ theorem, a statement written after its assumption rows, a theorem left unpinned
\ - belong to `package PROOF-MANIFEST` and are proved by the identity gate. What
\ this gate owns is the strength of its assumption claim, so these fixtures are
\ about that and nothing else.

: HOSTILE-AXIOM-ROW ( -- )
   s" an assumption row cannot be written into a manifest held at empty" T-LABEL
   [: false PROOF-MANIFEST:AXIOMS-ALLOWED!
      S\" theorem Habu.Common.Interning.intern_wf\ntype True\naxiom host_atomic_cas : cas_impl"
      PROOF-MANIFEST:TEXT ;]
...
: MOD-PUSH ( n -- ) {: k:n :}
   MOD-N @ MOD-MAX >= if E-CIN-STRUCT throw then
   k MOD-TOK MOD-N @ cells + !
   MOD-N @ 1+ MOD-N ! ;

\ A `Section` closes with the same word a `Module` does, so a closer only pops
\ when it names the module actually open. A section inside a module leaves the
\ stack alone.
: MOD-POP ( n -- ) {: k:n :}
...
: PHASE-DECLARATIONS ( -- )
   ...
   s" no statement in the interning proofs is admitted" T-LABEL
   ADMITTED-N @ 0 T=
...
: SCRATCH-DIR ( -- ptr u8 n )
   s" habu-ir-intern-parity" TMPDIR-MKDIR ;

: OBLIGATION-ROW ( n -- ) {: k:n :}
   k PROOF-MANIFEST:THEOREM$ k PROOF-MANIFEST:TYPE$ COMPILER-INTERN-ROCQ:STATEMENT+
   k PROOF-MANIFEST:THEOREM$ COMPILER-INTERN-ROCQ:ASSUMPTION+ ;

: EMIT-OBLIGATIONS ( -- )
   COMPILER-INTERN-ROCQ:START
   PROOF-MANIFEST:THEOREMS 0 ?do i OBLIGATION-ROW loop ;

: WRITE-OBLIGATIONS ( ptr u8 n -- ) {: dir:ptr diru:n :}
   dir diru s" InternParity.v" VPATH JOIN-PATH VPATH-U !
   VPATH VPATH-U @ COMPILER-INTERN-ROCQ:TEXT$ ATOMIC-WRITE-FILE ;

public

: PHASE-ROCQ ( -- )
   s" the interning model compiles" T-LABEL
   MODEL-FILE$ ROCQ-CMD:COMPILE
   EMIT-OBLIGATIONS
   CLEANUP-RESET
   SCRATCH-DIR 2dup CLEANUP-TREE+ WRITE-OBLIGATIONS
   s" every generated interning obligation is proved" T-LABEL
...
COMPILER-INTERN-PROOF-GATE:RUN
