...
\ is still readable.
: FULL-DEFINE ( IR-CTX:ctx IR-ARENA:arena IR-ARENA:arena IR-ID:ir-module-key IR-ARENA:arena IR-ARENA:arena -- IR-CTX:ctx IR-ARENA:arena IR-ARENA:arena IR-ID:ir-module-key IR-ARENA:arena IR-ARENA:arena )
...
: FULL-BODY ( IR-CTX:ctx -- )
   ...
   c sp sr tp tr key V-NAME STAGE
   c a r key sr tr [: FULL-DEFINE ;] catch
   {: rc:n :} 2drop 2drop 2drop
   rc E-IR-SCHEMA-CAP T=
   a IR-ARENA:USED pcells T=
   r IR-ARENA:USED rcells T=
   r c sp sr key V-BASE OP-SYM IR-SCHEMA:OPERANDS 1 T=
   r c sp sr key V-NAME OP-SYM IR-SCHEMA:DEFINED? TFALSE ;
...
: CAP-CASES ( -- )
   ...
   s" a definition past the row ceiling leaves both stores intact" T-LABEL
   FULL-RUN
