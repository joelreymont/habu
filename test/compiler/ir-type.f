...
\ ---- arena growth preserves interned types -----------------------------------
\ Three header cells and the first four-cell type fit the seed. The second
\ type grows the row arena even after scratch passes its former $80000 mapping.
: GROW-BODY ( IR-CTX:ctx -- )
   ...
   c a r key I32 {: t1:IR-ID:ir-type-id :}
   r IR-TYPE:TYPES 2 T=
   r t0 IR-TYPE:INT@ {: w0:IR-TYPE:width sg0:IR-TYPE:sign :}
   w0 IR--TYPE-WIDTH:W64 IR--TYPE-WIDTH:EQ TTRUE
   sg0 IR--TYPE-SIGN:SIGNED IR--TYPE-SIGN:EQ TTRUE
   r t1 IR-TYPE:INT@ {: w1:IR-TYPE:width sg1:IR-TYPE:sign :}
   w1 IR--TYPE-WIDTH:W32 IR--TYPE-WIDTH:EQ TTRUE
   sg1 IR--TYPE-SIGN:SIGNED IR--TYPE-SIGN:EQ TTRUE
   c a r key I64 IR-ID:TYPE-LOCAL t0 IR-ID:TYPE-LOCAL T=
   a IR-ARENA:USED 3 T=
   r IR-ARENA:USED 3 2 4 * + T= ;

: GROW-CASE ( -- )
   s" growing the type table preserves identities and fields" T-LABEL
...
: TD-VIEW-READ ( -- )
   TD-DEAD-VIEW IR-TYPE:FTYPES drop ;
...
: EMBED-BODY ( IR-CTX:ctx -- )
   {: c:IR-CTX:ctx :}
   c IR-CTX:BINDING@ CBIND:TARGET@ CTARGET:PTR-BITS 32 T=
   c 8 8 TAB-NEW {: key:IR-ID:ir-module-key a:IR-ARENA:arena r:IR-ARENA:arena :}
   c a r key I64 {: ti:IR-ID:ir-type-id :}
   c a r key IR--TYPE-SPACE:GENERIC ti IR-TYPE:POINTER {: p:IR-ID:ir-type-id :}
   r key p IR-TYPE:POINTER@ IR-ID:TYPE-LOCAL ti IR-ID:TYPE-LOCAL T=
   IR--TYPE-SPACE:GENERIC IR--TYPE-SPACE:EQ TTRUE
   r ti IR-TYPE:INT@ drop IR--TYPE-WIDTH:W64 IR--TYPE-WIDTH:EQ TTRUE ;

: EMBED-CASE ( CTARGET:arch CTARGET:abi -- )
   {: a:CTARGET:arch b:CTARGET:abi :}
   a b EMBED-BND [: EMBED-BODY ;] IR-CTX:WITH-CONTEXT ;

: EMBED-BAD-SPACE ( -- )
   CTARGET-ARCH:C66X CTARGET-ABI:C6000-EABI EMBED-BND
   [: TGT-SPACE-BODY ;] IR-CTX:WITH-CONTEXT ;

: EMBEDDED ( -- )
   [: EMBED-BAD-SPACE ;] E-IR-TYPE-TARGET TTHROWSQ
