...
: PADDING-AND-TRAP ( -- )
   ...
   SINK ENC-NOP   s" 90" X=
   \ llvm-mc: ud2
   SINK ENC-UD2   s" 0f0b" X= ;

: IMMEDIATE-STORES ( -- )
   \ llvm-mc: movl $0x12345678, 16(%rip)
   $12345678 >IMM32 16 MEM-RIP SINK ENC-MOV32-MI32   s" c7051000000078563412" X=
   \ llvm-mc: movl $-1, -129(%r13,%r12,8)
   -1 >IMM32 R13 R12 8 -129 MEM-IDX SINK ENC-MOV32-MI32   s" 43c784e57fffffffffffffff" X=
   \ llvm-mc: movl $0, (%rax)
   0 >IMM32 RAX MEM-AT SINK ENC-MOV32-MI32   s" c70000000000" X=
   \ llvm-mc: movl $-2147483648, (%r13)
   -2147483648 >IMM32 R13 MEM-AT SINK ENC-MOV32-MI32   s" 41c7450000000080" X=
   \ llvm-mc: movl $2147483647, 127(%rsp)
   2147483647 >IMM32 RSP 127 MEM-OFF SINK ENC-MOV32-MI32   s" c744247fffffff7f" X=
   \ llvm-mc: movl $1, 128(%r12)
   1 >IMM32 R12 128 MEM-OFF SINK ENC-MOV32-MI32   s" 41c784248000000001000000" X=
   \ llvm-mc: movl $-2, -128(%rbp)
   -2 >IMM32 RBP -128 MEM-OFF SINK ENC-MOV32-MI32   s" c74580feffffff" X=
   \ llvm-objdump: .Lhere: movl $7, .Lhere(%rip)
   \ Target is the instruction's beginning: -10 includes the trailing imm32.
   7 >IMM32 -10 MEM-RIP SINK ENC-MOV32-MI32
   SINK-LEN 10 T=   s" c705f6ffffff07000000" X= ;

: STORE-REFUSALS ( -- )
   RAX SINK ENC-PUSH
   [: 2147483648 >IMM32 RAX MEM-AT SINK ENC-MOV32-MI32 ;] E-OPERAND TTHROWSQ
   [: -2147483649 >IMM32 RAX MEM-AT SINK ENC-MOV32-MI32 ;] E-OPERAND TTHROWSQ
   [: 0 >IMM32 16 32 lshift >MEM SINK ENC-MOV32-MI32 ;] E-OPERAND TTHROWSQ
   [: 0 >IMM32 4 37 lshift >MEM SINK ENC-MOV32-MI32 ;] E-OPERAND TTHROWSQ
   [: 0 >IMM32 1 44 lshift >MEM SINK ENC-MOV32-MI32 ;] E-OPERAND TTHROWSQ
   s" 50" X=
   s" X64-GOOD-STORE ( X64ASM:imm32 X64ASM:mem ptr a -- ) X64ASM:ENC-MOV32-MI32"
      CHECK-QUIET-CANDIDATE! -1 T=
   s" X64-BAD-STORE ( X64ASM:imm64 X64ASM:mem ptr a -- ) X64ASM:ENC-MOV32-MI32"
