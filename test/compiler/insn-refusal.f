...
\
\ `XREG?` and `>LIMM` in `src/arch/arm64/asm.f` refuse by calling `die`: they
\ write a diagnostic and end the process with code 72. That refusal cannot be
\ observed from inside the engine that triggered it, so
\ `test/compiler/insn-cases.f` runs this file in a child engine, one row at a
\ time, and reads the child's exit status.
\
...
: RESERVED-ROW ( n -- ) {: r:n :}
   r RES-FORM@ r RES-A@ r RES-B@ r RES-C@ 0 COMPILER-INSN-CASES:EMIT-ROW drop ;
