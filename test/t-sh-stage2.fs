...
: SRC+ ( -- )  0 CL !
   ...
   s" selfhost/crash.fs" +F  s" selfhost/macho.fs" +F
   s" selfhost/engine.fs" +F  s" selfhost/engine2.fs" +F
...
\ reference: gforth builds the engine image for SRC (unsigned canonical)
: REF ( -- )
...
\ running it compiles the compiler and emits stage2
