\ t-dogfood.fs — the codegen's stack-juggling bugs ARE catchable by habu. The
...
\ charted. Loaded by all.fs (after habu.fs). See LESSONS.md § Debugging.
...
\ IF arms leave different stacks -> habu REFUSES the definition.
