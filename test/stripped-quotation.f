\ Build one stripped application and run the fresh image through its real entry.
require lib/test.f
...
package STRIPPED-QUOTATION-TEST
...
: SHOW-FAILURE ( n n n -- ) {: outu:n erru:n rc:n :}
   rc 0<> erru 0<> or if
      s" stripped-quotation child failed: " type rc . cr
...
: RUN-IMAGE ( -- )
   ...
   rc 0 T=
   erru LEN>N 0 T=
   OUT outu LEN>N S\" stripped-quotation: ok\n" T$= ;
