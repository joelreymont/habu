...
: BWM-WRITE-REFUSAL ( src-a src-u -- )
   NF-RUN $? WSTAT>RC ENGINE-ERROR:SEAL-VIOLATION <>
   abort" bootstrap write bypassed the sealed friend span" ;

: BWM-STAGE0-TEST ( -- )
   s" 0 data-base $20 + atomic!" BWM-WRITE-REFUSAL
   s" 1 data-base $20 + atomic-add drop" BWM-WRITE-REFUSAL
   \ Both matching and nonmatching CAS must check the protected write span.
   s" $90 0 data-base $20 + atomic-cas drop" BWM-WRITE-REFUSAL
   s" 0 0 data-base $20 + atomic-cas drop" BWM-WRITE-REFUSAL
   s" create BWM-PATH 47 c, 0 c, BWM-PATH data-base $20 + 8 realpath drop" BWM-WRITE-REFUSAL
