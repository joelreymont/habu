...
: SUITE-LINT-LIBS-LABEL? ( -- bool )
   ...
   s" ptx-cpp-slot-neg" SUITE-LABEL= if SUITE-TRUE exit then
...
: SUITE-INSTALL-TEST-HOOKS ( -- )
   [: SUITE-SETUP ;] is TEST:SETUP
   [: SUITE-CLEANUP ;] is TEST:TEARDOWN
   [: GT-POOL-DRAIN ;] is TEST:DRAIN
   [: SUITE-HB ;] is TEST:ARGS-BEGIN
   [: SUITE-ARG+ ;] is TEST:ARG+
   [: SUITE-RUN? ;] is TEST:SELECT?
   [: SUITE-HB-RUN ;] is TEST:RUNNER
   [: SUITE-HB-RUN-STDIN ;] is TEST:STDIN-RUNNER ;
