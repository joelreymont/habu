...
\ has drained and before TR-COMPLETE decides the run.
\
...
\
\ Calibration is bracketed the way the full-gate verdict brackets its attempts:
\ one fixed calibration spin before the workloads sets the budget factor, one
\ after checks that the host did not change speed underneath them, and
\ PERF-VERDICT:DRIFT-OK? - the single owner of that tolerance - decides. An
\ unstable bracket is re-measured EXACTLY ONCE, with a named diagnostic line
\ saying so; a second unstable bracket is red. There is no retry loop.
...
require test/run-lib.f                \ TR-CAL-REF-MS: the owner of the per-profile spin reference
...
\ calibrates against - test/run-lib.f TR-CAL-REF-MS, which owns the committed
...
\
\ TR-PREPARE already installed the driver's own factor before this phase runs.
...
: FACTOR ( n -- n )                   \ spin ms -> this host's budget factor, percent
   TR-CAL-REF-MS T-BUDGET-CAL-PCT ;

: CAL-LINE ( n n bool -- ) {: pre:n post:n stable:bool :}
   SB-RESET
   s" json-read-perf-phase: calibration pre=" SB-APPEND pre FMT:SB-U
   s"  post=" SB-APPEND post FMT:SB-U
   s"  stable=" SB-APPEND
   stable if s" true" else s" false" then SB-APPEND
   SB$ type cr ;

: ATTEMPT ( -- bool )                 \ measure once; true when the bracket held
   PROBE-MS {: pre:n :}
   pre FACTOR TEST-BUDGET:PERF-SET
   T-RESET
   JSON-READ-PERF-TEST:MEASURE
   PROBE-MS {: post:n :}
   pre post PERF-VERDICT:DRIFT-OK? {: stable:bool :}
   pre post stable CAL-LINE
   stable ;

: RERUN-LINE ( -- )
   s" json-read-perf-phase: calibration drifted; re-measuring once" type cr ;

: STABLE-ONCE ( -- )                  \ at most one re-measure, then red
   ATTEMPT if exit then
   RERUN-LINE
   ATTEMPT 0= if E-JRPP-DRIFT throw then ;
...
\ measured. Failures reach the gate as that fork's nonzero exit.
: START ( -- )
   ADMIT
   CLAIM
