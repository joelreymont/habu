...
- `src/core/type-field.f` — protected transactional shared field arena with nominal owner/layout/source roles, linear builder tokens, commit-only field ids, visibility checks, and snapshot/AOT persistence.
...
- `test/type-field-suite.f` — native-gated shared field-arena coverage for staged errors, strict LIFO, commit-only ids, role swaps, package/family/field visibility, parser-token copying, all-arena growth, snapshot persistence, PRODUCT/SUMTYPE/ENUM owners, and reflection.
