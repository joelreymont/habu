...
- `src/core/decl-event.f` — shared post-hook `DECL-EVENT` declaration syntax-event transaction: one event log with header (arity/POLICY/DERIVE) and variant events, the current-variant selector, field-record transaction bracketing, atomic publication, watermark rollback, read-only event reflection, and deterministic snapshot identity for both STRUCTURE and ENUM front ends.
- `src/core/structure-make.f` — STRUCTURE constructor generator (package `STRUCTURE-MAKE`): given a published product-kind family id, generates the sealed `FAMILY:MAKE` (declaration-order field values -> family value) and `FAMILY:UNMAKE` (family value -> declaration-order field values) checked words from the committed field schemas, reusing the shared sum/product constructor-seal machinery; validates fully before any registry write so a rejected generation is byte-identical (atomic publication). The parse front end's `;STRUCTURE` calls `STRUCTURE-MAKE:GENERATE`.
- `src/core/structure-decl.f` — the `STRUCTURE name arity [POLICY p] [DERIVE f+] (FIELD name type)* ;STRUCTURE` typed-declaration front end (package `STRUCTURE-DECL`): the one executable composite-declaration keyword. It parses the grammar, registers the product family, resolves each field type to a schema node with cell/byte layout, and drives the shared `DECL-EVENT` transaction, committing by persisting and rolling the family/schema/layout/event stream back to a byte-identical registry on any malformed/duplicate/reserved/unresolved/legacy reject. Loaded post-hook, after the shared `DECL-EVENT` module.
- `src/core/enum-decl.f` — the `ENUM name (full | compact) ;ENUM` typed-declaration front end (package `ENUM-DECL`, reached as `ENUM-DECL:ED-RUN`): the second `DECL-EVENT` consumer, binding to the proven `STRUCTURE-DECL` pattern. The first body token after the family name selects the mode irrevocably — a decimal is a full block declaration (`arity [POLICY p] [DERIVE f+] (VARIANT name (FIELD name type)* ;VARIANT)+`, registered `TK-SUM`), any other token is a compact payloadless enum (`v0 v1 ...`, registered `TK-ENUM`, arity 0). It registers the family, drives `DECL-EVENT` (header, variant open/close, and field events under the current-variant selector), and rolls the family/schema/layout/variant/event stream back to a byte-identical registry on any mixed-mode/malformed/duplicate/reserved/unresolved reject. Parse-only: it publishes the family/variant/field metadata and generates no constructors (a later dot). The global `ENUM` token still belongs to the legacy `src/core/sumtype.f` definer until the hard cutover rebinds it. Loaded post-hook, after `src/core/structure-decl.f`.
...
- `test/structure-make-suite.f` — behavior + rollback suite for the STRUCTURE constructor generator (`STRUCTURE-MAKE:GENERATE`): drives declarations through `DECL-EVENT` exactly as the front end will, then proves MAKE/UNMAKE round-trip bit-identically and preserve declaration order for cell/byte/pointer/nested-family/generic fields, field-role and field-arity mismatches reject at MAKE, identical declarations fold to an identical snapshot identity and byte-stable layout, and every reject (non-product/non-live family, no fields, rolled-back/uncommitted field via E-PF-ID, and a second generation) leaves the registries byte-identical.
