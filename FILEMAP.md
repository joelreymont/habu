...
- `src/core/enum-decl.f` — the `ENUM name (full | compact) ;ENUM` typed-declaration front end (package `ENUM-DECL`) and the global `ENUM` keyword itself (`: ENUM ( -- ) ENUM-DECL:ED-RUN ;`, the sole global entry, written as `src/core/structure-decl.f` writes `STRUCTURE`): selects full named-variant or compact payloadless grammar from the first body token, stages family/schema/layout/variant/event metadata through the shared event participant, arms the ORDER 820 participant that generates the per-variant constructors inside the same transaction, and relies on `GENERATED-DECL` for whole-declaration rollback. On a reject in a multi-error load it resynchronizes the input to its own `;ENUM` before the reporter swallows, which is the contract the deleted legacy collect-then-parse definer used to provide. `ED-REPLAY` registers a declaration a tool has already lexed.
...
- `tools/ptx/kernel-export.f` / `tools/ptx/kernel-export-lib.f` /
  ...
  external build embeds. Host-only and deterministic; named `E-KEXPORT-*`
  errors. Tests spawn the CLI twice and byte-compare both artifact pairs, pin
  manifest/PTX fragments, and cover the unknown-kernel and bad-out-dir
...
- `tools/decl-gen-probe.f` — staged render-vs-certify separator for declaration-time constructor generation: evaluates a declaration, then runs the payload-view capture and the whole-set render under their own `catch` so a generation failure names the stage that produced it, and dumps each rendered constructor definition as text. Refuses a family whose constructors already exist rather than tripping the generator's duplicate-name guard, so its subject is a private, unarmed, or failed declaration.
...
- `lib/cad-num-types.f` — package CAD-NUM scalar nominal numeric roles (byte-len, item-count, cell-count, index, byte-off, cell-off, alignment, positive-divisor, alloc-byte-len, alloc-cell-count) plus the on-stack `numeric-result<a>` sum; public checked validators wrap private audited `TRUSTED:` mints (MODEL-CAD-V2-PLAN.md B5.1). Production-loaded through arithmetic; final package sealing remains separate.
- `lib/cad-num-types-test.f` — CAD-NUM B5.1 boundary matrix (every role x valid/zero/negative/overflow) and the static cross-role/raw-n rejection candidates; focused direct-loaded gate home.
...
- `lib/cad-num-arithmetic.f` — package CAD-NUM (reopened) B5.2 closed role arithmetic: strict index/offset bounds, checked index-to-byte-offset multiplication, extent add/sub, offset/index advance/retreat/distance, count multiply/scale, total positive-divisor div/rem, cell<->byte conversions, align-up, and alignment predicates. Overflow/underflow/misalignment are returned `numeric-result` variants; success values route through slice 1's public validators (no minting here); six private `TRUSTED:` role->n projections complete the read set. Production-loaded by memory and inference modules; final package sealing remains separate.
- `lib/cad-num-arithmetic-test.f` — CAD-NUM B5.2 arithmetic matrix: strict equality bounds, independent commuted-quotient result-arm checks from zero through far wrap, arbitrary full-cell product and comparison properties, per-bit one/zero and lower/upper/cross-band generator coverage, the cast-free GPT-2 F32 composition shape, every original table boundary, and static role-swap/raw-exposure rejection; suite `cad-num-arithmetic` in the standard-library gate.
...
- `test/type-decl-suite.f` — behavior suite for the NEWTYPE/SUMTYPE declaration grammar (positives, negatives, rollback, multi-error, diagnostics).
...
- `test/field-proj-suite.f` — checker field-projection capability: the sealed, schema-aware armed window that mints `ptr <field-type>` from a `ptr family<args>` input and a committed field id. Drives the `field-project` op under a test-armed window (arming through a TRUSTED friend forwarder to the sealed `FIELD-PROJ!`), proving cell, byte-offset, generic-substituted, and pointer-role projections read back by value, and that every inconsistency (unarmed op, forged offset, offset past the family width, non-layout pointer, foreign family, value/pointer role confusion, wrong scalar type, uncommitted-id arming) rejects.
- `test/layout-defer.f` — DEFER-LAYOUT-BUFFER deferred-offset column: checker-transparent accessor, unbound-dies-named, NAME-BIND derive-from-count sizing, grow-to-largest reuse, and transactional sanity-ceiling rejection.
...
- `test/type-field-owner-suite.f` — focused lifecycle suite for the sealed field owner: exact mutation-free prepare, reversible commit, finalization, nested rollback, and sealed package ownership; plus the retired product-field names (checker verdict 0, dictionary absence, and rc-70 load failure) and the qualified owner API's certification and argument-role rejects.
- `test/generated-declaration-transaction-suite.f` — coordinator regression suite using an isolated test instance for aggregate-growth allocation identity, phase ordering, callback-depth corruption, cleanup poisoning, and diagnostics; production tests prove sealed controls, nested declaration invisibility, whole-owner/dictionary rollback, evaluator reuse, and constructor protection publication.
- `test/declaration-release-inventory.f` — proves from the production source that the transaction's release phase is total: runs `tools/release-inventory.f` over the eight sources hosting the release chain and over hostile fixtures for a rejecting release callback, comment-only and string-only occurrences of forbidden words, a reordered callback slot, a registration passing four callbacks, a duplicated release definition, a deferred vector bound to a fallible word, a lookalike name, and an unscanned call.
- `test/checker-decl-nested-bad.f` — child-process negative fixture: a nested declaration body that destroys its own checker rollback frame is refused instead of releasing the enclosing declaration's frame (exit 76, "declaration rollback frame mismatch").
- `test/checker-decl-depth0-bad.f` — child-process negative fixture: the same destruction at the outermost declaration with a grown frame arena; the rollback walk stays inside the arena and fails closed (exit 76) instead of popping past its base.
- `test/decl-event-suite.f` — behavior + rollback suite for the shared `DECL-EVENT` declaration syntax-event transaction: atomic publication, event reflection, watermark rollback (event arena + field ordinal + variant ordinal + current variant + field-record + enclosing checker frame), identical STRUCTURE/ENUM field events, field name-gate passthrough, duplicate POLICY/DERIVE and out-of-range arity rejects, and deterministic snapshot identity.
- `test/structure-make-suite.f` — behavior + rollback suite for `STRUCTURE-MAKE:GENERATE`: drives live declaration tokens through `DECL-EVENT`, proves exact MAKE/UNMAKE effects for every field kind, and proves stale tokens, invalid families, empty products, and duplicate generation leave all registries unchanged.
- `test/structure-decl-suite.f` — behavior + rollback suite for the STRUCTURE typed-declaration front end (`src/core/structure-decl.f`): a successful declaration persists its family and fields; field events reach `TYPE-FIELD` reflection with the declared names, schemas, order, slots, and byte offsets; POLICY and DERIVE reach the family record and the event stream; every reject anchor (E-TDECL-* family plus the field record's own name gate) fires at the offending token; a mid-declaration reject leaves every registry cursor byte-identical; and the snapshot identity is deterministic for an identical declaration against a fresh registry.
- `test/structure-certify-suite.f` — checker certify suite for STRUCTURE construct/access driven end-to-end from the STRUCTURE grammar (the STRUCTURE twin of the ENUM certify leaf): pins that the generated `FAMILY:MAKE`/`UNMAKE` reach the shared product-ctor typing identically to the legacy `PRODUCT` path across the slices only the STRUCTURE front end can declare — MAKE/UNMAKE exact inverses for wide-nested-product, pointer, and sum-payload fields; whole-layout width (the wide field consumes the whole bundle, not its flattened cells); generic field instantiation at concrete and wide types with linear instantiations fail-closed; pointer/value role positives and negatives; forged/malformed construct rejects before runtime (raw-cell forge, wrong arity, cross-family alias, split-without-UNMAKE) with whole-bundle transport allowed; a field-shape-specific `E-MISMATCH` diagnostic; and the STRUCTURE-derived ctor package's closed-but-callable protection.
