...
- `src/core/type-field.f` — protected transactional shared field arena with nominal owner/layout/source roles, linear builder tokens, provisional rows, commit-only field ids, and visibility checks.
...
- `test/type-field-suite.f` — native-gated shared field-arena coverage for staged errors, strict LIFO, rollback reuse, commit-only ids, role swaps, package/family/field visibility, parser-token copying, arena growth, PRODUCT/SUMTYPE owners, finalized ENUM rejection, and reflection.
