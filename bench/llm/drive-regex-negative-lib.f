...
: DRX-CAPTURE-HAS-CODE? ( -- bool )
   DS-OUT-BUF DS-OUT-U @ DRX-CODE$ CONTAINS? if DS-TRUE exit then
   DS-ERR-BUF DS-ERR-U @ DRX-CODE$ CONTAINS? ;

: DRX-NEGATIVE-CORRECT ( -- )
   DS-DIAG-PATH$ DRX-CODE$ WRITE-ALL
   1 DS-DIAG-COUNT !
...
: DRX-FINISH-CHECK-REJECT ( -- )
   DRX-CAPTURE-HAS-CODE? if DRX-NEGATIVE-CORRECT else DRX-NEGATIVE-FAIL then ;
