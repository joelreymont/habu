...
: DP-NEGATIVE-PASS? ( -- bool )
   DS-RC @ 0= if
      DS-OUT-BUF DS-OUT-U @ DP-EXPECTED-CODE$ CONTAINS? exit
   then
   DS-FALSE ;

: DP-NEGATIVE-SILENT? ( -- bool )
   DS-OUT-BUF DS-OUT-U @ s" silent success" CONTAINS? ;

: DP-LR-NEGATIVE-CORRECT ( -- )
   DS-DIAG-PATH$ DP-EXPECTED-CODE$ WRITE-ALL
   1 DS-DIAG-COUNT !
   s" reject" DP-LR-REJECT ;

: DP-FINISH-NEGATIVE ( -- )
   DP-NEGATIVE-PASS? if DP-LR-NEGATIVE-CORRECT exit then
   DP-NEGATIVE-SILENT? if
      DS-DIAG-PATH$ DS-WRITE-CAPTURE
      1 DS-DIAG-COUNT !
      DP-LR-PASS
      exit
