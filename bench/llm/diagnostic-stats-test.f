...
: DGST-NON-OBJECT-DIAGS$ ( -- ptr u8 n )
   s" []" ;
...
: DGST-EXPECT-NON-OBJECT-JSON ( -- )
   DGST-NON-OBJECT-DIAGS$ DGS-SCAN-DIAGS ;
...
: DGST-EXPECT-BLANK-DIAGS ( -- )
   DGST-DIAGS-BLANKS$ DGS-SCAN-DIAGS
   DGS-DIAGNOSTIC-COUNT 2 T=
   DGS-DIAGNOSTIC-REPAIR-CLASS? TTRUE ;
...
: DGST-MAIN ( -- )
   ...
   DGST-EXPECT-STATS
   DGST-EXPECT-DIAG-EVENTS
   [: DGST-EXPECT-BAD-ROUND ;] E-BM-FIELD TTHROWSQ
   [: DGST-EXPECT-BAD-FIELDS ;] E-BM-SCHEMA TTHROWSQ
   [: DGST-EXPECT-EMPTY-CLASS ;] E-BM-FIELD TTHROWSQ
   [: DGST-EXPECT-BAD-CLASS ;] E-BM-SCHEMA TTHROWSQ
   [: DGST-EXPECT-BAD-JSON ;] E-JSON-SYNTAX TTHROWSQ
   [: DGST-EXPECT-NON-OBJECT-JSON ;] E-JSON-TYPE TTHROWSQ
   T-REPORT
