...
: DB-NEGATIVE-PASS? ( -- bool )
   DS-RC @ 0= if
      DS-OUT-BUF DS-OUT-U @ DB-EXPECTED-CODE$ CONTAINS? exit
   then
   DS-FALSE ;

: DB-NEGATIVE-SILENT? ( -- bool )
   DS-OUT-BUF DS-OUT-U @ s" silent success" CONTAINS? ;

: DB-LR-NEGATIVE-CORRECT ( -- )
   DS-DIAG-PATH$ DB-EXPECTED-CODE$ WRITE-ALL
   1 DS-DIAG-COUNT !
   s" reject" DB-LR-REJECT ;

: DB-FINISH-NEGATIVE ( -- )
   DB-NEGATIVE-PASS? if DB-LR-NEGATIVE-CORRECT exit then
   DB-NEGATIVE-SILENT? if
      DS-DIAG-PATH$ DS-WRITE-CAPTURE
      1 DS-DIAG-COUNT !
      DB-LR-PASS
      exit
