...
: RATT-MAKE-NAMED-DIR ( ptr u8 n -- ) {: name:ptr nameu :}
   name nameu RATT-DIR-PATH!
   RATT-PATH$ MAKE-DIR ;
...
: RATT-WRITE-TRUST-ONE-SRC ( -- )
   SB-RESET
   s" s" SB-APPEND 34 SB-APPEND-C s"  ONE" SB-APPEND 34 SB-APPEND-C
   s"  s" SB-APPEND 34 SB-APPEND-C s"  -- i64" SB-APPEND 34 SB-APPEND-C
   s"  TRUST" SB-APPEND STR-LF SB-APPEND-C
   s" s" SB-APPEND 34 SB-APPEND-C s"  ONE2" SB-APPEND 34 SB-APPEND-C
   s"  s" SB-APPEND 34 SB-APPEND-C s"  -- i64" SB-APPEND 34 SB-APPEND-C
   s"  trust" SB-APPEND STR-LF SB-APPEND-C
   s" 0 set-check" SB-APPEND STR-LF SB-APPEND-C
   s" : ONE ( -- i64 ) 1 ;" SB-APPEND STR-LF SB-APPEND-C
   SB$ RATT-WRITE-CAND ;
...
: RATT-EXPECT-MULTI-ROUND ( -- )
   ...
   1 s" 10.f" RATT-EXPECT-ROUND
   2 s" 2.f" RATT-EXPECT-ROUND ;

: RATT-EXPECT-ARBITRARY-ROUNDS ( -- )
   s" 12" RATT-MAKE-DIR
   s" z.txt" RATT-WRITE-IN-DIR
   s" nested" RATT-MAKE-NAMED-DIR
   s" a.f" RATT-WRITE-IN-DIR
   s" 010.f" RATT-WRITE-IN-DIR
   s" 001.f" RATT-WRITE-IN-DIR
   RATT-ROOT$ s" 12" RA-CANDIDATES 3 T=
   0 s" 001.f" RATT-EXPECT-ROUND
   1 s" 010.f" RATT-EXPECT-ROUND
   2 s" a.f" RATT-EXPECT-ROUND ;
...
: RATT-MAIN ( -- )
   ...
   RATT-EXPECT-DIR-PRECEDENCE
   RATT-EXPECT-BUNDLE
   [: RATT-EXPECT-MISSING ;] E-RA-MISSING TTHROWSQ
   [: RATT-EXPECT-EMPTY-DIR ;] E-RA-MISSING TTHROWSQ
   [: RATT-EXPECT-BAD-ROUND ;] E-RA-CAPACITY TTHROWSQ
   [: RATT-EXPECT-MISSING-CANDIDATE ;] E-RA-MISSING TTHROWSQ
   [: RATT-EXPECT-MISSING-REF ;] E-RA-MISSING TTHROWSQ
   [: RATT-EXPECT-MISSING-TARGET ;] E-RA-MISSING TTHROWSQ
   [: RATT-EXPECT-BUNDLE-CAPACITY ;] E-RA-CAPACITY TTHROWSQ
   RATT-EXPECT-CHECK-PASS
   RATT-EXPECT-CHECK-REJECT
   RATT-EXPECT-RUN-TESTS-PASS
   RATT-EXPECT-RUN-TESTS-FAIL
   [: RATT-EXPECT-RUN-TESTS-TRUNCATED ;] E-PROC-TRUNCATED TTHROWSQ
   [: RATT-EXPECT-RUN-TESTS-TIMEOUT ;] E-PROC-TIMEOUT TTHROWSQ
   RA-DEFAULT-TIMEOUT!
   RATT-EXPECT-METRIC-CERTIFIED
   RATT-EXPECT-METRIC-REJECT
   RATT-EXPECT-FINAL-METRICS
   ...
   RATT-EXPECT-TASK-TEST-FAIL
   RATT-EXPECT-TASK-TRUST-ROW
   [: RATT-EXPECT-TASK-MISSING ;] E-RA-MISSING TTHROWSQ
   CLEANUP-RUN
