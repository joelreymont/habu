...
habu_value_t EVAL_APPLY(habu_value_t OP, habu_value_t ARGS, habu_value_t ENV) {
    return (is_nil(SYMBOL_EQ_P(OP, habu_make_symbol_from_string(habu_make_string("+", 1)))) ? (is_nil(SYMBOL_EQ_P(OP, habu_make_symbol_from_string(habu_make_string("-", 1)))) ? (is_nil(SYMBOL_EQ_P(OP, habu_make_symbol_from_string(habu_make_string("*", 1)))) ? (is_nil(SYMBOL_EQ_P(OP, habu_make_symbol_from_string(habu_make_string("/", 1)))) ? (is_nil(SYMBOL_EQ_P(OP, habu_make_symbol_from_string(habu_make_string("=", 1)))) ? (is_nil(SYMBOL_EQ_P(OP, habu_make_symbol_from_string(habu_make_string("<", 1)))) ? (is_nil(SYMBOL_EQ_P(OP, habu_make_symbol_from_string(habu_make_string(">", 1)))) ? (is_nil(SYMBOL_EQ_P(OP, habu_make_symbol_from_string(habu_make_string("cons", 4)))) ? (is_nil(SYMBOL_EQ_P(OP, habu_make_symbol_from_string(habu_make_string("car", 3)))) ? (is_nil(SYMBOL_EQ_P(OP, habu_make_symbol_from_string(habu_make_string("cdr", 3)))) ? (is_nil(SYMBOL_EQ_P(OP, habu_make_symbol_from_string(habu_make_string("list", 4)))) ? ({
    habu_value_t FN = EVAL_EXPR(OP, ENV);
    (is_nil(CONS_P(FN)) ? NIL : (is_nil(SYMBOL_EQ_P(habu_car(FN), habu_make_symbol_from_string(habu_make_string("closure", 7)))) ? NIL : APPLY_LAMBDA(FN, EVAL_LIST(ARGS, ENV), ENV)));
...
habu_value_t APPEND_ENV(habu_value_t ENV1, habu_value_t ENV2) {
    return (is_nil(NIL_P(ENV1)) ? habu_cons(habu_car(ENV1), APPEND_ENV(habu_cdr(ENV1), ENV2)) : ENV2);
}
...
habu_value_t IS_DEFUN_P(habu_value_t EXPR) {
    return (is_nil(CONS_P(EXPR)) ? NIL : SYMBOL_EQ_P(habu_car(EXPR), habu_make_symbol_from_string(habu_make_string("defun", 5))));
}

habu_value_t EVAL_TOPLEVEL(habu_value_t EXPR, habu_value_t ENV) {
    return (is_nil(IS_DEFUN_P(EXPR)) ? habu_cons(EVAL_EXPR(EXPR, ENV), ENV) : ({
    ...
    ({
      ...
      ({
        ...
        ({
          ...
          habu_cons(NAME, ENV_EXTEND(NAME, CLOSURE, ENV));
          });
        });
      });
    }));
