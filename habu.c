...
habu_value_t EVAL_PROGN(habu_value_t EXPRS, habu_value_t ENV) {
    ...
    habu_gc_add_root(&ENV);
    habu_value_t __result = (is_nil(NIL_P(EXPRS)) ? (is_nil(NIL_P(habu_cdr(EXPRS))) ? ({
    habu_value_t DUMMY = EVAL_EXPR(habu_car(EXPRS), ENV);
    habu_gc_add_root(&DUMMY);
    habu_value_t __let_result_35889 = EVAL_PROGN(habu_cdr(EXPRS), ENV);
    habu_gc_remove_root(&DUMMY);
    __let_result_35889;
    }) : EVAL_EXPR(habu_car(EXPRS), ENV)) : NIL);
    habu_gc_remove_root(&ENV);
...
habu_value_t EVAL_APPLY(habu_value_t OP, habu_value_t ARGS, habu_value_t ENV) {
    ...
    habu_gc_add_root(&ARGS);
    habu_gc_add_root(&ENV);
    habu_value_t __result = (is_nil(SYMBOL_EQ_P(OP, habu_make_symbol_from_string(habu_make_string("+", 1)))) ? (is_nil(SYMBOL_EQ_P(OP, habu_make_symbol_from_string(habu_make_string("-", 1)))) ? (is_nil(SYMBOL_EQ_P(OP, habu_make_symbol_from_string(habu_make_string("*", 1)))) ? (is_nil(SYMBOL_EQ_P(OP, habu_make_symbol_from_string(habu_make_string("/", 1)))) ? (is_nil(SYMBOL_EQ_P(OP, habu_make_symbol_from_string(habu_make_string("=", 1)))) ? (is_nil(SYMBOL_EQ_P(OP, habu_make_symbol_from_string(habu_make_string("<", 1)))) ? (is_nil(SYMBOL_EQ_P(OP, habu_make_symbol_from_string(habu_make_string(">", 1)))) ? (is_nil(SYMBOL_EQ_P(OP, habu_make_symbol_from_string(habu_make_string("cons", 4)))) ? (is_nil(SYMBOL_EQ_P(OP, habu_make_symbol_from_string(habu_make_string("car", 3)))) ? (is_nil(SYMBOL_EQ_P(OP, habu_make_symbol_from_string(habu_make_string("cdr", 3)))) ? (is_nil(SYMBOL_EQ_P(OP, habu_make_symbol_from_string(habu_make_string("list", 4)))) ? (is_nil(SYMBOL_EQ_P(OP, habu_make_symbol_from_string(habu_make_string("get-tag", 7)))) ? (is_nil(SYMBOL_EQ_P(OP, habu_make_symbol_from_string(habu_make_string("symbol=?", 8)))) ? (is_nil(SYMBOL_EQ_P(OP, habu_make_symbol_from_string(habu_make_string("make-symbol", 11)))) ? (is_nil(SYMBOL_EQ_P(OP, habu_make_symbol_from_string(habu_make_string("write-file", 10)))) ? (is_nil(SYMBOL_EQ_P(OP, habu_make_symbol_from_string(habu_make_string("read-file", 9)))) ? (is_nil(SYMBOL_EQ_P(OP, habu_make_symbol_from_string(habu_make_string("string-concat", 13)))) ? (is_nil(SYMBOL_EQ_P(OP, habu_make_symbol_from_string(habu_make_string("fixnum-to-string", 16)))) ? (is_nil(SYMBOL_EQ_P(OP, habu_make_symbol_from_string(habu_make_string("symbol-name", 11)))) ? ({
    habu_value_t FN = EVAL_EXPR(OP, ENV);
    habu_gc_add_root(&FN);
    habu_value_t __let_result_31098 = (is_nil(CONS_P(FN)) ? NIL : (is_nil(SYMBOL_EQ_P(habu_car(FN), habu_make_symbol_from_string(habu_make_string("closure", 7)))) ? NIL : APPLY_LAMBDA(FN, EVAL_LIST(ARGS, ENV), ENV)));
    habu_gc_remove_root(&FN);
    __let_result_31098;
    }) : HABU_SYMBOL_NAME(EVAL_EXPR(habu_car(ARGS), ENV))) : HABU_FIXNUM_TO_STRING(EVAL_EXPR(habu_car(ARGS), ENV))) : HABU_STRING_CONCAT(EVAL_EXPR(habu_car(ARGS), ENV), EVAL_EXPR(habu_car(habu_cdr(ARGS)), ENV))) : habu_read_file(EVAL_EXPR(habu_car(ARGS), ENV))) : habu_write_file(EVAL_EXPR(habu_car(ARGS), ENV), EVAL_EXPR(habu_car(habu_cdr(ARGS)), ENV))) : EVAL_EXPR(habu_car(ARGS), ENV)) : (is_nil(SYMBOL_EQ_P(EVAL_EXPR(habu_car(ARGS), ENV), EVAL_EXPR(habu_car(habu_cdr(ARGS)), ENV))) ? NIL : fixnum_to_value(1))) : habu_get_tag(EVAL_EXPR(habu_car(ARGS), ENV))) : EVAL_LIST(ARGS, ENV)) : habu_cdr(EVAL_EXPR(habu_car(ARGS), ENV))) : habu_car(EVAL_EXPR(habu_car(ARGS), ENV))) : habu_cons(EVAL_EXPR(habu_car(ARGS), ENV), EVAL_EXPR(habu_car(habu_cdr(ARGS)), ENV))) : (is_nil((value_to_fixnum(EVAL_EXPR(habu_car(ARGS), ENV)) > value_to_fixnum(EVAL_EXPR(habu_car(habu_cdr(ARGS)), ENV)) ? fixnum_to_value(1) : NIL)) ? NIL : fixnum_to_value(1))) : (is_nil((value_to_fixnum(EVAL_EXPR(habu_car(ARGS), ENV)) < value_to_fixnum(EVAL_EXPR(habu_car(habu_cdr(ARGS)), ENV)) ? fixnum_to_value(1) : NIL)) ? NIL : fixnum_to_value(1))) : (is_nil((value_to_fixnum(EVAL_EXPR(habu_car(ARGS), ENV)) == value_to_fixnum(EVAL_EXPR(habu_car(habu_cdr(ARGS)), ENV)) ? fixnum_to_value(1) : NIL)) ? NIL : fixnum_to_value(1))) : fixnum_to_value(value_to_fixnum(EVAL_EXPR(habu_car(ARGS), ENV)) / value_to_fixnum(EVAL_EXPR(habu_car(habu_cdr(ARGS)), ENV)))) : fixnum_to_value(value_to_fixnum(EVAL_EXPR(habu_car(ARGS), ENV)) * value_to_fixnum(EVAL_EXPR(habu_car(habu_cdr(ARGS)), ENV)))) : fixnum_to_value(value_to_fixnum(EVAL_EXPR(habu_car(ARGS), ENV)) - value_to_fixnum(EVAL_EXPR(habu_car(habu_cdr(ARGS)), ENV)))) : fixnum_to_value(value_to_fixnum(EVAL_EXPR(habu_car(ARGS), ENV)) + value_to_fixnum(EVAL_EXPR(habu_car(habu_cdr(ARGS)), ENV))));
    habu_gc_remove_root(&ENV);
    habu_gc_remove_root(&ARGS);
    ...
    habu_value_t __let_result_6505 = ({
      ...
      habu_value_t __let_result_54445 = ({
        ...
        habu_value_t __let_result_84907 = ({
          ...
          habu_value_t __let_result_2440 = ({
            habu_value_t NEW_ENV = ENV_EXTEND_LIST(PARAMS, ARG_VALS, COMBINED_ENV);
            habu_gc_add_root(&NEW_ENV);
            habu_value_t __let_result_40780 = EVAL_EXPR(BODY, NEW_ENV);
            habu_gc_remove_root(&NEW_ENV);
            __let_result_40780;
            });
...
habu_value_t ENV_LOOKUP(habu_value_t SYM, habu_value_t ENV) {
    ...
    habu_gc_add_root(&BINDING);
    habu_value_t __let_result_68794 = (is_nil(SYMBOL_EQ_P(SYM, habu_car(BINDING))) ? ENV_LOOKUP(SYM, habu_cdr(ENV)) : habu_cdr(BINDING));
    habu_gc_remove_root(&BINDING);
    __let_result_68794;
    }) : NIL);
    habu_gc_remove_root(&ENV);
...
habu_value_t EVAL_TOPLEVEL(habu_value_t EXPR, habu_value_t ENV) {
    ...
    habu_value_t __let_result_26255 = ({
      ...
      habu_value_t __let_result_35269 = ({
        ...
        habu_gc_add_root(&BODY);
        habu_value_t __let_result_44108 = ({
          ...
          habu_gc_add_root(&CLOSURE);
          habu_value_t __let_result_11885 = habu_cons(NAME, ENV_EXTEND(NAME, CLOSURE, ENV));
          habu_gc_remove_root(&CLOSURE);
          __let_result_11885;
          });
        habu_gc_remove_root(&BODY);
      ...
      habu_value_t __let_result_69325 = ({
        ...
        habu_value_t __let_result_64878 = ({
          ...
          habu_value_t __let_result_91700 = ({
            ...
            habu_value_t __let_result_28043 = ENV5;
...
habu_value_t REPL_LOOP(habu_value_t ENV) {
    ...
    habu_value_t __let_result_58386 = (is_nil(LINE) ? ({
  ({printf("\n"); NIL;});
          ...
          habu_value_t __let_result_94119 = ({
            ...
            habu_value_t __let_result_35161 = ({
              ...
              habu_value_t __let_result_12930 = ({
                ...
                habu_gc_add_root(&NEW_ENV);
                habu_value_t __let_result_95024 = ({
