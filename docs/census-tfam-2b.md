...
| Site | What | Note |
|---|---|---|
| habu2.f `EMIT-COLD-PREFIX` / `EMIT-HOST-LOAD-PREFIX` / `PFX-LOAD-CHECKER-FILES` / `PFX-LOAD-DECL-FILES` / `PFX-LOAD-CORE-FILES` | base files actually `included` at cold boot: explicit-layout checker/type-family sources, then the post-hook declaration seam, then structures and remaining core. | friend flag stays live across this block; `structures-effects.f` is absent and the seal follows the remaining core. |
| habu2.f `PFX-PROVIDE-CHECKER-FILES` / `PFX-PROVIDE-DECL-FILES` / `PFX-PROVIDE-CORE-FILES` | checker, declaration-seam, and shared-core rows preserve LOAD order; PATH/PROVIDE then carry the cold-only script-argv, internal-mark, and top-row rows that are outside `PFX-LOAD-CORE-FILES`. | `provided` preregisters every baked prefix path so user `require` is a no-op. Checker/core definitions and script-argv are already appended at this point; internal-mark and top-row are registered immediately before their guarded cold loads. LOAD, PATH, and PROVIDE manifests must remain exactly ordered. |
