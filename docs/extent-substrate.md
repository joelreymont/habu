...
arguments — so the product former + factorization ride **existing** parametric
unification with no new registry or unification kernel. Atoms are opaque names
(a product needs a whole new structured-atom grammar); CT roles are flat cons
with no argument slots (a product is not expressible without reinventing
parametric families inside the CT registry).
...
outside the parser prefix at `checker.f:2478`; 1 in `maki` — `fusion.f:71`; the
rest is docs prose).
...
| (b) **BTC-7 product/factorization** | ✗ new structured-atom grammar | ✗✗ flat cons, no arg slots | **✓ arity-2 family, ordered args, nesting — free** |
...
>   Migrate the one live atom sig (`maki/fusion.f:71`) and the docs
>   opportunistically; atoms and TFAM extents may coexist (both unify by
>   identity), but a single substrate is preferred — track the migration.

## Open questions for the orchestrator

1. **Legacy `extent-*` migration vs coexistence.** Adopt TFAM extents for new
   `EXTENT:` surface and migrate `maki/fusion.f:71` + `docs/ptx.md` examples, or
   allow atoms (device) + TFAM (golden) to coexist during transition? (Capability
   is unaffected either way; this is a consistency call.) Recommend: migrate
   opportunistically, single substrate as the target.
2. **`#`→tail mangling scheme.** EXTENT: must map `#M`,`#B`,`#T`,`#H` to unique
   lowercase tails that don't collide with existing families/roles. Fix the
   scheme in `habu-extent-typed-tensor-bde435dc` (e.g. a reserved `ext-` or `x`
   prefix). Flagged, not decided here.
