...
```sh
bin/hb test/compiler/tic6x-asm.f </dev/null
bin/hb test/compiler/arm32-asm.f </dev/null
...
On 2026-09-10, the C66x focused test and all 47 GNU comparisons passed.
LLVM 22.1.8 assembled 92 ARM/Thumb reference cases, but their Habu comparisons
and focused ARM test await the compiler's local `immediate` name-resolution
fix. The operand name is preserved as a regression trigger. No target program
has been executed on hardware; no full native compiler-suite result is claimed
for these isolated additive modules.
