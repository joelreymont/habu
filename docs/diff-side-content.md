...
targets carry SHA-256 of the empty byte string; an empty file must set binary to
zero.
...
SIDE-CHUNK  ( bytes -- )
SIDE-END    ( -- )
SIDE        ( present kind binary path body -- )
ROW-END     ( -- )
FINISH      ( -- artifact )
```
...
the canonical zero digest. Streaming body hashes live in writer-owned SHA-256
state, so unrelated SHA operations between chunks cannot alter the result.
`SIDE` prevalidates kind, path, body, binary classification, and output capacity
before changing writer state.

Errors are package-owned public constants: `DIFF-CONTENT:E-SYNTAX`,
`DIFF-CONTENT:E-CAPACITY`, and `DIFF-CONTENT:E-DIGEST`.
...
Validation rejects noncanonical empty binary files before exposing rows.
