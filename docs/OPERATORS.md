...
**Total Operators**: 266 (238 core + 28 c[ad]r combinations)
**Test Coverage**: 590 compiler tests (100% passing)
...
Summary:
- **Arithmetic** (49): +, -, *, /, mod, rem, min, max, abs, 1+, 1-, signum, gcd, lcm, isqrt, integer-length, expt, square, neg, log2, cube, double, half, avg, range, quot, reciprocal, sqr, lerp, median3, map-range, nearest-multiple, triangle-number, factorial, fibonacci, next-power-of-2, prev-power-of-2, pentagonal-number, hexagonal-number, abs-diff, distance, pow2, pow10, lucas-number, min*, max*, sum, product, negate, sqr-diff
- **Arithmetic Aliases** (2): inc, dec
- **Rounding** (10): floor, ceiling, truncate, round, ffloor, fceiling, ftruncate, fround, round-up-to, round-down-to
- **Comparison** (14): <, >, =, <=, >=, /=, equal, eql, eq, min3, max3, sign, compare, clamp-01
- **Boolean** (7): and, or, not, implies, xnor, nand, nor
- **Bitwise** (40): logand, logior, logxor, lognot, ash, logcount, logtest, logbitp, lognand, lognor, logeqv, logandc1, logandc2, logorc1, logorc2, set-bit, clear-bit, toggle-bit, mask, low-bits, rotl, rotr, bit-field, bit-field-set, hamming-distance, parity, reverse-bits, count-leading-zeros, count-trailing-zeros, bit-width, msb-position, lsb-position, bit-set?, bit-clear?, test-bit, bit-count, popcount, all-bits-set?, any-bits-set?, no-bits-set?
- **Predicates** (50): zerop, plusp, minusp, evenp, oddp, null, numberp, integerp, atom, listp, consp, symbolp, between, power-of-2?, aligned?, high-bit?, within?, outside?, same-sign?, nonzero?, divisible?, multiple-of?, divides?, coprime?, positive-or-zero?, negative-or-zero?, strictly-between?, approximately?, square-number?, in-range?, out-of-range?, triangular-number?, pentagonal-number?, hexagonal-number?, one?, negative-one?, positive-power-of-2?, pair?, empty?, in-open-range?, in-closed-range?, prime?, composite?, negative?, nonnegative?, nonpositive?, exact-power-of-2?, multiple?, factor?
- **Scheme Aliases** (6): zero?, positive?, negative?, even?, odd?, number?
- **Control Flow** (17): if, cond, case, when, unless, progn, begin, if-let, when-let, select, if-not, when-not, unless-let, cond-let, and-let*, or-let, dotimes
- **Variables** (6): let, let*, let1, setq, incf, decf
- **Functions** (2): lambda, defun
- **Macros** (1): defmacro
- **Data** (10): quote, cons, car, cdr, list, first, rest, second, third, fourth
- **Utility** (22): identity, clamp, align-up, align-down, constrain, bool->int, int->bool, negate-if, swap-if, wrap, wrap-range, toggle, flip, normalize, denormalize, clamp-positive, clamp-negative, saturate, identity?, different?, max-of-3, min-of-3
...
**Total: 238 core operators + 28 c[ad]r combinations = 266 operators**
