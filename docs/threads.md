...
- **Atomics (step 3): done + validated.** `atomic@` (LDAR `0xC8DFFD29`), `atomic!`
  ...
  **rebuilds to byte-for-byte fixpoint** with them (`hb-new` 2252992 vs 2248896
  baseline) via `BF-BUILD-ALL` on a temp engine (shared `bin/hb` untouched), and
...
verified compile/define-only, `habu2.f:1168/2107/2183`); `BODYLEN`+`BODYBUF`;
