...
The guard is the native `bin/hb` refresh/fixpoint path in
`tools/build-fixpoint-main.f -- install --force`: it concatenates the native toolchain
sources and compiles them under `bin/hb` itself. The standalone errors (exit 70) on any undefined word —
...
util walk rt crash macho engine engine2 stage2`), and the exact-tree refresh checks
the native self-rebuild directly. `tools/build-fixpoint-main.f -- install --force` refreshes `bin/hb` from
the current native sources and verifies the output is byte-identical. No-binary
recovery runs `tools/bootstrap.sh`: Gforth creates only private `HB_TMP`
artifacts, then the installed `bin/hb` runs the same native fixpoint. Refresh the
exact tree before running `test/run.f`; a broken subset, code generator, or
fixpoint fails during that refresh.
