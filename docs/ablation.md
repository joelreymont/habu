...
## Manual GPT-2 checkpoint tests

These exceptional leaves take a checkpoint root as their single argument and
stay outside `maki/test.f`. The artifact leaf needs only the checkpoint; the
other listed leaves also require CUDA.

| Member | Contract | Rerun |
|--------|----------|-------|
| `maki/infer/gpt2-generate-test.f` | production generation against a real checkpoint | `bin/hb --load maki/infer/gpt2-generate-test.f -- <checkpoint-root>` |
| `maki/infer/gpt2-token-guard-test.f` | production token-bound refusal against a real checkpoint | `bin/hb --load maki/infer/gpt2-token-guard-test.f -- <checkpoint-root>` |
| `maki/infer/gpt2-serve-close-test.f` | post-service checkpoint owner census | `bin/hb --load maki/infer/gpt2-serve-close-test.f -- <checkpoint-root>` |
| `maki/infer/gpt2-artifact-test.f` | exact pinned sizes/hashes and the real 160-tensor census | `bin/hb --load maki/infer/gpt2-artifact-test.f -- <checkpoint-root>` |
| `maki/infer/gpt2-model-device-test.f` | real OPEN upload, ownership, refusal rollback, CLOSE, and unmap | `bin/hb --load maki/infer/gpt2-model-device-test.f -- <checkpoint-root>` (device host) |
