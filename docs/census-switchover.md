...
Correction to the premise: the source-local checker self-typing boundaries
(`*-RC>PTR`/`USIGS-CELL-AT`, combinators) are NOT sum/option-dischargeable —
they are rc>ptr self-casts and combinator boundaries owned by other capability
dots.

The ADT-dischargeable sites are the null/unset-sentinel boundaries (currently
`builder-emit`/`stdlib-boundary`/`prim-axiom`), which exist only because the
checker has no option/null-literal:
- `src/habu/debug.f:37 BP-NULL -- ptr u8` (null code-pointer sentinel) → option<ptr>
- `lib/task.f:99 TASK-NULL -- ptr a` → option<ptr>
- `src/habu/habu2.f c-defer-find-unset / c-defer-cell` (unset sentinel xt)
  → option<xt> / typed defer (`src/habu/habu2.f`; DEFER-UNSET is option<xt>)
- `src/os/env-base.f:79 NULL$ -- ptr u8 n` (absent env values) → option<str>
- `src/habu/hide.f:20 BFR-USIG-END-PTR` reset sentinel — weaker candidate.

Classification: ~6–8 sites discharge once option/sum land; the overwhelming
majority (engine/asm builder-emit boundaries, raw-syscall boundaries, nominal
casts, FFI boundaries) are genuinely unexpressible and stay. The ADT campaign
itself adds zero unchecked boundaries; discharges are separate follow-on dots.
...
- **Wave A (after 8+9+12)** — option<scalar>: the 65 single-value+flag
  ...
  lib/string,date,float,map,process-env + tools/{imgdump,imagedisasm,date,json}.
  FIND-SUB/INDEX-OF have the widest caller edits.
