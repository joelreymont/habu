...
elided, keeping the compiled word minimal — the diagnostic and the `HB-ERROR:BAD-TAG`
...
`HB-ERROR:BAD-TAG` die; the entry/seed/mode axis is folded into the artifact key, the
...
relocatable `TF-STR` pool — and exits `HB-ERROR:BAD-TAG` (process exit status 85,
