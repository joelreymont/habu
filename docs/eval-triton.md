...
as this repository commits no host-language helpers; the Habu-side reducers
`tools/ptx/bandwidth.f`,
...
- **Triton column:** `/tmp/gemm-triton-gb10.py` (verbatim below, out-of-tree per
  this doc's convention — this repository commits no `.py`), the
...
Out-of-tree per this doc's convention (this repository commits no `.py`), the
same way the Orin step-1 `/tmp/triton_matmul.py` is quoted
verbatim above.
