...
Gate: passing; 1m38.25s on 2026-06-27 on Linux/aarch64 with batched warm-image trust export, batched repair-diagnostic JSONL assertions, measured 8-way outer and 4-way nested gate pools, early independent builder phases, warm checker/tool images, shared hb-build maker cache, single-build strict positive AOT coverage, deduplicated AOT negative coverage, REPL builder coverage folded into `tools/hb-build-test.f`, and the checked DAG pool
