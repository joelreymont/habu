...
Design-reference (2026-07-18, tfinite): implements `docs/effects.md` § R8-1 (schema encoding, fields) and § R8-3 (mandatory Maki registration). Acceptance: one sealed canonical `effect-row` is mandatory per op schema and stored only through the typed nominal handle path; registration validates every operand/attribute/capability slot against schema arity/kind and derives schema identity from canonical row contents, not the handle; the registry's fail-closed membership gate (the `OPR-REF`/`E-OPR-INCOMPLETE` precedent) rejects a missing, forged, stale, cross-owner, raw-stored, duplicate, wrong-kind, or inconsistent-PURE row — mutation-matrix rows 14, 15.
