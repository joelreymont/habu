...
- tools/build-fixpoint.f: BF-APPEND-SNAP-KEEP/-REPL/-MARK/-BUILD,
  ...
  BUILD-SNAP-HDR source-local TRUST assertion (line 582 area).
...
- Source-local snap/snc/snd TRUST assertions that lose all callers;
  layout.f SNAP-CELL/STB/SDB-class constants that lose all users; and
  engine-suite/gate tests exercising restore.
...
fixpoint), run the focused restore and full native gates, expect a binary-size
...
SNAPSHOT-PREPARE + persist hooks, SNAP-CHECK/INSTALL hooks, source-local
TRUST assertions owned only by snapshot code, snap layout constants, snap
