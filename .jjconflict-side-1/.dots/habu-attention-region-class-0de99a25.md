...
FOLDED IN from habu-automatic-aggressive-fusion-828cdeb3 leg (c) at its 2026-07-15 closure: when the attention region class lands, its acceptance also covers the softmax+matmul fused-attention proof (ROW-REDUCE->MATMUL fusion for the attention pattern) - one fused region vs the split baseline, device-correct, with the ablation-bench evidence pattern of maki/fusion-bench-device-test.f.
