...
src/habu/driver-io.f:3-152 exposes 35 DRV-* words globally. It is appended to build/stdin images and called by stage2, AOT, snapshot, and maker paths, yet most status rendering, buffer/cursor, size-map, reload-retirement, and failure helpers are implementation details. Wrap it in package DRIVER-IO; export only the actual emit-image, success/failure, size-map, and reload-retirement entry points used across build boundaries; keep scratch buffers, low-level writes, formatting, and state private. Update all generated/direct callers without compatibility globals and ensure package qualification survives the appended-source/AOT/snapshot paths. Preserve stdout/stderr, exit codes, engine bytes, size-map rows, reload behavior, and build ordering exactly. Add old-global/private rejects, qualified public positives, and byte-golden success/failure/size-report fixtures on every command path. Measure dictionary-name bytes, JIT/DATA/CODELEN, build image size, and driver latency before/after. Verify stdin/metabuild/maker/AOT/snapshot/fixpoint and both target gates, package/host/dot lints, and full native gate. Parent: habu-pkg-native-build-f598557c.

RETIREMENT CLAUSE (added 2026-08-15, orchestrator): driver-io.f
enters package-diff-lint's OLD-GLOBAL? category today (the
habu2.f/layout.f precedent: a body of existing global code whose
packaging is blocked on THIS dot, which the gate would otherwise
reject every edit to). Completing this dot MUST remove driver-io.f
from that category - the exemption is temporary by construction
and its removal is part of this dot's acceptance.
