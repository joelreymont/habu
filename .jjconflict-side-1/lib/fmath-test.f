\ lib/fmath-test.f - known-value tests for the shared host exp core (FMATH:FEXP).
\ Known values (x1000 round) across the range-reduction domain; the activation
\ and VJP coverage over FMATH:FEXP lives one layer up in maki/fmath-test.f.
...
\ FMATH:FEXP across the range (exercises k = round(x/ln2) reduction)
...
\ central finite difference of FMATH:FEXP at x=1 matches exp(1) (exp' = exp)
