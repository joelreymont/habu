...
\ GETENV and TMP-PATH retain caller/vector cursors only while they run.  Clear
\ those raw host pointers at the capture seam; the restored engine receives its
\ real argv/envp from startup DATA instead.
...
s" ENV-SNAPSHOT-PREPARE" s" --" TRUST
