\ repl.fs — interactive caf REPL (gforth-hosted, drives the live checker). Enter
\ a checked definition and see its inferred effect (✓) or the caf diagnostic (✗);
...
\ checkable caf — it is the harness that RUNS caf on what you type.

require caf.fs

\ The REPL's own words are infrastructure, not caf to be checked — define them
...
: HELP ( -- )
   cr +C ." caf REPL" -X ."  — type a checked definition to see its effect:" cr
...
CHECKING-ON? on                            \ user input in the REPL is checked caf
