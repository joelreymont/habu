...
pub const Emitter = struct {
    ...
    pub fn emit(self: *Emitter, node: *const Ir) Error!void {
        ...
        switch (node.*) {
            ...
            .truename => |op| try self.emitUnaryOp(op.operand, .truename),
            .ensure_directories_exist => |op| try self.emitUnaryOp(op.operand, .ensure_directories_exist),
            ...
            .equalp => |op| try self.emitBinaryOp(op, .equalp),
            ...
            .integerp => |op| try self.emitUnaryOp(op.operand, .integerp),
            .realp => |op| try self.emitUnaryOp(op.operand, .realp),
            ...
            .macroexpand_1 => |op| try self.emitUnaryOp(op.operand, .macroexpand_1),
            ...
            .software_type => try self.emitOp(.software_type),
            .machine_type => try self.emitOp(.machine_type),
            .machine_instance => try self.emitOp(.machine_instance),
            .machine_version => try self.emitOp(.machine_version),
            .software_version => try self.emitOp(.software_version),
            ...
            .array_rank => |op| try self.emitUnaryOp(op.operand, .array_rank),
            .array_total_size => |op| try self.emitUnaryOp(op.operand, .array_total_size),
            .array_dimensions => |op| try self.emitUnaryOp(op.operand, .array_dimensions),
            ...
            .copy_symbol => |op| try self.emitBinaryOp(op, .copy_symbol),
            .makunbound => |op| try self.emitUnaryOp(op.operand, .makunbound),
            .set_sym_val => |op| try self.emitBinaryOp(op, .set_sym_val),
            ...
            .fixnum_le => |op| try self.emitBinaryOp(op, .fixnum_le),
            .fixnum_lt => |op| try self.emitBinaryOp(op, .fixnum_lt),
            .fixnum_gt => |op| try self.emitBinaryOp(op, .fixnum_gt),
            .fixnum_ge => |op| try self.emitBinaryOp(op, .fixnum_ge),
            .fixnum_eq => |op| try self.emitBinaryOp(op, .fixnum_eq),
