...
: PFX-FILES ( n [ n ptr n ptr u8 n -- ] -- ) {: parts row :}
   ...
   then
   parts PFX-OWNER and 0 <> if
      PFX-COMMON LPOWNERGUARD s" src/core/checker-owner-guard.f" row execute
   then
...
: PFX-LOAD-STDLIB-FILES ( -- )
   PFX-BASELIB PFX-RESTLIB or ['] PFX-LOAD-ROW PFX-FILES ;
...
: PFX-LOAD-INTMARK ( -- )
   PFX-INTERNAL ['] PFX-LOAD-ROW PFX-FILES ;
...
: PFX-LOAD-TOPROW ( -- )
   PFX-TOPROW ['] PFX-LOAD-ROW PFX-FILES ;
...
: PFX-PROVIDE-CORE-FILES ( -- )
   PFX-CORE PFX-DYNAMIC or PFX-SEAL or PFX-ARGV or PFX-INTERNAL or PFX-TOPROW or ['] PFX-PROVIDE-ROW PFX-FILES ;
...
: PFX-PROVIDE-STDLIB-FILES ( -- )
   PFX-BASELIB PFX-RESTLIB or ['] PFX-PROVIDE-ROW PFX-FILES ;
...
: EMIT-HOST-LOAD-PREFIX ( -- )
   ...
   PFX-OWNER ['] PFX-LOAD-ROW PFX-FILES
   PFX-OWNER ['] PFX-PROVIDE-ROW PFX-FILES
   ...
   PFX-SEAL ['] PFX-LOAD-ROW PFX-FILES
   PFX-LOAD-STDLIB-COLD
   PFX-LOAD-SCRIPT-ARGV-COLD
   PFX-LOAD-INTMARK-COLD
   PFX-LOAD-TOPROW-COLD
...
: EMIT-LOC-FIND ( -- )
   ...
   ll LBL,  10 0 CMPI,  C-LT lmiss BCOND,
      ...
      lcmp LBL,  13 6 CMP,  C-GE lhit BCOND,
         ...
         \ Byte-exact, unlike dictionary lookup: a local answers to its DECLARED
         \ spelling only, so a body may write a word in the word's own case while
         \ a local of the same letters is live. src/core/checker.f LOC-REF? reads
         \ the two names the same way, and docs/forth.md states the rule.
...
: EM-AOT-PATCH-SITES ( -- )
   ...
   ploop LBL,  22 23 CMP,  C-GE pdone BCOND,
      ...
      pbound B,                                      \ else an absolute primitive index
   prec LBL,
      5 SITE-TARGET-MASK LIT64,  25 25 5 AND,
      \ The base the records pass left behind: it registered LAOTNREC records and
   ...
   pbound LBL,
      ...
      ppatch B,
   pname LBL,
      5 SITE-TARGET-MASK LIT64,  4 25 5 AND,         \ x4 = name-off u32, tags cleared
...
: CAPTURE-STRING ( -- )
   LMAIN LABEL@ LKWSQ     2 ['] CAPTURE-PLAIN-STRING   CFN-ENTRY
   LMAIN LABEL@ LKWCQ     2 ['] CAPTURE-PLAIN-STRING   CFN-ENTRY
   LMAIN LABEL@ LKWDOTQ   2 ['] CAPTURE-PLAIN-STRING   CFN-ENTRY
   LMAIN LABEL@ LKWESQ    3 ['] CAPTURE-ESCAPED-STRING CFN-ENTRY
...
: JIT ( -- )
   ...
   LBL PROF:LPROFJSON !  LBL PROF:LPROFNUM !  LBL PROF:LPROFPCT !
   LBL PROF:LPROFNAME !  LBL PROF:LPROFQUAL !  LBL PROF:LPROFSYNC !
   LBL PROF:LPROFROW !
...
\
\ A NAME THAT RESOLVES TO NEITHER TABLE KEEPS ITS NAME. That is not a failure:
\ a PARTIAL capture - a stripped application, a chain capture, any window
\ smaller than the whole tree - calls words the engine it boots into carries
\ from its own prefix, and no index of this payload can name one of those. The
\ row then sets SITE-NAME-TAG and the seed resolves it the way it always did,
\ in the scope the capture recorded, through the sealed-WID gate. A qualified
\ scope (WID-QUAL) takes the same path, because the qualifier names a package
\ whose id belongs to the booting engine and not to this image.
...
: AOT-BIND-SITE ( n -- n ) {: s:n :}
   ...
   w WID-QUAL = if -1 exit then                      \ a qualified name is the seed's to resolve
   s AOT-SITE-NOFF AOT-POOL$ {: a:ptr u:n :}
   a u w AOT-PRIM-INDEX {: p:n :}
   p 0 >= if p exit then
   a u w AOT-PAYLOAD-INDEX {: k:n :}
   k 0 < if -1 exit then                             \ the image does not carry it: keep the name
   \ Marked as payload-relative: the seed adds the base it can compute and this
   \ build cannot, because a partial image's records land above a cold prefix
   \ that does not exist yet here.
   k SITE-TARGET-MASK and SITE-REC-TAG or ;
...
: AOT-MEMO-FIND ( n n -- n ) {: noff:n w:n :}   \ -2 when the pair is not memoized
...
: AOT-SITE-INDEX ( n -- n ) {: s:n :}
   ...
   hit -2 <> if hit exit then                        \ -1 is an answer too: the name stays
   s AOT-BIND-SITE {: idx:n :}
   noff w idx AOT-MEMO-ADD
   idx ;

: AOT-SITE-TARGET ( n -- n ) {: s:n :}
   s AOT-SITE-INDEX {: k:n :}
   k 0 >= if k exit then
   s AOT-SITE-NOFF SITE-TARGET-MASK and SITE-NAME-TAG or ;
