...
pub const Builtins = struct {
    ...
    pub fn init(heap: *Heap) !Builtins {
        return .{
            ...
            .@"%array-rank" = try heap.intern("%array-rank"),
            .@"%array-total-size" = try heap.intern("%array-total-size"),
            .@"%array-dimensions" = try heap.intern("%array-dimensions"),
...
pub const Compiler = struct {
    ...
    fn compileLambdaCore(self: *Compiler, args: Value, env: *const Env, return_type: ?Value) anyerror!*Ir {
        ...
        const materializeSpecialSym = struct {
            fn run(_: *Compiler, heap: *runtime.Heap, sp: SpecialParam) anyerror!Value {
                return if (sp.sym_uid != 0) blk: {
        ...
        const wrapSpecialBindings = struct {
            fn run(comp: *Compiler, heap: *runtime.Heap, specials: []const SpecialParam, body: *const Ir) anyerror!*Ir {
                ...
                for (specials, 0..) |sp, i| {
                    special_syms[i] = try materializeSpecialSym(comp, heap, sp);
        ...
        if (special_params.items.len > 0) {
            ...
            defer lambda_special_bindings.deinit(self.allocator);
            for (special_params.items) |sp| {
                if (sp.kind == .aux) continue;
                try lambda_special_bindings.append(self.allocator, .{
                    .sym = try materializeSpecialSym(self, heap, sp),
                    .idx = sp.idx,
                    .stage = switch (sp.kind) {
                        .required => .required,
                        .optional => .optional,
                        .rest => .rest,
                        .key => .key,
            ...
            lam_ir.lambda.special_bindings = try self.allocator.dupe(Ir.SpecialBinding, lambda_special_bindings.items);
        }
    ...
    fn compileFunctionDefine(self: *Compiler, sym_val: Value, name: []const u8, idx: u16, value_ir: *const Ir) anyerror!*Ir {
        ...
        return try self.builder.setSymbolFunction(sym_ir, value_ir);
    }
    ...
    fn compileEvalWhen(self: *Compiler, args: Value, env: *const Env) anyerror!*Ir {
        ...
        const heap = self.heap orelse return error.InvalidPackage;
        const cl_compile = (try heap.internInPackage("COMMON-LISP", "COMPILE")) orelse return error.InvalidPackage;
        const cl_load = (try heap.internInPackage("COMMON-LISP", "LOAD")) orelse return error.InvalidPackage;
        const cl_eval = (try heap.internInPackage("COMMON-LISP", "EVAL")) orelse return error.InvalidPackage;

        // Check situations for :compile-toplevel/:load-toplevel/:execute and
        // legacy CLTL COMPILE/LOAD/EVAL names still used in upstream systems.
    ...
    const unary_dispatch = [_]UnaryEntry{
        ...
        .{ .field = "%array-rank", .tag = .array_rank },
        .{ .field = "%array-total-size", .tag = .array_total_size },
        .{ .field = "%array-dimensions", .tag = .array_dimensions },
    ...
    fn compileUnaryPrim(self: *Compiler, args: Value, env: *const Env, prim: PrimTag) anyerror!*Ir {
        ...
        return switch (prim) {
            ...
            .array_rank => try self.builder.arrayRank(operand),
            .array_total_size => try self.builder.arrayTotalSize(operand),
            .array_dimensions => try self.builder.arrayDimensions(operand),
...
test "compile defun typed name" {
    ...
    try testing.expect(ir_def.* == .set_symbol_function);
...
test "compile defun sets lambda name" {
    ...
    try testing.expect(ir_def.* == .set_symbol_function);
    try testing.expect(ir_def.set_symbol_function.right.* == .lambda);
    try testing.expectEqual(foo_sym.raw, ir_def.set_symbol_function.right.lambda.name.raw);
}
