...
\ owner of that list, so this front end refuses `if` exactly where the legacy
...
: DERIVE-ONE ( ptr u8 n -- )            \ apply one feature + emit its event
   DERIVE-GUARD
   2dup s" eq" CORE-STR=CI IF 2drop FAM @ DRV-EQ DERIVE-SET FAM @ DRV-EQ EMIT-DERIVE EXIT THEN
   2dup s" hash" CORE-STR=CI IF 2drop FAM @ DRV-HASH DERIVE-SET FAM @ DRV-HASH EMIT-DERIVE EXIT THEN
