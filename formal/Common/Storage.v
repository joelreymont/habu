(* Habu.Common.Storage — a model of the compiler IR storage and lifetime layer:
   ...
   region inside a single 64K mapping, and a module-serial budget, and it takes
   ...
   5. Growth allocates from the context, and this model does not charge it.
      ...
      65408-byte scratch region.  In this model growth is free, so every
  ...
  10. Machine integers are `nat`.  IR-CTX:GEN-NEXT-N and IR-ARENA:AGEN-NEXT-N
   ...
   the committed ceiling".  The real bound is the context's single 64K
   mapping, and it bites first.  Reaching capacity C costs 8 + 16 + ... + C
   cells of a 65408-byte scratch region, so the largest capacity an arena can
   ever reach — even as the only allocation in its context — is 2048 cells.
   ...
   untouched — but the ceiling a caller commits to above 2048 cells is
...
Definition map_bytes : nat := 65536.         (* IR-CTX:MAP-BYTES, context.f:73 *)
...
Example ex_scratch_bumps :
  ...
  /\ scratch_take 128 65409 = None
  /\ scratch_take 65536 1 = None.
Proof. repeat split; vm_compute; reflexivity. Qed.
...
(* FINDING 3, as arithmetic.  `growth_bytes cap` is what the doubling schedule
   ...
   cells.  A ceiling of 2048 cells fits the scratch region; the next capacity
   the doubling would reach does not, so 4096 is unreachable and any ceiling
   above 2048 cannot be enforced by IR-ARENA:GROW. *)
Definition growth_bytes (cap : nat) : nat := (2 * cap - seed_cells) * slot_bytes.

Example arena_ceiling_beyond_the_mapping :
  Nat.leb (growth_bytes 2048) scratch_cap = true
  /\ Nat.ltb scratch_cap (growth_bytes 4096) = true.
Proof. split; vm_compute; reflexivity. Qed.

Example ex_constants :
  hdr_bytes = 128 /\ scratch_cap = 65408 /\ seed_cells = 8
