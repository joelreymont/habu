...
\ a thread-0 sequential fold. Inactive lanes (col >= k) are identity-seeded by
\ EMIT-ROW-LOAD (-inf), so max ignores them and exp(-inf-max)=0 drops out of the
\ sum - no per-collective masking needed. Param registers (CG-SM-PARAMS):
\ in=%rd1, out=%rd2, k=%r1. Load after lib/errors.f, lib/string.f, lib/fmt.f,
\ src/arch/ptx/emit.f, and lib/ptx/cg.f. Checked Habu; SM-BLK threads per block.

256 constant SM-BLK                       \ threads per block (= reduction fold bound)
...
: EMIT-REDUCE ( n n -- n ) {: tile op :}        \ op: 0=max 1=add 2=min
   ...
   SB-RESET s" st.shared.f32 [" CG-S ra CG-R s" ], " CG-S tile CG-F s" ;" CG-S CG-LINE
   ...
   lloop CG-LDEF
   CG-NEXT-P {: pd :}
   SB-RESET s" setp.ge.u32 " CG-S pd CG-P s" , " CG-S ri CG-R s" , " CG-S SM-BLK SB-U s" ;" CG-S CG-LINE
