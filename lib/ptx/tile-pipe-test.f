...
\  1. The KERNEL: definition IS the positive proof: MM-PIPED composes the typed
\     pipelined register-blocked GEMM - PIPE-SETUP / PIPE-ACC-ZERO / PIPE-LOOP
\     (cp.async double-buffered staging) / STAGE-SLICES / A-FRAG / B-FRAG.V4 /
\     RB-FMA under the checked K-UNROLL / PIPE-STORE - and certifies end to end.
...
\  3. Byte-identity: EMIT-PIPED (typed) emits PTX byte-for-byte identical to the
\     legacy EMIT-MATMUL, proving the typed layer is a PURE checker refinement of
\     the device-proven register-blocked kernel (~283 GFLOP/s on Orin NX).
\
...
: TP-MAIN ( -- )
   ...
   \ part 3: typed emit is byte-identical to the legacy register-blocked emit
   PTX-CAPTURE-ON EMIT-MATMUL PTX-CAPTURE-OFF  PTX-CAPTURE$ {: la:ptr lu:n :}
   s" typed PTX == EMIT-MATMUL PTX (pure type refinement)" T-LABEL
