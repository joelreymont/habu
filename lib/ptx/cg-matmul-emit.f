\ cg-matmul-emit.f - PTX emitters for the REGISTER-BLOCKED tiled SGEMM kernel MM.
\
...
\
\ The raw emitter vocabulary beneath the checked GEMM: constants, entry params,
\ thread/tile setup, cp.async double-buffered staging, the register-blocked
...
\ toward flash-attention. cg-matmul.f EMIT-MATMUL emits a self-contained kernel `MM`.
\
...
\
\ Load after src/arch/ptx/emit.f and lib/ptx/cg.f; emits to stdout.
\ ==============================================================================
