...
\ adjoint of a reduce. Needed by Loom's autograd VJP table (../loom/docs/autograd.md).
TRUSTED: BROADCAST ( uniform<f32> -- tile<f32,b,m> )
   EMIT-BROADCAST ;

\ BLOCK-MAX arg-max SELECT - the one genuinely new primitive the AD layer needs
\ (../loom/docs/autograd.md): the adjoint of BLOCK-MAX. A masked scatter that routes the
