...
: FS-TEST-WALK-THROW-CB ( ptr u8 n -- )
   2drop E-A-BOUNDS throw ;       \ no walk path raises the callback's code

: FS-TEST-WALK-THROW ( -- )
   FS-TEST-ROOT [: FS-TEST-WALK-THROW-CB ;] WALK-FILES ;

: FS-TEST-WALK-NEST-CB ( ptr u8 n -- )
   2drop FS-TEST-ROOT [: 2drop ;] WALK-FILES ;
...
: FS-TEST-NUL-WRITE ( -- )
   S\" /tmp/habu-fs-unsafe\x00tail" s" x" WRITE-ALL ;

: FS-TEST-NUL-APPEND ( -- )
   S\" /tmp/habu-fs-unsafe\x00tail" s" x" APPEND-FILE ;
...
: FS-TEST-THROWS ( -- )
   ...
   [: FS-TEST-NUL-EXISTS ;] E-FS-PATH-UNSAFE TTHROWSQ
   [: FS-TEST-NUL-WRITE ;] E-FS-PATH-UNSAFE TTHROWSQ
   [: FS-TEST-NUL-APPEND ;] E-FS-PATH-UNSAFE TTHROWSQ
...
: FS-TEST-WALK-THROW-CLOSES ( -- )
   ...
   [: FS-TEST-WALK-THROW ;] E-A-BOUNDS TTHROWSQ
