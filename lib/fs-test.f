...
: FS-TEST-WRITE-DIR ( -- )
   0 SCRIPT-ARGV$ s" x" WRITE-ALL ;

: FS-TEST-APPEND-DIR ( -- )
   0 SCRIPT-ARGV$ s" x" APPEND-FILE ;

: FS-TEST-WRITE-MISSING-PARENT ( -- )
   0 SCRIPT-ARGV$ s" missing-parent/out.txt" FS-TEST-OUT JOIN-PATH
...
: FS-TEST-IO ( -- )
   ...
   FS-TEST-IO-PATH FS-TEST-READ-BUF FS-TEST-READ-CAP READ-ALL FS-TEST-WRITE-LEN FS-TEST=
   FS-TEST-READ-BUF FS-TEST-WRITE-LEN s" xyz" FS-TEST$=
...
: FS-TEST-THROWS ( -- )
   ...
   ['] FS-TEST-MISSING-READ catch E-FS-OPEN FS-TEST=
   ['] FS-TEST-READ-TOO-LARGE catch E-FS-CAPACITY FS-TEST=
   ['] FS-TEST-WRITE-DIR catch E-FS-OPEN FS-TEST=
   ['] FS-TEST-APPEND-DIR catch E-FS-OPEN FS-TEST=
   ['] FS-TEST-WRITE-MISSING-PARENT catch E-FS-OPEN FS-TEST=
   ['] FS-TEST-APPEND-MISSING-PARENT catch E-FS-OPEN FS-TEST= ;
