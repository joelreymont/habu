...
\
\ The module lives in `package RENDER`. Callers build report text through the
\ qualified public API: RENDER:RESET clears the shared buffer, RENDER:RB+ appends
\ a counted string, RENDER:RB# appends a signed decimal, RENDER:NL appends a
\ newline, RENDER:RB$ hands back the accumulated bytes, and RENDER:CM / LBAR /
\ BAR / RBAR write the CSV comma and the markdown table-cell bars. The buffer
\ append/get primitives keep their bare-operator RB+/RB#/RB$ tails because a bare
\ `+` public word would shadow the arithmetic `+` used throughout the module. The
\ raw byte/char emitters, the JSON/CSV formatting toolkit, the fixed-decimal
\ formatters, the key/value DSL, and every buffer and state cell are
\ package-private.
\
...
: MD-N ( ptr u8 n n -- ) {: la:ptr ln:n v:n :} s" - " RB+ la ln RB+ s" : " RB+ v RB# NL ;
...
: CVN ( ptr u8 n n -- ) {: la:ptr ln:n v:n :} la ln RB+ CM v RB# NL ;
: CVR ( ptr u8 n n n -- ) {: la:ptr ln:n num:n den:n :} la ln RB+ CM num den RB-RATIO4 NL ;
