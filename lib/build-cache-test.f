...
: CHECK-SYMLINK-ROOT-REJECT ( -- )
   A$ MAKE-DIRS
   B$ EXISTS? if B$ REMOVE-TREE then
   A$ B$ MAKE-SYMLINK
   BUILD-CACHE:RESET
   B$ BUILD-CACHE:ROOT!
   [: BUILD-CACHE:ROOT$ 2drop ;] catch E-BUILD-PATH T=
   BUILD-CACHE:CAUSE E-FS-PATH-UNSAFE T=
   BUILD-CACHE:SELECTED-ROOT$ B$ T$=
   B$ SYMLINK? TTRUE
   B$ REMOVE-FILE ;
