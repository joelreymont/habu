...
\
\ TRUSTED PLUMBING. catch admits only a stack-preserving quotation (checker RSCATCH
\ unifies its in/out rows), and a nested quotation captures no enclosing local, so the
\ body's arbitrary result row S cannot be threaded through catch in checked code - the
\ same limit that makes lib/test/snap.f SNAP= and combinators.f EACH/MAP/FOLD TRUSTED.
\ WITH-BYTES is therefore TRUSTED with a checker-typed signature enforced at every call
\ site (the static role-swap matrix in memory-test.f proves the boundary). The mapping
\ and body xt are parked off the data stack (WB-CUR-*, save/restored to locals per call
\ so nesting rides the native call stack) and the caught quotations take no data-stack
\ argument, so a throw leaves the row clean (row ++ code), never restored buffer cells.
private
...
: WB-COMBINE ( n n -- )              \ (primary cleanup -- ) primary error wins; else cleanup propagates
   over if drop throw else nip dup if throw then drop then ;
...
TRUSTED: WITH-BYTES ( R CAD-NUM:alloc-byte-len [ R ptr u8 CAD-NUM:alloc-byte-len -- S ] -- S )
   {: body :}
