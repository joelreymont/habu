...
424243 constant FMT-ATOMIC-SEED
424244 constant FMT-ATOMIC-FAIL-SEED
...
: FMT-TEST-ATOMIC-ALIASES ( -- )
   SB-RESET s" builder-source" SB-APPEND
   FMT-ATOMIC-PATH SB$ ATOMIC-WRITE-FILE
   FMT-ATOMIC-PATH FMT-READ-BUF 64 READ-ALL 14 T=
   FMT-READ-BUF 14 s" builder-source" T$=
   SB-RESET FMT-ATOMIC-PATH SB-APPEND
   SB$ s" builder-target" ATOMIC-WRITE-FILE
   SB$ FMT-ATOMIC-PATH T$=
   FMT-ATOMIC-PATH FMT-READ-BUF 64 READ-ALL 14 T=
   FMT-READ-BUF 14 s" builder-target" T$=
   \ Both creation paths apply the process umask to the same requested mode.
   FMT-REMOVE-PATH s" mode-control" WRITE-ALL
   FMT-ATOMIC-PATH STAT-MODE FS-MUT-MODE-PERM and
   FMT-REMOVE-PATH STAT-MODE FS-MUT-MODE-PERM and T=
   FMT-REMOVE-PATH REMOVE-FILE ;

: FMT-ATOMIC-TEMP ( n n -- ptr u8 n ) {: seed:n attempt:n :}
   FMT-ATOMIC-PATH seed attempt FS-MUT-BUILD-ATOMIC-TMP ;

: FMT-TEST-ATOMIC-COLLISION ( -- )
   FMT-ATOMIC-SEED 0 FMT-ATOMIC-TEMP
   FMT-COLLIDE-BUF FMT-COLLIDE-U FMT-COPY!
   FMT-COLLIDE s" unrelated" WRITE-ALL
   FMT-ATOMIC-PATH s" replacement" FMT-ATOMIC-SEED FS-MUT-ATOMIC-WRITE-SEED
   FMT-ATOMIC-PATH FMT-READ-BUF 64 READ-ALL 11 T=
   FMT-READ-BUF 11 s" replacement" T$=
   FMT-COLLIDE FMT-READ-BUF 64 READ-ALL 9 T=
   FMT-READ-BUF 9 s" unrelated" T$=
   FMT-ATOMIC-SEED 1 FMT-ATOMIC-TEMP
   FMT-UNIQUE-BUF FMT-UNIQUE-U FMT-COPY!
   FMT-UNIQUE EXISTS? TFALSE
   FMT-COLLIDE REMOVE-FILE ;

: FMT-TEST-ATOMIC-FAILURE ( -- )
   FMT-MKDIR-PATH MAKE-DIR
   FMT-MKDIR-PATH FMT-ATOMIC-FAIL-SEED 0 FS-MUT-BUILD-ATOMIC-TMP
   FMT-UNIQUE-BUF FMT-UNIQUE-U FMT-COPY!
   [: FMT-MKDIR-PATH s" replacement" FMT-ATOMIC-FAIL-SEED FS-MUT-ATOMIC-WRITE-SEED ;]
   E-FS-IO TTHROWSQ
   FMT-MKDIR-PATH DIR? TTRUE
   FMT-UNIQUE EXISTS? TFALSE
   FMT-MKDIR-PATH REMOVE-DIR ;
...
: FMT-TEST-TEMP-COLLISION ( -- )
   ...
   FMT-TEMP-BASE s" collide" FMT-TMP-SEED 1 FS-MUT-BUILD-TEMP-TRY
   FMT-COLLIDE-BUF FMT-COLLIDE-U FMT-COPY!
   FMT-COLLIDE FMT-UNIQUE T$= ;

: FMT-TEMP-EXHAUST-CANDIDATE ( n -- ptr u8 n ) {: attempt:n :}
   FMT-TEMP-BASE s" exhaust" FMT-TMP-SEED attempt FS-MUT-BUILD-TEMP-TRY ;

: FMT-TEMP-EXHAUST-PREP ( -- )
   0 begin dup FS-MUT-TMP-RETRIES < while
      dup FMT-TEMP-EXHAUST-CANDIDATE MAKE-DIR
      1+
   repeat drop ;

: FMT-TEMP-EXHAUST-CLEAN ( -- )
   0 begin dup FS-MUT-TMP-RETRIES < while
      dup FMT-TEMP-EXHAUST-CANDIDATE REMOVE-DIR
      1+
   repeat drop ;

: FMT-TEST-TEMP-EXHAUST ( -- )
   FMT-TEMP-EXHAUST-PREP
   [: FMT-TEMP-BASE s" exhaust" FMT-TMP-SEED FS-MUT-MAKE-TEMP-DIR-SEED 2drop ;]
   E-FS-IO TTHROWSQ
   FMT-TEMP-EXHAUST-CLEAN ;
