...
: STAT-EXISTS? ( ptr u8 ptr u8 n n -- bool )
   {: path output stat-fn:n errno-fn:n :}
   RESET path 0 READABLE! output STAT-BYTES 1 WRITABLE!
...
: IDENTITY ( ptr u8 n ptr u8 n n -- n n bool )
   {: text bytes:n buffer stat-fn:n errno-fn:n :}
   text bytes CHECK-PATH
   text bytes buffer STAT-BYTES + FS-PATHZ-INTO buffer stat-fn errno-fn STAT-EXISTS? if
      HB-TARGET-MACOS? if buffer UNSIGNED-INT else buffer FS-U64@ then
      buffer INODE-OFFSET + FS-U64@ FS-TRUE
...
: COMPARE-IDENTITIES ( ptr u8 n ptr u8 n ptr u8 CAD-NUM:alloc-byte-len -- bool )
   ...
   right right-bytes buffer stat-fn errno-fn IDENTITY {: right-device:n right-inode:n right-exists:bool :}
